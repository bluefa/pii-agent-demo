...
| 대상 | 확인 내용 | 소스 |
|------|----------|------|
| API | 사용 가능한 엔드포인트, 요청/응답 구조, 비동기 여부 | `docs/swagger/*.yaml` |
| 기존 UI | 수정 대상 컴포넌트, 유사 패턴이 있는 다른 컴포넌트 | `app/components/**` |
| 데이터 모델 | 관련 타입, 상태 enum, 에러 유형 | `lib/types.ts`, `lib/types/*.ts` |
| Mock 동작 | 시뮬레이션 방식, 지연 시간, 성공/실패 비율 | `lib/mock-*.ts` |
| 상태 흐름 | ProcessStatus 전이, 현재 단계에서의 전후 관계 | `lib/process/calculator.ts` |

### 출력: 현황 보고

사용자에게 아래 형식으로 보고한다. 이 보고가 이후 모든 논의의 기반이 된다.

```markdown
## 현재 상태 (코드 기준)

**API**: [비동기/동기], [엔드포인트 목록 테이블]
**UI**: [기존 컴포넌트명], [현재 동작 설명]
**데이터**: [관련 타입 요약], [상태 모델]
**Mock**: [동작 방식], [지연/확률]
```
