...
| 동작 중 Terraform task 개수 | 제안 파생 지표: `count(task WHERE kind='TERRAFORM_JOB' AND status='IN_PROGRESS')` (ADR-021에 *명명된* metric 아님) | 🆕 위와 동일 (⚠️ 주의 1) |
...
| 항목 | 필드 | API |
|---|---|---|
| task 노드 그래프 (seq 순 선형) | `task[]`: `seq, kind, operation, status, fail_count, error_code, started_at, finished_at` | `PipelineDetail.tasks` 에 포함 (P4 응답에 embed — 별도 목록 엔드포인트 두지 않음) |
