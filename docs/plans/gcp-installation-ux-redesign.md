...
> 전체 진행률 바 + "GCP 리소스 생성 / 에이전트 연동" 상태 표시 + 조치/실패 카드만 노출
...
| 단계 표시 | TF 용어 직접 노출 | "GCP 리소스 생성 / 에이전트 연동" (카운트 없이 상태만) |
...
| 내부 모델 | → 사용자에게 보이는 라벨 | 설명 |
|-----------|------------------------|------|
| `serviceTfStatus` | **GCP 리소스 생성** | 고객 GCP 프로젝트에 네트워크/IAM 등 리소스 생성 |
| `bdcTfStatus` | **에이전트 연동** | 에이전트 연결을 위한 설정 수행 |
| `regionalManagedProxy` / `pscConnection` | **조치 필요 카드** | 사용자가 직접 수행해야 하는 항목 (필요할 때만 표시) |

- 리소스를 일일이 나열하지 않음
- 그룹별 카운트 없이 **집계 상태만 표시** (완료/진행 중/대기 중/실패)
- 전체 진행률은 헤더 `(n/m 완료)`에서 한 번만 표시 → 카운트 혼란 제거
...
│   ├── 상태 아이콘 (●/◐/○/✕) + 상태 텍스트
│   └── 🔄 새로고침 버튼
├── 프로그레스 바 (completedCount / totalCount)
├── 그룹 상태 2줄 (카운트 없이 상태만)
│   ├── GCP 리소스 생성    ─  ● 완료 / ◐ 진행 중 / ○ 대기 중 / ✕ 실패
│   └── 에이전트 연동      ─  ● 완료 / ◐ 진행 중 / ○ 대기 중 / ✕ 실패
├── 안심 문구 (조치/실패 없을 때만)
├── 설치 오류 카드 (실패 시만)
└── 조치 필요 카드 (액션 필요 시만)
...
```typescript
// 그룹별 집계 상태 (카운트 없이 상태만)
const getGroupStatus = (resources, field: 'serviceTfStatus' | 'bdcTfStatus') => {
  if (resources.every(r => r[field] === 'COMPLETED')) return 'COMPLETED';
  if (resources.some(r => r[field] === 'FAILED')) return 'FAILED';
  if (resources.some(r => r[field] === 'IN_PROGRESS') ||
...
| 집계 결과 | 라벨 | 아이콘 | 색상 |
|----------|------|--------|------|
| `COMPLETED` | `완료` | ● | `statusColors.success` |
| `FAILED` | `실패` | ✕ | `statusColors.error` |
| `IN_PROGRESS` | `진행 중` | ◐ | `statusColors.warning` |
| `PENDING` | `대기 중` | ○ | `statusColors.pending` |
...
```
┌──────────────────────────────────────────────────────┐
│  ✕ GCP 에이전트 설치 중 오류 발생 (3/5 완료)      🔄 │  ← red bg/border
│  ████████████████████████░░░░░░░░░░░░░░  60%        │
│                                                      │
│  GCP 리소스 생성           ✕ 실패                    │  ← red
│  에이전트 연동              ◐ 진행 중                 │  ← orange
...
```
페이지 진입                    →  로딩 → API 호출 → 상태별 렌더링
🔄 새로고침                    →  버튼 spinner → API → 결과 반영
[네트워크 자동 설정]           →  "설정 중..." → API → 성공: 카드 사라짐 / 실패: 에러 표시
[승인 방법 보기]               →  가이드 아코디언 펼침/접힘
전체 완료                      →  onInstallComplete 콜백
```
...
|---------|----------|----------|------|
| 메인 카드 | `warning.bg/border` | `error.bg/border` | `success.bg/border` |
| 프로그레스 바 | `warning.dot` | `error.dot` | `success.dot` |
| 조치 카드 | `warning.bg/border` | — | — |
| 오류 카드 | — | `error.bg/border` | — |
| PSC 가이드 | `info.bg` + `border-blue-200` | — | — |

---

#### 특징

- 리소스 개별 나열 없음 → 리소스 수 증가해도 UI 크기 고정
- 그룹별 카운트 제거 → 헤더 `(n/m 완료)`와 충돌 없음
- "GCP 리소스 생성 / 에이전트 연동" → 내부 용어(TF, BDC) 완전 은닉
- 조치/실패 카드는 필요할 때만 별도 카드로 분리
- 조치 없으면 "자동으로 설치가 진행 중입니다" 안심 문구
