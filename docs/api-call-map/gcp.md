...
>
> 컴포넌트 경로: `page.tsx` → `ProjectDetail` → `GcpProjectPage` → `CloudTargetSourceLayout`
> → (processStatus 분기) `*Step.tsx`. 설치 단계는 `InstallationStatusSlot` → `GcpInstallationStatus`
> → `GcpInstallationInline`. AWS/Azure 와 공유 레이아웃이며 설치만 GCP 고유.
...
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/GCP_TARGET_CONFIRM` | `useGuide` | `WaitingTargetConfirmationStep.tsx:52`; `useGuide.ts:50` |
...
| 버튼클릭 | `Run Infra Scan` / 스캔 재시도 | POST | `/integration/api/v1/target-sources/{id}/scan` | `startScan` | `ScanPanel.tsx:64`; `api/scan.ts:8` |
...
| 모달진입 | 승인 요청 확인 모달(`IdcSubmitModal`) | (없음) | — | `approvalModal.open()` 만 | `CandidateResourceSection.tsx:139` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/GCP_APPROVAL_PENDING` | `useGuide` | `WaitingApprovalStep.tsx:53`; `useGuide.ts:50` |
...
| 모달버튼 | 취소 모달 `요청 취소` | POST | `/integration/api/v1/target-sources/{id}/approval-requests/cancel` | `cancelApprovalRequest` | `WaitingApprovalCancelButton.tsx:21`; `api/index.ts:509` |
| 모달버튼 | 취소 성공 후 프로젝트 갱신 | GET | `/integration/api/v1/target-sources/{id}` | `onSuccess` → `refreshProject` → `getProject` | `WaitingApprovalCancelButton.tsx:30`; `WaitingApprovalStep.tsx:39`; `api/index.ts:257` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/GCP_APPLYING` | `useGuide` | `ApplyingApprovedStep.tsx:34`; `useGuide.ts:50` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/GCP_INSTALLING` | `useGuide` | `CloudInstallingStep.tsx:36`; `useGuide.ts:50` |
...
| 화면진입 | GCP 설치 상태 (1회 fetch, 완료 시 `onComplete`) | GET | `/integration/api/v1/gcp/target-sources/{id}/installation-status` | `useInstallationStatus` → `getGcpInstallationStatus` | `GcpInstallationInline.tsx:44`; `useInstallationStatus.ts:67`; `api/gcp.ts:14` |
| 화면진입 | 설치 완료 감지 시 프로젝트 갱신 | GET | `/integration/api/v1/target-sources/{id}` | `onComplete` → `refreshProject` → `getProject` | `GcpInstallationInline.tsx:50`; `CloudInstallingStep.tsx:39`; `api/index.ts:257` |
| 버튼클릭 | 설치 상태 오류 `재시도` | GET | `/integration/api/v1/gcp/target-sources/{id}/installation-status` | `InstallationErrorView` → `fetchStatus` | `GcpInstallationInline.tsx:55`; `useInstallationStatus.ts:60`; `api/gcp.ts:14` |
| 버튼클릭 | 확정 리소스 오류 `재시도` | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `retryConfirmed` | `GcpInstallationInline.tsx:35`; `ConfirmedIntegrationDataProvider.tsx:59`; `api/index.ts:404` |
| 모달진입 | 완료 파이프라인 카드 클릭 → 상세 모달 | (없음) | — | 이미 로드된 데이터 렌더링 | `GcpInstallationInline.tsx:122` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/GCP_CONNECTION_TEST` | `useGuide` | `WaitingConnectionTestStep.tsx:64`; `useGuide.ts:50` |
| 화면진입 | 확정된 연동 대상 | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `getConfirmedIntegration` | `ConfirmedIntegrationDataProvider.tsx:41`; `api/index.ts:404` |
...
| 버튼클릭 | `설정`(논리 DB)·`완료 승인 요청` → 모달 오픈 | (없음) | — | 모달 오픈만 | `ConnectionTestCard.tsx:294`, `:330` |
...
| 모달버튼 | 논리 DB `저장` 성공 후 프로젝트 갱신 | GET | `/integration/api/v1/target-sources/{id}` | `handleSaved` → `refreshProject` → `getProject` | `ConnectionTestCard.tsx:146`; `WaitingConnectionTestStep.tsx:70`; `api/index.ts:257` |
...
| 모달버튼 | `요청하기` 성공 후 프로젝트 갱신 | GET | `/integration/api/v1/target-sources/{id}` | `handleSubmitApproval` → `refreshProject` → `getProject` | `ConnectionTestCard.tsx:158`; `api/index.ts:257` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/GCP_ADMIN_APPROVAL` | `useGuide` | `ConnectionVerifiedStep.tsx:108`; `useGuide.ts:50` |
| 화면진입 | 확정된 연동 대상 | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `getConfirmedIntegration` | `ConfirmedIntegrationDataProvider.tsx:41`; `api/index.ts:404` |
| 버튼클릭 | 확정 리소스 오류 `다시 시도` | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `ConfirmedResourcesSlot` retry | `ConfirmedResourcesSlot.tsx:17`; `api/index.ts:404` |
| 버튼클릭 | `연결 테스트 재실행` → 확인 모달 오픈 | (없음) | — | `setConfirmKind('retest')` 만 | `ConnectionVerifiedStep.tsx:64` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/GCP_COMPLETED` | `useGuide` | `InstallationCompleteStep.tsx:107`; `useGuide.ts:50` |
| 화면진입 | 확정된 연동 대상 | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `getConfirmedIntegration` | `ConfirmedIntegrationDataProvider.tsx:41`; `api/index.ts:404` |
| 화면진입 | 완료 테이블 논리 DB 요약 | GET | `/integration/api/v1/target-sources/{id}/test-connection/latest-results` | `ConfirmedIntegrationTable(variant=complete)` → `getLatestTestConnectionResultSummaries` | `InstallationCompleteStep.tsx:122`; `ConfirmedIntegrationTable.tsx:58`; `api/index.ts:594` |
| 버튼클릭 | 확정 리소스 오류 `다시 시도` | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `ConfirmedResourcesSlot` retry | `ConfirmedResourcesSlot.tsx:17`; `api/index.ts:404` |
