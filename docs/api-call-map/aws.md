...
>
> 컴포넌트 경로: `page.tsx` → `ProjectDetail` → `AwsProjectPage` → `CloudTargetSourceLayout`
> → (processStatus 분기) `*Step.tsx`. 설치 단계는 `InstallationStatusSlot` → `AwsInstallationStatus`
...
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/AWS_TARGET_CONFIRM` | `useGuide` | `WaitingTargetConfirmationStep.tsx:52`; `GuideCardContainer.tsx:19`; `useGuide.ts:50` |
...
| 버튼클릭 | `Run Infra Scan` / 스캔 실패 `다시 시도` | POST | `/integration/api/v1/target-sources/{id}/scan` | `startScan` | `ScanPanel.tsx:61`; `api/scan.ts:8` |
...
| 모달진입 | 승인 요청 확인 모달(`IdcSubmitModal`) | (없음) | — | `approvalModal.open()` 만 | `CandidateResourceSection.tsx:139`; `IdcSubmitModal.tsx:61` |
...
> (`CandidateResourceSection.tsx:142-143`). 위 `resources`/`target-sources/{id}` 호출과 동일.
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/AWS_APPROVAL_PENDING` | `useGuide` | `WaitingApprovalStep.tsx:53`; `useGuide.ts:50` |
...
| 모달버튼 | 취소 모달 `요청 취소` | POST | `/integration/api/v1/target-sources/{id}/approval-requests/cancel` | `cancelApprovalRequest` | `WaitingApprovalCancelButton.tsx:29`; `api/index.ts:509` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/AWS_APPLYING` | `useGuide` | `ApplyingApprovedStep.tsx:45`; `useGuide.ts:50` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 (AUTO/MANUAL) | GET | `/integration/api/v1/admin/guides/{AWS_AUTO_INSTALLING\|AWS_MANUAL_INSTALLING}` | `useGuide` | `CloudInstallingStep.tsx:52`; `useGuide.ts:50` |
...
| 화면진입 | AWS 설치 상태 (1회 fetch, 완료 시 `onComplete`) | GET | `/integration/api/v1/aws/target-sources/{id}/installation-status` | `useInstallationStatus` → `getAwsInstallationStatus` | `AwsInstallationInline.tsx:67`; `useInstallationStatus.ts:66`; `api/aws.ts:14` |
| 화면진입 | 설치 완료 감지 시 프로젝트 갱신 | GET | `/integration/api/v1/target-sources/{id}` | `onComplete` → `refreshProject` → `getProject` | `AwsInstallationInline.tsx:72`; `CloudInstallingStep.tsx:38`; `api/index.ts:257` |
| 버튼클릭 | 수동 설치 `Script 다운로드` (MANUAL only) | GET | `/integration/api/v1/aws/target-sources/{id}/terraform-script` | `getAwsTerraformScript` | `AwsInstallationInline.tsx:87`; `api/aws.ts:27` |
| 버튼클릭 | 설치 상태 오류 `다시 시도` | GET | `/integration/api/v1/aws/target-sources/{id}/installation-status` | `InstallationErrorView` → `fetchStatus` | `AwsInstallationInline.tsx:109`; `useInstallationStatus.ts:60`; `api/aws.ts:14` |
| 버튼클릭 | 확정 리소스 오류 `재시도` | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `retryConfirmed` | `AwsInstallationInline.tsx:163`; `ConfirmedIntegrationDataProvider.tsx:59`; `api/index.ts:404` |
| 모달진입 | 수동 설치 `가이드 보기`(`TfScriptGuideModal`) | (없음) | — | `setGuideOpen(true)` 만, 정적 콘텐츠 | `AwsInstallationInline.tsx:174`; `TfScriptGuideModal.tsx:19` |
| 모달버튼 | 가이드 모달 버튼 | (없음) | — | close/copy 만 | `TfScriptGuideModal.tsx:62` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/AWS_CONNECTION_TEST` | `useGuide` | `WaitingConnectionTestStep.tsx:83`; `useGuide.ts:50` |
| 화면진입 | 확정된 연동 대상 | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `getConfirmedIntegration` | `ConfirmedIntegrationDataProvider.tsx:41`; `api/index.ts:404` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/AWS_ADMIN_APPROVAL` | `useGuide` | `ConnectionVerifiedStep.tsx:108`; `useGuide.ts:50` |
| 화면진입 | 확정된 연동 대상 | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `getConfirmedIntegration` | `ConfirmedIntegrationDataProvider.tsx:41`; `api/index.ts:404` |
...
|---|---|---|---|---|---|
| 화면진입 | 페이지 SSR 로드 | GET | `/install/v1/target-sources/{id}` (SSR·상위 BFF 직접) | `bff.targetSources.get` | `page.tsx:17`; `lib/bff/http.ts:88` |
| 화면진입 | 가이드 카드 | GET | `/integration/api/v1/admin/guides/AWS_COMPLETED` | `useGuide` | `InstallationCompleteStep.tsx:107`; `useGuide.ts:50` |
| 화면진입 | 확정된 연동 대상 | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `getConfirmedIntegration` | `ConfirmedIntegrationDataProvider.tsx:41`; `api/index.ts:404` |
...
| 버튼클릭 | 확정 리소스 오류 `다시 시도` | GET | `/integration/api/v1/target-sources/{id}/confirmed-integration` | `ConfirmedResourcesSlot` retry | `ConfirmedResourcesSlot.tsx:17`; `api/index.ts:404` |
| 버튼클릭 | `인프라 변경` / `연결 테스트 재실행` → 확인 모달 오픈 | (없음) | — | `setConfirmKind(...)` 만 | `InstallationCompleteStep.tsx:63`, `:71` |
| 모달진입 | `ConfirmRewindModal` | (없음) | — | — | `ConfirmRewindModal.tsx:51` |
