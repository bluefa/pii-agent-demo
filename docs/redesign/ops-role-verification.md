...
목이 늘 `VALID` 만 돌려주면 판정 넷 중 하나만 볼 수 있어 화면을 검증할 수 없다. 그렇다고 특정
대상을 상시 실패로 박으면 그 대상을 쓰는 다른 데모가 같이 깨진다. 그래서 **저장한 Role 이름**이
시나리오를 고른다 (`lib/bff/mock/aws.ts`). 운영 화면에서 Role [수정] → 이름에 키워드 포함 → 저장.
