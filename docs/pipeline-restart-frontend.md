...
**공통 어휘 — `RestartBadge`(`r24Task.tsx`)**: 실행에 제목이 붙는 모든 자리에 같은 문구
`↻ #123에서 재시작` 을 쓴다. 아이콘+번호(`↻ #123`)만으로는 장식으로 읽혀 "재시작"이라는
단어가 화면에서 사라진다는 피드백에 따른 것이다. 조사는 `에서` 로 고정한다 — `(으)로부터` 는
끝자리 숫자 읽기에 따라 갈려(…4 → 로부터, …6 → 으로부터) 임의의 id 에 하드코딩할 수 없다.
표시 위치: 현재 작업 카드(`CurrentPipelineCard`)·실패 카드(`LastRunFailedCard`)의 제목 행,
작업 상세 헤더의 `#id` 옆, 이력 테이블 행.

- **이력 테이블**(`TargetDetailView`): `origin_pipeline_id` 가 있으면 작업 컬럼에 배지 —
  클릭 시 원본 상세로(행 클릭과 분리). 유형 컬럼은 INSTALL 그대로(결정 1의 화면 가치).
