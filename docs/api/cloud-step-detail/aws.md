...
- **Identity fields (AWS-specific).** `AwsProjectPage` builds `ProjectIdentity` with
  ...
  **`AWS Account ID`** (`project.awsAccountId`, mono) and **`Region Type`**
  ...
  project-number (GCP). `jiraLink` is null.
- **Transport.** All client functions call `fetchInfra*Json('/...')`; the transport prepends
  `INTERNAL_INFRA_API_PREFIX = '/integration/api/v1'`. Paths below are the **internal** browser paths.
...
| `Run Infra Scan` | `POST /target-sources/{id}/scan` | `startScan` (v1 API; `ScanController`), 완료 후 `getConfirmResources` 재조회 |
...
- **`startScan`** (Run Infra Scan): `app/lib/api/scan.ts#startScan` → route
  ...
  `ScanController` (`features/scan/ScanPanel`) owns the trigger/poll and re-runs `getConfirmResources` on completion.
...
**Transition (advance):** 테스트 성공이 서버에 반영되면 `onResourceUpdate`=`refreshProject`(`getProject`)
경로로 step 6 전이. (cloud 플로우만 자동 폴링; AWS도 cloud 공용 경로 그대로.)
...
> 페이지 자체는 SSR `bff.targetSources.get(id)` (`GET /target-sources/{id}`, client `getProject`)로 진입하며
...
### 4. API Client
- `getConfirmedIntegration(id, {signal})` → `GET /target-sources/{id}/confirmed-integration`
- (SSR/진입) `getProject(id)` → `GET /target-sources/{id}`
...
- **`getConfirmedIntegration`**: step 4/6 체인과 동일 — `app/lib/api/index.ts#getConfirmedIntegration`
  ...
  `lib/bff/mock/confirm.ts#getConfirmedIntegration` → `lib/mock-data.ts`.
- **`getProject`** (진입): `app/lib/api/index.ts#getProject` → route `.../[targetSourceId]/route.ts (GET)`
  → `bff.targetSources.get` → `lib/bff/mock/target-sources.ts#get` → `lib/mock-data.ts`.
...
2. **Identity fields.** AWS → `AWS Account ID` + `Region Type` (`awsAccountId`/`awsRegionType`); no
   tenant/subscription (Azure) or project-number (GCP). `monitoringMethod: 'AWS Agent'`.
