...
| `Run Infra Scan` | `POST /integration/api/v1/target-sources/{id}/scan` | `startScan` (v1 API). 완료 시 `handleScanComplete` → 후보 `getConfirmResources` 재조회 **+ `refreshProject` (`getProject`)** |
...
- `CandidateResourceSection` owns the fetch (`useEffect` + `AbortController` → `getConfirmResources`), maps via
  `catalogToCandidates`, and on scan complete (`handleScanComplete`) re-runs the candidate fetch **and** `refreshProject` (`getProject`). It renders `ScanController` (`features/scan/ScanPanel`) wrapping
...
- **getConfirmResources** → `app/lib/api/index.ts:getConfirmResources` → `app/integration/api/v1/target-sources/[targetSourceId]/resources/route.ts (GET)` → `bff.confirm.getResources` → mock `lib/bff/mock-adapter.ts:mockBff.confirm.getResources` → `lib/bff/mock/confirm.ts:mockConfirm.getResources` (`lib/mock-data.ts:getProjectByTargetSourceId` for the project, then each resource via the local `lib/bff/mock/confirm.ts:toResourceCatalogItem`). Real: `lib/bff/http.ts:httpBff.confirm.getResources` → `GET {BFF_URL}/install/v1/target-sources/{id}/resources` → `extractResourceCatalog`.
...
- `AzureInstallationInline` (the Azure-specific install component) uses the **shared** hook
  ...
  (3 cards from `AZURE_PHASE_CARDS`: `서비스 측 리소스 설치 진행` / `BDC 측 리소스 설치 진행` / `Private Link 모듈 설치 진행`) + 
...
최신 결과를 폴링한다. 폴링이 `SUCCESS`를 관측하면 `ConnectionTestPanel`이 `onResourceUpdate`(= `refreshProject` → `getProject`)를 호출하고, mock은 이미 `processStatus`를 `CONNECTION_VERIFIED`로 전환해 두므로 재조회로 6단계 화면으로 전이된다.
...
- `ConnectionTestPanel` owns polling via `app/hooks/useTestConnectionPolling.ts` (`interval = 4_000`;
  `triggerTestConnection` + `getTestConnectionLatest`, stops when `status !== 'PENDING'`). On the
  `PENDING → SUCCESS` transition it calls `onResourceUpdate` (= `refreshProject` → `getProject`) to advance to step 6. Lazy-loads
...
### 4. API Client
- `getTestConnectionLatest(id)` → `GET /target-sources/{id}/test-connection/latest` (`fetchInfraJson`, raw — the client fn does **not** catch; the 404→`null` mapping is in the hook `app/hooks/useTestConnectionPolling.ts:fetchLatestTest` try/catch).
...
|---|---|---|
| mount | `GET /integration/api/v1/target-sources/{id}/confirmed-integration` | `getConfirmedIntegration` (raw) |
| 페이지 초기 로드(SSR) | (HTTP 없음) `page.tsx` 서버 컴포넌트가 `bff.targetSources.get(id)` **직접 호출** + `extractTargetSource` | route.ts/`getProject` 미경유, `processStatus:7` |
...
### 4. API Client
- `getConfirmedIntegration(id, {signal})` → `GET /target-sources/{id}/confirmed-integration`.
- Initial page load is **not** a client DAL call: `page.tsx` (Server Component) imports `bff` from `@/lib/bff/client` and calls `bff.targetSources.get(id)` then `extractTargetSource` directly (no `getProject`/route.ts HTTP hop). The CSR `getProject` → route.ts → `bff.targetSources.get` chain is only the `refreshProject` path.

### 5. Adapter 계층 (full chain)
- **getConfirmedIntegration** — identical chain to Step 4 §5.
- **initial SSR load** → `app/integration/target-sources/[targetSourceId]/page.tsx` → `bff.targetSources.get(id)` (`@/lib/bff/client`) → mock `mockBff.targetSources.get` → `lib/bff/mock/target-sources.ts:mockTargetSources.get` (real → `GET /install/v1/target-sources/{id}`) → `lib/target-source-response.ts:extractTargetSource`. No `/integration/api/v1` request. The CSR `getProject` → `.../[targetSourceId]/route.ts (GET)` → `bff.targetSources.get` chain is the `refreshProject` refetch path only.
