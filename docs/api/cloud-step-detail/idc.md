...
- **Routing.** Two independent switches. **(1) Provider page:** `ProjectDetail.tsx:renderProvider()`
  does `switch (project.cloudProvider) { case 'IDC': … }` → `IdcProjectPage.tsx` (keyed by
  `targetSourceId`). This is selected by `cloudProvider`, **not** `serviceCode` or `processStatus`.
  **(2) Step:** inside `IdcProjectPage.tsx` → `IdcTargetSourceLayout.tsx` → `renderStep()` switches on
  `project.processStatus` → `steps/IdcStep<N>*.tsx`. Each step component receives the shared `IdcStepProps`
...
`Status` (health) column (integration targets only). The page is first loaded SSR: `page.tsx` (Server Component)
calls `bff.targetSources.get(id)` + `extractTargetSource` **directly** (no `getProject`/route.ts HTTP hop;
carries `processStatus: 7` + project health).
...
- `getIdcResources(id)` (`@/app/lib/api/idc`) → `GET /idc/target-sources/{id}/resources`.
- (SSR 진입) client DAL 미사용 — `page.tsx`가 `bff.targetSources.get(id)` (`@/lib/bff/client`) + `extractTargetSource` 직접 호출 (CSR `getProject`/route.ts 아님; 그 chain은 `refreshProject` 전용).
