...
### 5.1 리소스 목록 (ResourceTable) — TC 상태 + [관리] 버튼 추가

기존 `ResourceRow`에 이미 `showConnectionStatus` / `ConnectionIndicator`가 있으나 현재 `false`로 비활성.
이번에 **연결 상태 컬럼 활성화** + **[관리] 버튼 컬럼 추가**.
...
│──────────────────────────────────────────────────────────────────────────────│
│                                                                              │
│  타입   리소스 ID          DB 유형       Credential    연결 상태     액션    │
│  ─────────────────────────────────────────────────────────────────────────── │
│  RDS    rds-prod-01       MySQL         cred-01       ● 성공       [관리]   │
│  RDS    rds-prod-02       PostgreSQL    cred-02       ● 성공       [관리]   │
│  RDS    rds-staging       MySQL         cred-01       ● 실패       [관리]   │
...
#### 구현 포인트
- `ResourceTable`: `showConnectionStatus`를 TC 단계에서 `true`로 변경
- `ResourceRow`: 마지막 컬럼에 **[관리]** 버튼 추가 → 클릭 시 논리 DB 관리 모달 오픈
- 연결 상태는 기존 `ConnectionIndicator` 재사용 (마지막 TC의 리소스별 결과 매핑)
...
### 5.2 [관리] 클릭 → 논리 DB 관리 모달

리소스별 [관리] 버튼 클릭 시 모달이 열립니다.
모달은 **2개 API를 동시 호출**하여 데이터를 구성합니다:
- `GET tested-logical-databases?resourceId=xxx` → TC 결과 논리 DB 전체 목록
- `GET excluded-logical-databases?resourceId=xxx` → 현재 제외된 목록

```
┌──────────────────────────────────────────────────────────────────────┐
│  논리 DB 관리 — rds-prod-01 (MySQL)                             [X] │
│──────────────────────────────────────────────────────────────────────│
│                                                                      │
│  ┌─ 논리 DB 목록 (TC 결과 기준) ───────────────────────────────────┐ │
│  │                                                                 │ │
...
│  │  db_reporting         연동 대상                                 │ │
│  │  db_user_data         연동 대상                                 │ │
│  │  db_legacy_01         ✕ 제외됨                     [제외 해제]  │ │
│  │  db_temp_migration    ✕ 제외됨                     [제외 해제]  │ │
│  │                                                                 │ │
│  └─────────────────────────────────────────────────────────────────┘ │
│                                                                      │
│  총 6개 논리 DB · 제외 2건                                            │
...
| 동작 | 설명 |
|------|------|
| **제외 추가** | 연동 대상 DB 선택 → [제외 추가] → 해당 DB가 "✕ 제외됨"으로 이동 |
| **제외 해제** | "✕ 제외됨" 행의 [제외 해제] → 해당 DB가 "연동 대상"으로 복귀 |
| **[저장]** | `PUT excluded-logical-databases` 호출 (전체 교체) → 부수 효과로 TC 이력 초기화 |
| **[취소]** | 변경 사항 버리고 모달 닫기 |

#### TC 미수행 리소스의 [관리] 클릭 시

`tested-logical-databases` API가 404를 반환하므로, 논리 DB 목록 없이 제외 목록만 표시합니다.

```
┌──────────────────────────────────────────────────────────────────────┐
│  논리 DB 관리 — ec2-worker (Oracle)                             [X] │
│──────────────────────────────────────────────────────────────────────│
│                                                                      │
│  ℹ 연결 테스트가 아직 수행되지 않았습니다.                              │
│    테스트 완료 후 논리 DB 목록이 표시됩니다.                             │
│                                                                      │
│  현재 제외 목록: 없음                                                 │
│                                                                      │
│                                                    [닫기]            │
...
### 5.3 연결 테스트 패널 — TC 전체 성공 시 [다음] 버튼 활성화
...
- 마지막 TC 결과가 **전체 리소스 SUCCESS**일 때만 [다음 →] 활성
- 1개라도 FAIL이거나 TC 미수행이면 비활성(disabled)

### 5.4 [다음] 클릭 → 논리 DB 확인 모달 (읽기 전용)

[다음] 버튼 클릭 시, 각 리소스별 논리 DB 수 + 제외 목록을 **읽기 전용**으로 보여줍니다.
...
| 버튼 | 동작 |
|------|------|
| **[확인]** | `PUT test-connection-confirmation { confirmed: true }` → TC 완료 처리 → 다음 단계 이동 |
| **[취소]** | 모달 닫기, 현재 상태 유지 |

### 5.5 전체 흐름 시나리오

```
1. 리소스 목록에서 각 리소스의 연결 상태 확인
       ↓
2. 필요 시 [관리] 클릭 → 논리 DB 관리 모달
       - 논리 DB 목록 조회 (tested-logical-databases)
       - 제외 목록 조회 (excluded-logical-databases)
       - 제외 추가 / 제외 해제 → [저장]
       - ⚠ 저장 시 TC 이력 초기화됨
       ↓
3. [연결 테스트 수행] → TC 실행
...
4. TC 완료
       - 전체 SUCCESS → [다음 →] 활성화
       - 일부 FAIL → [다음 →] 비활성, 실패 리소스 확인 후 재시도
       ↓
5. [다음 →] 클릭 → 논리 DB 확인 모달 (읽기 전용)
       - 리소스별 논리 DB 수 + 제외 목록 표시
       ↓
6. "확인되셨습니까?" → [확인]
...
```
[관리] 모달에서 제외 변경 → TC 이력 초기화 → [연결 테스트 수행] → TC 성공 → [다음] → 확인 → 완료
         ↑                                                                          ↓
         └──────────── 제외 변경이 필요하면 돌아감 ←─── [취소]로 되돌아옴 ───────────┘
```
