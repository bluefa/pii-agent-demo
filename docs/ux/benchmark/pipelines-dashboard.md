...
4. **카드 마크는 그릇 없이 글리프만** — 운영 알림 카드가 마크를 쓰는 방식 그대로
   ...
   실제로 눌리는 건 카드 전체다.
5. **"작업 목록 / 최근 7일" 헤더 줄 삭제** — 카드 안에 목록 말고 들어 있는 게 없어서 제목이
   가리킬 대상이 자기 자신뿐이었고, 기간은 이미 상단 세그먼트가 켜져 있다(P10 의 반복 중 하나).
6. **페이저는 가운데, 건수와 윗줄 구분선 삭제** — 마지막 행의 아래 테두리가 이미 그 경계를
   그리고 있었다. 현재 페이지 숫자만 브랜드색(14px). **다만 조회 창 고지는 남긴다** — 건수는
   취향이지만 "200건까지만 불러왔다"는 사실은 없으면 잘린 목록을 전체로 읽게 된다(P3).
7. **타일 4장이 폭을 꽉 채운다** — `minmax(0,260px)` 상한을 풀어 `grid-cols-4` 로. 1600px 에서
   타일이 왼쪽에 몰리고 오른쪽 400px 가 비어 있었다.
8. **식별자는 링크색** — Target Source 번호와 Provider 는 이 행을 여는 식별자라 파란색
   (`--pl-info-text`). 실측 대비 흰 배경 5.99:1 · hover(gray-100) 5.43:1. 원색 `--pl-info` 는
   3:1 대라 12px 텍스트로 쓸 수 없다.
9. **행 hover 를 gray-50 → gray-100** — 카드가 흰색이라 gray-50(#F9FAFB)은 1.03:1, DOM 에는
   있는데 화면에는 없는 hover 였다.
