...
- **시안이 그린 상태색은 이 앱의 색이 아니다.** 시안 C 의 진행 세그먼트는
  ...
  *"색상이 너무 알록달록하다 — 상태는 모두 단일 색상으로"*. 상태는 5색에서 하나의 중립 램프
  ...
  판정을 모르고 그린 것이다. **이후 오너가 이 화면에 한해 판정을 뒤집었다 — §4 참고.**
...
**진행도는 단계 세그먼트로.** 연속 막대는 "4 중 1"까지만 말하고 **어느 단계에서 멈췄는지**는
말하지 못하는데, 그게 목록에서 알고 싶은 유일한 것이다(`PipelineStepStrip`). 캡션이 그 단계를
부른다 — `2단계에서 실패 · 1/4`. 8단계를 넘으면 조각이 사이 간격(2px)보다 얇아져 읽히지 않으므로
원래의 요약 막대로 되돌아간다(테스트로 고정).

**이 화면은 모노크롬 램프의 예외다 — 오너 판정.** *"완료된건 초록색, 현재 진행중인건 파란색"* ·
*"상태는 그냥 태그없이 텍스트로만 표현하고 텍스트 색상만 남겨"*. 위 §진단 정정에 적었듯 섹션
전체는 모노크롬으로 한 번 정리된 상태이고, 이 화면만 되돌린다. 근거는 두 가지다:

- **한 행이 상태 하나만 들면 무게로 갈리지만, 세그먼트는 아니다.** 110px 안에 끝남·진행 중·미착수
  세 상태가 나란히 붙는데, 12px 블록 셋을 명도만으로 구분할 수는 없다. pill 에서는 하지 않던
  일을 여기서는 색이 한다.
- **칩을 벗기면 색 말고 남는 게 없다.** pill 의 배경·테두리·아이콘은 복잡한 행 안에서 낱말 하나를
  읽히게 하려고 있던 것인데, 이 목록에서는 낱말이 자기 열에 혼자 서 있다.

세그먼트: 끝남 `--pl-ok` · 진행 중 `--pl-info` · 실패 `--pl-err` · 미착수 `--pl-gray-200`.
취소된 실행의 끝난 칸은 **초록이 아니라 회색**이다 — 돌긴 했지만 성공하지 않았고, 초록은 성공했다고
말한다. 상태 텍스트는 같은 색을 쓰되 `-text` 램프로 간다: 원색은 채움용으로 섞여 있어 흰 배경
13px 텍스트로는 4.5:1 아래로 떨어진다.

**아직 모노크롬으로 남은 곳** — 세그먼트가 8단계를 넘어 요약 막대로 폴백할 때, 그리고 상세 화면의
`StatusPill`·`PipelineProgressBar` 전부. 섹션 전체로 확장할지는 별도 결정이다.
