...
⚠️ 맨 오른쪽 칸은 **필드가 어디에 선언돼 있는지**이지, 가운데 칸의 근거가 아니다.
초기화가 무엇을 지우는지는 계약에 없다 — `resetTargetSource` 설명은 "초기(IDLE) 상태로
강제 초기화합니다" 한 줄이고 어떤 필드도 거명하지 않는다. **가운데 칸 두 줄은 모두 추론이고,
아래 줄의 불변성은 도메인 오너 확인이 근거다.**
...
- 계약이 같은 값을 응답마다 다른 철자로 싣는다(목록 camel · 상세 snake). `supportRawData` 와
  같은 자리의 함정이다. 홉이 어긋나면 **에러도 빈 목록도 없이 도장이 한 번도 안 찍히므로**
  두 홉과 두 렌더 지점을 각각 못 박아 뒀다: 목록 홉은 `ops-service-support-raw-data-route.test.ts`,
  상세 홉은 `OpsTargetView.stamp.test.tsx`(철자를 camel 로 바꾸면 깨지는 것까지 확인),
  렌더 지점은 `ServiceDetailView.test.tsx` · 같은 `OpsTargetView.stamp.test.tsx`.
- 목: 시드가 `piiAgentFirstInstalledAt` 을 주지 않으면 키를 **아예 안 싣는다**. null 로 채우면
  목이 "한 번도 마친 적 없다"를 단정하게 된다.
- 목이 값을 **만드는** 경로도 있어야 한다. 설치 확정(`confirmInstallation`, `projects.ts` 의 쌍)이
  7단계에 처음 도달할 때만 찍고, 이미 값이 있으면 건드리지 않는다 — 이 조건이 빠지면 재확정마다
  날짜가 움직여 "최초 1회" 라는 문구가 거짓이 된다. 초기화 → 재확정 왕복이 테스트에 있다.
...
- 목록 카드 2층의 가로 여유가 694 → 520px 로 줄어 Azure 의 36자 GUID 두 개가 더 짧게 잘린다.
  전문은 `title` 로 남긴다 — 이 카드가 이미 정한 규칙인데 두 GUID 만 빠져 있었다(교차 리뷰가
  잡았다). 꼬리만 다른 두 구독이 목록에서 같은 값으로 보이던 상태라, 지금은 테스트로 못 박았다.
