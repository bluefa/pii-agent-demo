...
## 확장 (2026-08-22, 오너 피드백): 안내 줄 14px

성공 정착 카드에서 시각 메타와 안내 줄이 **같은 계층으로 읽힌다**는 지적.

```
2026-06-01 09:04 완료 (82일 전) · 소요 4분 20초      ← counts, 12px #6B7684
관리자에게 승인을 요청하면 다음 단계로 넘어가요. …      ← guidance, 12px #4E5968
```

색만 한 칸 진하게 둔 것(#6B7684 → #4E5968)으로는 갈라지지 않는다. 크기가 같으면 눈은 두
줄을 같은 종족으로 묶고, 색 차이는 그 안에서의 강조로 읽힌다. `connProgress.guidance` 를
**14px** 로 올려 16(제목) / 14(안내) / 12(메타) 로 한 칸씩 내려가게 했다 — 무엇을 하면
되는지가 언제 끝났는지보다 위에 선다.

색은 그대로 #4E5968 이다: success 판(#EFF9F3)에서 6.67:1 이고, 14px 는 아직 large text 가
아니라 AA 기준은 4.5:1 그대로다.
