...
- 3층: 재실행 배지·취소 요청 · **"AWS 설치" 결합 태그**(무채, DELETE만 err 톤) + ⓘ 툴팁(레시피명+설명문, hover+키보드 포커스) · `작업 등록` 시각 · 재시작됨 링크 — 공리 2·3
...
- **run # 완전 제거(2026-08-09 오너 지시)**: 시안 E는 `run #131`을 3층에 남겼으나
  오너가 `"Infra 작업 현황 run #129" 이건 삭제해줘`로 잘라냈다. 화면에 run 번호는
  없고 URL·브라우저 탭 제목(`작업 #129 · 서비스명`)이 정체성을 담당한다.
  h1은 `Infra 작업 현황`(text.pageTitle)으로 승격, CTA는 그 행 우측.
- **1층 순서·톤(같은 날 오너)**: `GCP #1002`(provider 평문 14/medium → id 12px primary),
  서비스 이름·코드 weight medium, 코드는 SDU 칩과 같은 파랑 태그.
- **시각 라벨**: `생성` → `작업 등록`(= `created_at`, 실행 시작 아님).
  PENDING의 `시작 예정` 시각은 밴드 16/bold(태스크 이름 자리)에서 14/normal로 강등.
