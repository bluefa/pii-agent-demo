...
6. **섹션 제목은 카드 안, 높이는 항상 동일 (오너 지시).** `현재 작업`·`작업 이력` 이 각 카드의
   첫 줄이고 그 아래 rule — 카드 밖에 두면 바로 아래 16px 작업 이름과 두 제목이 경쟁한다.
   행은 stretch, 카드는 `h-full flex-col`, 이력 본문은 `flex-1` 이라 어느 쪽이 크든 두 카드가
   같은 줄에서 끝난다(`detailStyles.sectionCard`).
