# Step4 · Terraform Script 다운로드 — 벤치마킹 → 레일 '설치 스크립트' 항목 채택 기록
...
- **`참고` → `리뷰요청` → `설치 스크립트`** — 색은 `statusColors.warning.textDark`
  (orange-800). 파랑(내가 할 일)과 겹치지 않는 유일한 강조색이다.
  vivid orange-500 을 글자에 쓰지 않은 이유는 레일 표면(gray-100)에서 2.8:1 로 AA 미달이기 때문.
  `리뷰요청` 은 오너가 되물어 폐기했다 — 이 제품에서 "요청"은 워크플로 명사라(`연동 요청` 77곳,
  `승인 요청` 63곳, `요청자` 18곳) 아무도 요청하지 않은 자료에 붙으면 응답 의무로 읽힌다.
  후보 중 **두 모드를 다 견디는 것**만 남겼다: 자동에선 미리 보는 자료지만 수동에선 실제로
  받아서 실행하는 산출물이라, "미리 보기/미리 확인" 계열은 수동에서 거짓이 된다.
...
1차 시도는 수동일 때 `service` 단계의 `action` 슬롯에 다운로드 버튼을 그대로 얹는 것이었다.
**오너가 화면에서 기각했다** — h40 outline 버튼이 12px 태그 두 개 사이에 끼어 혼자 커 보였다.
액션 슬롯은 그 정도 무게를 감당하는 자리가 아니다(IDC "방화벽 확인"도 같은 문제를 안고 있다).

**최종: 두 모드 모두 참고 항목을 가리키는 텍스트 링크 한 줄.** 무게는 거기까지다.

- 단계 설명 아래 `note` 한 줄 — 수동은 "Terraform Script**에서 스크립트를 내려받을 수 있습니다**",
  자동은 "…**에서 자세한 설치 사항을 확인할 수 있습니다**". 링크는 같고 목적어만 다르다.
- 단계 헤더에는 다운로드 버튼이 없다. 다운로드는 참고 항목 히어로 한 곳에만 있다.
- 참고 항목은 두 모드 모두 그대로 남는다 — "언제든 확인 가능"은 오너 요구였다.

**스펙과의 거리** — `docs/cloud-provider-states.md:90` 의 `[TF Script 다운로드]` 액션은
단계 안에 **버튼으로** 있지 않고 한 클릭 뒤에 있다. 원래 문제(별도 탭 뒤에 숨어 존재를 모름)는
명시적 안내 문구로 해소되지만, 문자 그대로의 스펙 충족은 아니다. 오너가 무게를 우선한 결정.
