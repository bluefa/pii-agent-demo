...
| 5 웨이파인딩·시간 | h1 옆 서비스명, document.title, 밴드 경과/소요 + 시작→종료 툴팁. ~~PlBreadcrumb 복원~~ — 구현 후 오너 피드백으로 **제거**(2026-08-09, "대시보드›1099›작업 #130 없애자"): P6의 탭/히스토리 변별은 document.title·부제가 담당, 목록 복귀는 사이드바로 충분 |
...
- **statusKo 잔여 표면** — 한글 상태 어휘는 이 상세 화면(밴드 pill + PipelineStatusBadge)까지만.
  대시보드·서비스 목록·이력의 `StatusPill`(enum 원문)과 target-detail의 `STATUS_VIEW`(R24 Figma
  영문 라벨, 오너 확정)는 별도 결정 없이는 건드리지 않는다 — 화면 간 어휘가 아직 섞여 있음을 인지한 상태의 유보
- `pipelineCrumbs` 헬퍼 — 브레드크럼 제거로 다시 고아(선존 dead code, 이 PR 최종 diff는 해당 파일 무변경). 삭제는 별도 정리 건
