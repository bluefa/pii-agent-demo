...
`layout.tsx`의 `isOpsTarget` 분기에서 `main` 배경을 `--pl-gray-300`(#D0D5DD)으로. 한
줄이고, masthead 의 `-mt-6 -mx-8` 이 이미 main 의 패딩 박스를 덮으므로 헤더는 알아서 흰
면을 유지한다.

- masthead ↔ 바닥 1.045 → **1.474:1**
- **본문 카드 ↔ 바닥도 같이 1.474** — P3가 헤더뿐 아니라 본문에서도 풀린다
- 크롬이 가장 밝은 면으로 남으므로 칩들이 제 `gray-100` 배경을 유지하고, 고도도 맞다

**왜 gray-300 에서 멈추나**: 중립 램프의 마지막 잔잔한 값이다. 한 칸 더 내리면
`#98A2B3`(2.58:1)인데, 페이지가 슬레이트 캔버스가 되어 다른 제품처럼 읽히면서도 **3:1 은
여전히 못 넘는다** — 시각적 비용을 치르고 기준은 못 얻는다. 두 안 모두 실제로 렌더해서
비교했다.

첫 시도는 `gray-100`(1.102)이었으나 오너가 "너무 구분이 안 된다"고 해 한 칸 더 내렸다.

검증: 6개 탭 전수 검사에서 바닥색(`#D0D5DD`)으로 칠해진 채 뒤도 바닥인 요소 **0건**,
...
### 3:1 은 면이 아니라 선에 준다 (오너 후속: "3 이상은 되어야 하지 않나")

**두 면 사이 3:1 은 밝은 테마에서 도달할 수 없다.** 흰색 대비 3:1 이 되는 회색은
`#959595` 하나뿐이고(계산: L = 1.05/3 − 0.05 = 0.30), 바닥을 거기까지 내리면 색조 조정이
아니라 중간 회색 관리자 테마가 된다 — 실제로 렌더해서 확인했다. 흰 카드를 쓰는 이상
이건 취향이 아니라 천장이다.

그리고 **WCAG 1.4.11 이 실제로 요구하는 대상이 면이 아니다.** 이 기준은 UI 컴포넌트를
식별하는 데 필요한 시각 정보와 내용 이해에 필요한 그래픽에 3:1 을 건다. 페이지 크롬과
본문 사이의 장식적 층 구분은 대상이 아니다. **경계선은 대상이고, 거기서는 3:1 이 쉽다.**

`tabStrip` 의 `border-b` 를 `--pl-border`(#E4E7EC, 흰색 대비 1.24) → `--pl-gray-500`
(#667085)로. 실측 **선↔헤더 4.97:1 · 선↔바닥 3.37:1** — 양쪽 다 3 을 넘는다.

⚠️ 이 두 값은 바닥색과 묶여 있다. 바닥을 gray-100 → gray-300 으로 내리면서 선↔바닥이
4.51 → 3.37 로 같이 내려왔다. **바닥을 한 칸 더 내리면(gray-400, #98A2B3) 선↔바닥이
1.93 이 되어 3 아래로 떨어진다** — 바닥과 선은 따로 못 고른다. 하나를 바꾸면 다른 하나를
다시 재야 한다.

`--pl-border-strong`(#D0D5DD)을 선으로 쓰지 않는 이유는 이제 두 개다: 흰 면 대비
1.47:1 이라 애초에 경계가 아니었고, 지금은 그 값이 **바닥 자체**라 선이 바닥에 묻힌다.
램프 토큰을 border 로 쓰는 선례는 `OpsProcessRail` 의 `border-[var(--pl-gray-300)]`.

정리 — **면은 1.474(밝은 테마에서 잔잔함을 지키는 한계), 선은 4.97 / 3.37(기준 통과).**
구분을 지는 것은 선이다.
