...
아니라 중간 회색 관리자 테마가 된다 — 실제로 렌더해서 확인했다. 램프 안의 어떤 값도
`#F2F4F7`(1.102) 위로 의미 있게 못 올라간다.
...
(#667085)로. 실측 **선↔헤더 4.97:1 · 선↔바닥 4.51:1** — 양쪽 다 3 을 넉넉히 넘는다.

`--pl-border-strong`(#D0D5DD)을 안 쓴 이유: 1.47:1 이라 흰 면 두 개 사이의 헤어라인이지
두 영역 사이의 경계가 아니다. 램프 토큰을 border 로 쓰는 것은 선례가 있다
(`OpsProcessRail` 의 `border-[var(--pl-gray-300)]`). `design-guard` 69 통과.

정리 — **면은 1.102(밝은 테마의 천장), 선은 4.97(3:1 기준 통과).** 구분을 지는 것은 선이다.
