...
| 인스턴스 = 아코디언 밴드 | 인스턴스 행 제거 → `RdsInstancePanel`(클러스터 행 **바로 아래** colspan 밴드). 카드 3줄 = `◉ 이름 [역할]` / `AZ · 엔진` / `host:port`를 **3열 그리드로 나란히** — 인스턴스는 서로 비교해서 고르므로 세 카드의 역할·AZ·엔드포인트가 한 눈높이에 선다. 엔드포인트·AZ는 표에 열이 없어 이번에 처음 보인다 |
...
**셰브런 + 인라인 밴드**가 둘을 한 컨트롤로 해결한다. 셰브런은 이 앱에 이미 있는 "여기 더 있다" 신호이고(`toggleClosed`가 primary hue를 쓰는 이유가 정확히 이것), 밴드는 자기 행 말고 다른 곳에 뜰 수가 없다. Athena 그룹과 같은 문법이라 표 안에 접힘 어휘가 하나로 유지된다.

비용은 **열었을 때만** 세로 206px. 기본 접힘이라 평소에는 0이고, 이것이 아코디언을 고른 대가다.
