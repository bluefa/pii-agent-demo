...
| 인스턴스 = 아코디언 밴드 | 인스턴스 행 제거 → `RdsInstancePanel`(클러스터 행 **바로 아래** colspan 밴드). **1 인스턴스 = 1줄**, 밴드는 그 합만큼 길어진다. 밴드 자체 3열(인스턴스 · 가용 영역 · 엔드포인트)을 인스턴스가 전부 채운다 — 엔드포인트·AZ는 바깥 표에 열이 없어 이번에 처음 보인다. 역할 칩만 열을 갖지 않고 이름 옆에 붙는다 |
...
**셰브런 + 인라인 밴드**가 앞의 둘을 한 컨트롤로 해결한다. 셰브런은 이 앱에 이미 있는 "여기 더 있다" 신호이고(`toggleClosed`가 primary hue를 쓰는 이유가 정확히 이것), 밴드는 자기 행 말고 다른 곳에 뜰 수가 없다. Athena 그룹과 같은 문법이라 표 안에 접힘 어휘가 하나로 유지된다.

밴드의 **아래 여백이 위보다 크다**(`mt-3` 12px / `mb-6` 24px, 오너: "아래 공간이 윗 공간보다 최소 8픽셀은 더 크게"). 밴드는 위의 클러스터 행에 속하므로, 그 행에 가깝고 다음 리소스에서 먼 것이 소속을 말한다. 두 값 모두 간격 세트 위.
