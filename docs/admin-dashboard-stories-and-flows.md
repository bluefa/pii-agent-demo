...
- [AC2] 다음 지표가 각각 카드로 표시된다:
  - **누적 시스템 수** — 등록된 전체 서비스코드 수
  - **연동 대상 DB** — 전 시스템 합산 연동 대상 DB 수
  - **연동중 DB** — 설치 완료 후 모니터링 중인 DB 수
  - **연동률** — (연동중 DB / 연동 대상 DB) × 100 (%)
  - **연동 끊김** — Unhealthy DB 수 (KPI)
- [AC3] 각 카드에 이전 대비 증감 추이가 표시된다 (선택, MVP 이후)
...
- [AC2] 테이블 컬럼:
  ...
  | 컬럼 | 설명 |
  |------|------|
  | 시스템명 | 서비스코드의 serviceName |
  | 담당자 | 시스템 담당자 이름 |
  | N-IRP 코드 | 태그 형태로 N개 표시 |
  | SW-PLM 코드 | 태그 형태로 N개 표시 |
  | 연동 방식 | AWS, Azure, GCP, IDC, SDU, 수동조사 — 태그 형태로 N개 표시 |
  | 연동 대상 DB | 해당 시스템의 연동 대상 DB 총 수 |
...
- [AC2] 클릭 시 현재 적용된 검색/필터 조건 기준으로 전체 데이터가 CSV로 다운로드된다
  - API: `GET /api/v1/admin/dashboard/systems/export?search={keyword}&integration_method={method}`
...
```
┌─────────────────────────────────────────────────────────────────┐
│ AdminHeader ("PII Agent 관리자")            [서비스 관리] [대시보드] │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  KPI Cards                                                      │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌────────┐│
...
│  └───────────────────────────────────┘           [CSV 추출]     │
│  ┌─────┬────┬──────┬───────┬───────┬─────┬───┬───┬───┬────┐    │
│  │시스템│담당│N-IRP │SW-PLM │연동방식│대상DB│ H │ U │연동중│    │
│  ├─────┼────┼──────┼───────┼───────┼─────┼───┼───┼───┼────┤    │
│  │시스템│김OO│NI-001│PLM-01 │AWS    │  45 │ 38│  3│ 41│    │    │
│  │  A  │    │NI-002│PLM-02 │Azure  │     │   │   │   │    │    │
│  ├─────┼────┼──────┼───────┼───────┼─────┼───┼───┼───┼────┤    │
│  │시스템│박OO│NI-003│       │GCP    │  12 │ 12│  0│ 12│    │    │
│  │  B  │    │      │       │IDC    │     │   │   │   │    │    │
│  └─────┴────┴──────┴───────┴───────┴─────┴───┴───┴───┴────┘    │
│  [< 1 2 3 ... 8 >]                                   총 152건  │
│                                                                 │
...
| 파라미터 | 타입 | 설명 | 기본값 |
|---------|------|------|--------|
| `search` | string | 통합 검색 (시스템명, 담당자, 서비스코드, N-IRP, SW-PLM) | - |
