...
|---|---|---|
| `CandidateResourceSection` | `getConfirmResources` | `AsyncState<CandidateResource[]>`, `selectedIds`, `vmConfigs`, approval modal state |
...
> PR #357 의 `useProjectResources`, `useProjectPageFormState` 는 **Wave 16-D4 에서 제거**. `selectedIds` / `vmConfigs` / approval modal state 는 `CandidateResourceSection` 내부로 이동 (candidate 전용 관심사이므로).

### 5-3. Section / Table 2-tier 구분
...
### 5-4. Cancellation 은 `AbortController` 로만
...
interface Props {
  ...
  readonly?: boolean;
  // selection / vmConfigs / approval 은 이 컴포넌트 내부에서 관리
}
...
### 5-5. ProjectPage 구조 (AWS/Azure/GCP)
...
- 본 문서의 섹션 **5-5 To-Be ProjectPage** 가 IDC/SDU 예외 없이 **3 provider 만** 다룸 (이미 반영 완료)
...
| 파일 | 내용 |
|---|---|
| `lib/types/resources/candidate.ts` | `CandidateResource` interface. scan 메타, `connectionStatus: 'PENDING'` literal, `isSelected` 없음 (선택은 Section state) |
...
| 파일 | 변경 |
|---|---|
| `lib/resource-catalog.ts` | `catalogToResources` → `catalogToCandidates(...): CandidateResource[]`. `confirmedIntegrationToResources` → `confirmedIntegrationToConfirmed(...): ConfirmedResource[]`. `approvedIntegrationToResources` → `approvedIntegrationToApproved(...): ApprovedResource[]`. **하드코딩(`isSelected: true`, `integrationCategory: 'TARGET'`) 제거**. |
...
| `_components/candidate/CandidateResourceSection.tsx` | `getConfirmResources` 직접 호출. `selectedIds`, `vmConfigs`, `expandedVmId`, approval modal state 를 내부 useState 로 소유. AbortController 사용. |
...
**각 Section 표준 구조** (5-4 의 의사 코드 패턴을 3 곳에 반복):
...
| 파일 | 내용 |
|---|---|
| `_components/candidate/CandidateResourceTable.tsx` | `candidates: CandidateResource[]`, `selectedIds: string[]`, `onSelectionChange`, `onCredentialChange`, `onVmConfigSave`, `onRequestApproval`, `readonly?` 등 props. 기존 `DbSelectionCard` 의 presentation 로직 이식. |
...
| 파일 | 내용 |
|---|---|
| `_components/shared/ResourceSection.tsx` | `{ step, targetSourceId }` 받아 step 에 따라 정확히 하나의 Section 렌더 (5-5 의 switch 패턴) |
