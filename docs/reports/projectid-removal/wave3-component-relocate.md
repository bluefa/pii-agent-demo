...
**⛔ CLAUDE.md 규칙 3: 상대 경로 import 금지 — `@/` 절대 경로만.**
아래 예시는 모두 절대 경로 사용.
...
`_components/` 내부 파일들이 서로를 `@/app/projects/[targetSourceId]/...` 로 참조 중. **CLAUDE.md 규칙 3에 따라 전부 `@/app/integration/projects/[targetSourceId]/_components/...` 절대 경로로 변경**:
...
**일괄 치환 (단일 치환 패턴 — 모든 파일에 동일)**:
```bash
# 이동 후, _components 하위에서만 실행
find app/integration/projects/\[targetSourceId\]/_components -type f \( -name "*.tsx" -o -name "*.ts" \) \
  -exec sed -i '' 's|@/app/projects/\[targetSourceId\]|@/app/integration/projects/[targetSourceId]/_components|g' {} +

# 잔여 확인
grep -rn "@/app/projects/\[targetSourceId\]" app/integration/projects/\[targetSourceId\]/_components/
# 기대: 0건
```

**치환 결과 예시** (모든 변환이 동일 패턴):
- `@/app/projects/[targetSourceId]/common` → `@/app/integration/projects/[targetSourceId]/_components/common`
- `@/app/projects/[targetSourceId]/aws` → `@/app/integration/projects/[targetSourceId]/_components/aws`
- `@/app/projects/[targetSourceId]/azure`, `gcp`, `idc`, `sdu` — 동일
- `@/app/projects/[targetSourceId]/idc/IdcProcessStatusCard` → `@/app/integration/projects/[targetSourceId]/_components/idc/IdcProcessStatusCard`
- `@/app/projects/[targetSourceId]/sdu/SduProcessStatusCard` → `@/app/integration/projects/[targetSourceId]/_components/sdu/SduProcessStatusCard`

상대 경로는 **쓰지 않는다** (CLAUDE.md ⛔ 규칙 3).
