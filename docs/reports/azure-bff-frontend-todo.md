...
- `test connection` 관련 API와 UI는 이번 범위에서 제외한다.
...
- [ ] upstream base path를 `/infra/v1`에서 `/install/v1` 기준으로 맞춘다.
...
- [ ] 관리자 목록 진입용 `getProjects()`를 새 `TargetSourceDetail[]` 응답 기준으로 다시 매핑한다.
  대상: [app/lib/api/index.ts](/Users/study/pii-agent-demo-azure-bff-todo/app/lib/api/index.ts)
- [ ] 새 목록 응답의 `process_status` string을 현재 FE `ProcessStatus`로 임시 매핑할지, 목록 UI를 string 기반으로 바꿀지 결정한다.
...
- [ ] 관리자 목록 테이블에서 더 이상 보장되지 않는 `projectCode`, `resourceCount`, `connectionTestComplete` 의존을 제거하거나 대체 표시로 바꾼다.
...
- [ ] `POST /install/v1/target-sources/services/{serviceCode}/target-sources` 기준으로 생성 요청 body를 맞춘다.
  대상: [app/lib/api/index.ts](/Users/study/pii-agent-demo-azure-bff-todo/app/lib/api/index.ts)
- [ ] 현재 입력 필드 중 swagger에 없는 `projectCode`를 UI에서 제거하거나 임시 로컬 label로만 쓸지 결정한다.
...
- [ ] Azure 생성 시 실제로 쓰는 값은 `description`, `cloudProvider`, `tenantId`, `subscriptionId` 중심으로 재정렬한다.
  대상: [app/components/features/ProjectCreateModal.tsx](/Users/study/pii-agent-demo-azure-bff-todo/app/components/features/ProjectCreateModal.tsx)
...
- [ ] `approval-history`와 `approval-requests/latest`가 더 이상 `input_data.resource_inputs`를 주지 않는 전제로, 요청 상세 모달을 summary UI로 축소할지 별도 상세 API를 기다릴지 결정한다.
...
- [ ] `ApprovalWaitingCard`와 `ApprovalApplyingBanner`의 "요청 내용 확인" 액션을 새 summary schema 기준으로 다시 설계한다.
...
- [ ] Azure selection 복원 로직이 approval history 상세 입력에 의존하지 않도록 보정한다.
...
- [ ] 기존 `getAzureSettings()` 의존을 제거한다. Issue #222에는 `/azure/settings`가 없고, 대신 `/azure/scan-app`과 detail metadata를 조합해야 한다.
...
- [ ] `AzureInfoCard`가 새 scan app schema `app_id/status/fail_reason/fail_message/last_verified_at`를 표시하도록 바꾼다.
...
- [ ] `AzureInstallationInline`가 새 installation status schema에 맞게 동작하도록 조정한다.
...
- [ ] Azure 전환 범위에서는 `ConnectionTestPanel`을 렌더링하지 않도록 분기하거나 임시 제거한다.
  대상: [app/components/features/ProcessStatusCard.tsx](/Users/study/pii-agent-demo-azure-bff-todo/app/components/features/ProcessStatusCard.tsx)
- [ ] Azure 경로에서 test connection API wrapper와 polling 훅이 더 이상 호출되지 않도록 정리한다.
  대상: [app/lib/api/index.ts](/Users/study/pii-agent-demo-azure-bff-todo/app/lib/api/index.ts), [app/hooks/useTestConnectionPolling.ts](/Users/study/pii-agent-demo-azure-bff-todo/app/hooks/useTestConnectionPolling.ts), [app/components/features/process-status/ConnectionTestPanel.tsx](/Users/study/pii-agent-demo-azure-bff-todo/app/components/features/process-status/ConnectionTestPanel.tsx)
