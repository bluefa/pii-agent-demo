...
- 실행형 Swagger와 실제 Next.js API 응답 전수 대조는 `/integration/api/v1/**` 공개 경로 기준으로 완료했다.
...
- [x] Next.js route handler 경로를 `/api/v1/**`에서 `/integration/api/v1/**` 기준으로 재배치한다.
...
- [x] 실행형 Swagger와 실제 Next.js API 응답이 일치하는지 `/integration/api/v1/**` 기준으로 전수 확인한다.
...
- [x] 특히 Swagger schema와 실제 응답 body shape가 달라지던 엔드포인트를 우선 보정한다.
  예: `GET /integration/api/v1/user/me`, `GET /integration/api/v1/user/services`, `GET /integration/api/v1/target-sources/{targetSourceId}`, `GET /integration/api/v1/target-sources/{targetSourceId}/process-status`, `GET /integration/api/v1/target-sources/{targetSourceId}/secrets`, `POST /integration/api/v1/target-sources/{targetSourceId}/scan`, `GET /integration/api/v1/azure/target-sources/{targetSourceId}/installation-status`
- [x] 불일치가 있으면 실행형 Swagger 또는 route 계약 설명을 실제 응답에 맞게 정리하고, 검증 결과를 PR description에 남긴다.
  대상: [docs/swagger/issue-222-client.yaml](/Users/study/pii-agent-demo/docs/swagger/issue-222-client.yaml), [docs/swagger/user.yaml](/Users/study/pii-agent-demo/docs/swagger/user.yaml)

### 14. Next.js API 공개 경로 재정렬 검토

- [x] Next.js API 공개 경로를 `/integration/api/v1/**`로 정리했다.
...
- [x] 실행형 Swagger의 호출 주소도 실제 Next.js API 경로와 동일하게 맞췄다.
...
- [x] upstream BFF 계약 경로는 계속 `/install/v1/**`를 유지한다.
...
- [x] 8단계: 최종 Swagger-실제 API 계약 검증
- [x] 9단계: Next.js API 공개 경로를 `/integration/api/v1/**`로 재정렬
...
- [x] 현재 Next.js API 공개 경로는 `/integration/api/v1/**`로 유지한다.
