...
> 검토 기준: 2026-04-28, `data-integration-platform` PR #8126 (`a3723542720279b139eef00836b85fa20aff5225`)의 `self-installation-tool` GuideController, GuideService, GuideContentValidator 기준

### 1. PR #8126에서 추가된 신규 BFF API

| Method | Path | Before | After | 변경 내역 |
| --- | --- | --- | --- | --- |
| GET | `/install/v1/admin/guides/{name}` | `구현 없음` | `{`<br>`  "name": "string",`<br>`  "contents": {`<br>`    "ko": "string",`<br>`    "en": "string"`<br>`  },`<br>`  "updatedAt": "datetime"`<br>`}` | guide name으로 저장된 ko/en content 조회 API 추가 |
| PUT | `/install/v1/admin/guides/{name}` | `구현 없음` | `{`<br>`  "name": "string",`<br>`  "contents": {`<br>`    "ko": "string",`<br>`    "en": "string"`<br>`  },`<br>`  "updatedAt": "datetime"`<br>`}` | guide name으로 ko/en content를 생성 또는 갱신하는 API 추가 |

### 2. 기존 Guide CMS 내부 Swagger 대비 변경된 계약

| 항목 | 기존 Guide CMS 내부 Swagger | PR #8126 BFF 구현 |
| --- | --- | --- |
| API Tag | `Guides` | `Admin Guides` |
| Upstream path | 별도 wave로 예정 | `/install/v1/admin/guides/{name}` |
| Guide name 검증 | `GUIDE_NAMES` enum에 없으면 `404 GUIDE_NOT_FOUND` | BFF는 name catalog를 검증하지 않음 |
| 저장 row 없음 | valid name drift이면 빈 content 반환 | 어떤 name이든 저장 row가 없으면 빈 content 반환 |
| Error format | `application/problem+json` ProblemDetails | `application/json` ErrorMessage |
| HTML 검증 실패 코드 | `GUIDE_CONTENT_INVALID` | `GUIDE_CONTENT_INVALID` |
| 요청 구조 검증 실패 코드 | `GUIDE_CONTENT_INVALID` 중심 | `VALIDATION_FAILED` |
...
| `code` | `VALIDATION_FAILED`는 요청 구조 문제, `GUIDE_CONTENT_INVALID`는 HTML 또는 빈 본문 문제를 의미한다. | 에러 코드 카탈로그 |
