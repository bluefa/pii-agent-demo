# IDC / SDU 타입 분리 + Final removal wave (combined)

> 원래 2 개 wave 였던 (1) IDC/SDU 타입 분리 + (2) `Project.resources` 필드 final removal 을 **하나의 wave 로 합친 버전**. 사용자 판단 — IDC/SDU 타입 설계는 "대충" 해도 되니 한 번에 전부 처리.
>
> **Phase 4 (`project-resources-removal`) 머지 완료 후 시작.**
>
> 본 wave 머지 후 `Project.resources` 가 코드베이스에서 완전히 사라짐.
...
1. `IdcProject` / `SduProject` 전용 타입 정의 (최소 사양, "대충" 해도 됨). `Project` 와 공통 필드는 상위 `BaseProject` 로 추출 or 복사. Resources 는 IDC 에만 포함.
2. IDC / SDU 페이지 prop 타입을 새 전용 타입으로 마이그레이션.
3. IDC 내부의 `project.resources` 사용처 3곳을 `idcProject.resources` (전용 필드) 로 치환.
4. `getProject` 의 반환 타입을 `Project | IdcProject | SduProject` union 으로 확장.
5. **`Project.resources` 타입 필드 삭제** — 기존 `@deprecated` 마킹을 실제 제거로.
6. **`lib/target-source-response.ts` normalizer 의 `resources` 라인 삭제** (공통 `Project`/`BaseProject` 타입에 없는 필드가 되므로 normalize 안 함).
7. **`mockTargetSources.get` / 전체 mock 체인의 외부 응답에서 `resources` 반환 제거**. 단 mock 내부 도메인 모델 (스캔 시뮬레이션 등) 은 계속 `resources` 유지 — IDC 용 normalize 경로만 분기.
8. **`docs/swagger/issue-222-client.yaml` 의 `ClientTargetSourceDetail.resources` 실제 삭제**. IDC 전용 응답 shape 이 필요하면 별도 스키마 추가.

## Done 정의 (combined — 본 wave 에서 모두 처리)
...
| `Project` alias 자체 | `TargetSource` 로 rename 이 깔끔하지만 호환 용도로 유지해도 됨 (판단 위임) |
| `lib/target-source-response.ts` normalize 의 `resources` 라인 **삭제** | ✅ (cloud provider) / IDC 는 별도 normalizer |
| `mockTargetSources.get` 외부 응답의 resources **제거** | ✅ (IDC 만 IDC-전용 변환기로 resources 포함) |
| `docs/swagger/issue-222-client.yaml` 의 `ClientTargetSourceDetail.resources` **삭제** | ✅ (IDC 전용 스키마 별도 정의) |

### "대충" 가이드

- 타입 계층 설계에 과도한 시간 쓰지 말 것. `BaseProject` + 3 서브타입 + union 정도면 충분.
- IDC/SDU 페이지 내부 로직은 건드리지 않음 — prop 타입만 교체하고 기존 로직 유지.
- SDU 는 원래 `project.resources` 를 쓰지 않으므로 타입 교체만.
- IDC 의 `IdcResourceTable` / `FirewallGuide` 등 하위 컴포넌트는 prop 이 바뀌지 않는 이상 그대로.
- 깔끔한 "전용 타입" 설계는 본 wave 범위 밖. 추후 별도 리팩토링 가능.
...
```
refactor(target-sources): IDC/SDU 타입 분리 + Project.resources 필드 final removal (idc-sdu-final-removal)

근거: docs/reports/resource-data-source/phase4-project-resources-removal.md § Done 정의 (후속 wave, combined).

- lib/types.ts: BaseTargetSource / CloudTargetSource / IdcTargetSource / SduTargetSource / TargetSource union 정의. Project.resources 필드 삭제.
- lib/target-source-response.ts: normalizer 분화. Cloud provider 는 resources 제외, IDC 전용 normalizer 는 resources 포함.
- app/lib/api/index.ts: getProject 반환 타입 Project → TargetSource.
- IDC/SDU 페이지 prop 타입 마이그레이션 (project.resources → idcProject.resources).
- AWS/Azure/GCP 페이지 prop 타입 마이그레이션 (CloudTargetSource 로 narrow).
- Mock 응답 변환기: Cloud 는 toIssue222TargetSourceInfo (resources 제외), IDC 는 전용 toIdcTargetSource (resources 포함), SDU 는 toSduTargetSource.
- docs/swagger/issue-222-client.yaml: ClientTargetSourceDetail.resources 삭제. IDC 전용 schema 추가.
- Mock 내부 도메인 모델 (Project 도메인 객체 in mockData, mockProjects 등) 은 시뮬레이션 용으로 resources 필드를 계속 보유 — 외부 응답 경로에서만 제외.
```
...
**Phase 4 follow-up** (`docs/reports/resource-data-source/phase4-followup-step-resources-fetch-issue.md`): 본 wave 머지 + stabilization 후 — step 4-7 `confirmed-integration` 중복 호출 제거 (`IntegrationTargetInfoCard` prop 확장).
