...
- [ADR-022](022-terminal-state-notification.md) — terminal-state notification reuses this claim/lease model directly: a terminal-unnotified pipeline is claimable work (tx1 claim → deliver → guarded `notified_at` write-back), so notification inherits the two-transaction split and fencing with no separate relay/lease
...
- 2026-07-03 (alignment pass, post-LIN-30 audit): aligned the claim-index description to the
  composite `(status, next_due_at)` index the MySQL 8 orchestrator implementation uses — the
  LIN-30 revision had described it as a PostgreSQL-style partial index, but MySQL 8 (the target
  engine) has no partial indexes; added the MySQL 8 + JPA pseudo-SQL note
