...
external_id 유무·동기/비동기·fan-out이 특수 케이스를 만들지 않는다: **dispatch가 있는 task는 현재 attempt를
`context.attempt`로 받고, dispatch가 없는 조건 대기 task는 `context.attempt = null`이며 check는 `input`
만으로 조건을 평가한다.** 동기는 response의 결과 필드를, fan-out은 response의 handle N개를 평가 — 분기
...
- 실행 입력이 `pipeline.parameters`(데이터)가 되어 실행 단위·입력 변경이 스키마 ALTER를 강제하지
  ...
  (결정 2; 1:N 등 단위 확장은 additive·O22).
- dispatch 산출을 `attempt.response(jsonb)`로 보존해 handle 1/N/0개를 정형 컬럼 없이 흡수하고,
  check 입력을 단일 객체 `TaskExecutionContext{input, attempt?}`로 통일해 id 없는 task가 특수 케이스가
  되지 않으며, 한 action이 N개를 깔 때도 attempt 1 : check N + 집계로 흡수한다(결정 1.1/1.4/1.6).
...
| S24 | check/postCheck 입력 계약 = **단일 객체 `TaskExecutionContext{input, attempt?}`**(2차) — 개별 인자 아닌 객체라 필드 추가에 시그니처 불변; external_id 직접 안 받음(handle은 attempt.response); attempt?는 nullable(조건 task=null → input만 평가, "attempt 항상 존재" 오류 정정); 동기/비동기·fan-out 비분기(response 형태로 흡수); 차이는 결과의 쓰임뿐(check=전이 입력, postCheck=관측) → 결정 2/1.4/1.5 (2026-06-14, 2차 개정) |
