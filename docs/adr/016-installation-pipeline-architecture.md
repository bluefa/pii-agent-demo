...
task_check      id, task_id, checked_at, started_at,
                ...
                -- attempt_id(nullable): fan-out(①)+재dispatch에서 handle-poll 행을 attempt 경계로
                --   묶을 때만 발사 시점 기록; 단수 handle이면 external_handle join으로 충분(NULL). 결정 1.8/3.1.
...
③ 사고 조사 surface가 task 단위 merged timeline이다. attempt와의 상관관계가 필요하면 양쪽의
`external_handle` 매칭으로 soft link가 복원된다 — 모호한 경우 link가 없는 것이 정확한 표현이다.
...
- **dispatch (0..1)**: 실행(side effect) API 호출. handle을 받으면 external_handle로 저장.
  복수 dispatch가 필요하면 별도 task로 분리. **복수 handle이 나오면 먼저 구분한다**: ② 독립 action
  N개를 묶은 것이면 **task N개로 분리**(기본·대부분의 답) → 각 attempt·handle은 단수로 복귀;
  ① 한 dispatch가 원자적으로 N handle을 내고 분리 불가면 attempt는 단수로 두고 **handle N개를 각각
  CHECK용 task_check 행으로** 만들어 독립 폴링, task 완료 = "모든 handle SUCCEEDED" 집계(결정 3.1).
  ①/②는 입력 따라 갈림(Open O23).
- **check (0..1)**: 완료 확인 호출. task당 최대 하나의 check 경로.
...
- **timeoutPolicy**: per-call HTTP deadline, execution timeout, TTL. **per-call deadline은
  ...
  시간 + 여유(예: 90초/240초).
- **completionRule**: 외부 응답을 task 결과(DONE|PENDING|FAILED)로 매핑하는 규칙.
...
모델에 내재한다 — dispatch가 낸 handle을 check가 폴링하며 `task_attempt.external_handle ↔
task_check.external_handle`로 연결된다(신규 없음). (B) **handle 외 산출(생성 리소스 ID·응답값)** 은
...
4. 복구 규칙: external_handle 없이 dispatch timeout(Part II)보다 늙은 DISPATCHING 행은 단순
...
**attempt는 action 단수를 유지한다**(handle을 배열 컬럼으로 만들지 않음) — dedup·복구 추론이
"action 1회, 재dispatch = 통째로 새 시도"에 의존하기 때문이다. 한 dispatch가 원자적으로 **N
handle**을 내는 진짜 fan-out(①)에서는 attempt를 단수로 두고, handle N개를 각각 CHECK용 task_check
행(행의 external_handle에 개별 handle)으로 독립 폴링하며 task 완료 = **"모든 handle SUCCEEDED"
집계**(reconciler 완료 로직에 한 줄). 새 테이블 불요 — task_check가 호출당 1행이라 다행을 자연
수용한다(정규화 자식 테이블은 과함, 미채택). **①+재dispatch에서만** 이전/새 시도의 handle 집합을
attempt 경계로 구분하려 handle-poll task_check 행에 `attempt_id`를 발사 시점에 기록한다(단수
handle이면 external_handle join으로 충분, attempt_id NULL) — attempt↔check 명시 링크가 *처음으로*
값을 갖는 지점이다. (독립 action이면 ②로 task를 나누는 것이 기본 — Open O23.)
...
| dispatch 호출 후, external_handle 영속화 전 | DISPATCHING 잔류 → dispatch 복구가 재dispatch; dedup이 중복 흡수 |
...
| Dispatch 복구 | external_handle 없는 DISPATCHING 경과 | 재dispatch |
...
| Dispatch 복구 timeout | 5분 | 전역 | external_handle 없는 DISPATCHING 재dispatch (3.1) |
...
| S21 | external_handle 복수/fan-out: attempt는 action 단수 유지(배열 컬럼 금지); ② 독립 action은 task 분리(기본), ① 원자적 N handle은 handle별 CHECK 행 + "모든 handle SUCCEEDED" 집계(새 테이블 불요); ①+재dispatch에서만 task_check.attempt_id 발사 시점 기록 → 결정 1.2/2/3.1, Open O23 (2026-06-14) |
...
- **DB migration** — task_check.api_result에 PENDING 추가, task_check.started_at 추가,
  ...
  parameters 키 + 비정규화 복제 컬럼 유지(O21). task_check.attempt_id(nullable) 추가(fan-out+
  재dispatch 조건부 사용).
...
| O23 | fan-out 케이스가 ①(분리 불가 원자적 N handle)인가 ②(독립 action)인가 | ②면 task 분리(모델 무변경, 기본), ①이면 handle별 CHECK 행 + "모든 handle SUCCEEDED" 집계. 현재 입력 미확인 |
