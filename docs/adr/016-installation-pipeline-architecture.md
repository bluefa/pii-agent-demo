...
Proposed (2026-06-11) · 전면 재구성 (2026-06-12) · tick 외부호출 모델 통합 (2026-06-13) · N-cap soft target 확정 / P0 해소 (2026-06-13) · **poll 동시성 budget D-T7 (2026-06-13)**
...
(VT + per-call deadline, D-T1/D-T2), (나) dispatch 폭주로부터 worker 큐 보호(N-cap, 결정 4b).
빠진 **(다)가 poll/check 호출량이 IM API 서버(M개 pod)를 과부하**시키는 축이다. **poll은 slot과
...
- **C는 넉넉히 잡는다.** 대부분의 호출은 sub-second로 끝나 budget을 즉시 비우므로, C는 드문 느린
  호출(200초+) 집중과 burst만 묶으면 된다. R5 런타임 설정(O17).
...
timeout, per-call HTTP deadline(post-check 및 task별 오버라이드 포함), 외부 호출 동시성
budget(C, 결정 6 D-T7), breaker 임계(pickup window, probe 간격), task_check 보존, 알림
라우팅은 DB 기반 런타임 설정으로 관리자 설정 페이지에서 변경하며 변경은 이벤트로 감사된다.
운영 튜닝에 재배포가 필요 없다.
...
| O15 (신규) | 느린 호출/빠른 호출 VT 병렬 자원 격리(별도 lane) 필요? | 대량 동시 설치/삭제 실재 시만 재검토; 현재 불필요. D-T7 budget C는 단일 전역 한도라 fast/slow lane 분리와는 별개(O17) |
| O16 (신규) | 200초 check API 자체의 최적화(provider 비동기화 / 확인 빈도 완화) | 별도 과제로 분리 — 결정 6은 아키텍처 방어이지 근본 최적화가 아님 |
| O17 (신규) | 외부 호출 동시성 budget C 기본값 + 단일 tick 내 동시 발사 상한(thundering-herd) | 방향 확정(PENDING 기반 in-flight budget, D-T7); C 값·tick당 발사 상한 미정. 빠른 호출 다수라 C는 넉넉히 |
...
| S11 (신규) | N-cap = soft target(잘 설계된 worker 큐가 실질 backpressure); 상태기계 정확성만 leader 무관, capacity는 leader-serialized soft; split-brain·잔존 job·외부 caller 일시 초과 감수; 하드 글로벌 상한은 BFF lease 아닌 Infra Manager 429 → 결정 3.2, 4b (2026-06-13) |
| S12 (신규) | poll 부하(축 다)는 in-flight 동시성 budget C로 제어 — PENDING task_check로 count(새 상태·WAIT_EXTERNAL 계수 아님), C 넉넉히, soft·leader-serialized, IM 429→requeue(fail_count 미소모); 3.3 catch-up은 dispatch=N-cap·poll=C → 결정 6 D-T7 (2026-06-13) |
