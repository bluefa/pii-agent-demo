...
**task success = reconciler가 그 단일 handle의 최신 check 결과를 completionRule(DONE|PENDING|FAILED)로
평가**: DONE이면 task DONE, FAILED면 fail_count 정책, PENDING이면 계속 폴링. 한 dispatch가 분리 불가한
...
- dispatch 산출을 `attempt.response(jsonb)`로 보존해 dispatch 종류별 응답 형태를 정형 컬럼 없이
  ...
  task가 특수 케이스가 되지 않는다(결정 1.4); 모든 dispatch가 단수 handle이라 attempt:handle은 1:1이다
  (그 handle은 terminal까지 N번 폴링되어 attempt:task_check는 1:N).
