...
부재=성공"이 멱등 정의에 포함되어야 한다.) 각 dispatch task가 실제로 이 불변식을 만족하는지는 task별
검증 대상이다(Open O28).
...
최신 attempt.response에서 추출한 미완 id만 폴링하고, 이전 고아 제출은 방치된다 — 고아 job도 실제 실행되나 멱등 apply라 인프라에 무해하고,
...
N — 결정 2/1.6). 재dispatch = 통째로 새 attempt = 새 response이므로 멱등성·복구 추론("action 1회")이
...
O-번호는 본문 cross-reference와 **공유**하며(O8·O10·O18–O20·O27·O28), 해소되면 Part III **Resolved**로 옮긴다.
