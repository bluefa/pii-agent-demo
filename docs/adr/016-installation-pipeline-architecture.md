...
- 규범적 설계는 **Part I**, 설정 기본값은 **Part II**, 결정 변경 이력·해소된 질문은 **Part III**,
  구현 노트·미해결 질문·영향 파일은 **Part IV**에 둔다.
...
- 단일 논리 스케줄러(reconciler)가 고정 tick(주기는 Part II)으로 깨어나 due task를 선별한다 —
  아직 dispatch 안 된 READY(파이프라인별 최저 seq의 현재 task), 상태 확인이 도래한
  RUNNING/WAITING_EXTERNAL, 시간 초과 task —
...
pipeline        id, parameters(jsonb), type(INSTALL|DELETE), provider,
                ...
                target_source_id
                -- parameters = 실행 입력(현재 {target_source_id}). 생성 시 박제(append-only);
                --   실행 단위/입력이 바뀌어도 컬럼 ALTER 없이 키만 추가. dispatch/check가 값을 꺼내 씀.
                -- target_source_id = parameters['target_source_id']의 비정규화 복제(조회 인덱스용).
                --   진실의 출처는 parameters; 복제 컬럼 유지 여부는 Open O21.

task            id, pipeline_id, seq, name, type(EXECUTE|WAIT_EXTERNAL),
                status(READY|WAITING_SLOT|DISPATCHING|RUNNING|
                ...
                -- 단수 external_handle 컬럼 없음: dispatch 산출(handle 1/N개)의 home은
                --   attempt.response(jsonb); 폴링 대상 미완 id는 거기서 추출(결정 3.1).
...
task_attempt    id, task_id, attempt_no, started_at, finished_at,
                ...
                --   handle(들)의 home — 단수/복수/0개를 JSON 구조로 흡수(terraform {job_id} ·
                --   fan-out {handles:[...]} · 동기 {result}). 보존용이지 매 tick control 입력이
                --   아니다 — handle 추출(normalize)은 dispatch 시점 코드의 일(결정 2/3.1).
...
task_check      id, task_id, checked_at, started_at,
                ...
                error_code, latency_ms, external_handle, attempt_id?, detail(jsonb)
                -- 외부 호출당 1행(관측의 장부); Task 소속. id마다 자기 행 — 한 행이 여러 id를 묶지 않음.
                --   N개 id 확인 시 행 단위(호출마다 N행 vs check 1회 1행)는 미정(Open O24).
                ...
                -- attempt_id(nullable): N handle+재시도 시 check↔attempt 연결용; 사용 여부는 행 단위 O24에 종속(Open O26).
...
정당성이 오히려 강화된다. 또한 **한 action이 N개 확인 대상을 원자적으로 낳는 경우(attempt 1 : check N, fan-out ①)** 도 중첩
구조면 어색하지만 형제라 1:N이 깔끔하다. force-check도 attempt 경계와 무관하다. ② crash 복구 순간에는 check의
attempt 소속이 본질적으로 모호하며, 관측은 사실이고 사실은 해석보다 먼저 기록되어야 한다.
③ 사고 조사 surface가 task 단위 merged timeline이다. attempt와의 상관관계가 필요하면
task_check.external_handle(확인한 id)이 attempt.response의 id 집합에 속하는지로 soft link가
복원된다(또는 attempt_id — O26). 모호한 경우 link가 없는 것이 정확한 표현이다.

내부 task 상태가 UI 어휘보다 풍부한 것은 의도다. 매핑:

| 내부 | 보드 라벨 |
|---|---|
| READY / WAITING_SLOT | 대기 (WAITING_SLOT은 큐 순번 추가 표시) |
...
(의존성 대기를 위한 BLOCKED 상태도 두지 않는다 — task는 READY로 시작하고, 순차 chain이라
"predecessor DONE" 여부는 seq 순서에서 파생된다. reconciler가 파이프라인별 최저 seq 비terminal
task만 전진시키므로 의존성은 별도 상태 없이 지켜진다. 따라서 task 상태는 9종이다.)
...
- **dispatch (0..1)** — `dispatch(parameters)`: 실행(side effect) API 호출. **원응답을 통째로
  `attempt.response(jsonb)`에 보존**(handle 1/N/0개를 JSON 구조로 흡수 — 단수 컬럼 없음). 복수
  dispatch가 필요하면 별도 task로 분리.
- **check (0..1)** — `check(attempt, parameters)`: 완료 확인 호출(결과는 **상태 전이 입력**).
  task당 최대 하나의 check 경로. external_id를 직접 받지 않고 attempt를 통째로 받는다(결정 1.4).
- **postChecks (0..N)** — `postCheck(attempt, parameters)`: 완료 후 best-effort 결과 조회.
...
- **timeoutPolicy**: per-call HTTP deadline, execution timeout, TTL. **per-call deadline은
  ...
  시간 + 여유(예: 90초/240초).
- **completionRule**: 외부 응답을 결과로 매핑하는 규칙(개별 id: DONE|PENDING|FAILED) + **id
  집합에 대한 집계**(기본 = 전부 SUCCEEDED → task DONE). 결정 1.6.
...
**입력 계약 — `check(attempt, parameters)`, external_id를 직접 받지 않는다.** response가 attempt에
포함되므로 attempt를 주면 산출물 접근이 보장되고, 미래에 다른 attempt 필드(attempt_no 등)가 필요해도
시그니처 변경 없이 코드만 바꾼다. external_id를 직접 받는 형태면 **id 없는 task(조건·동기)** 가
"무엇을 넘기나(null?)"로 난감해진다 — attempt는 **항상 존재**하므로 그 특수 케이스가 사라진다.
동기/비동기도 분기하지 않고 response 형태로 흡수한다(handle 있음→폴링, 결과 있음→평가, 없음→조건).

**attempt 1 : check N (fan-out ①).** 한 dispatch(attempt 1행)가 external_id를 N개 낼 수 있고 그
N개 각각이 확인 대상이다 — **attempt는 단수 유지**(action 1회), 확인만 N개다(형제 구조가 1:N을
자연 표현). **task success = reconciler 집계**: 기대 집합(분모) `{id1..idN}`을 `attempt.response`
에서 추출해 전부 SUCCEEDED면 DONE, 하나라도 FAILED면 fail_count 정책, RUNNING 남으면 계속.
**다음 check는 미완 id만** 재호출한다(terminal id 재폴링 안 함 — C·IM 낭비 방지; 완료 id 결과는
id별 최신 check 행 조회로 보존, Open O27). 저장 단위(id별 관측)와 판정 단위(task 전체)가 분리된다.
(이는 독립 action을 task로 나누는 ②와 무관 — 여기는 "한 호출이 N개를 원자적으로 깐다".)
...
> **현재 실행 단위 = `target_source_id`(1 pipeline : 1 target)** 이며, 변경은 pipeline 모델
> 변경을 수반한다(여러 target 1:N은 의도가 아님). 단위 변경 시 확장 경로(group 묶기 /
> pipeline_target 정규화)는 **어느 것도 task에 target_source_id를 넣지 않는다** — Open(O22)으로만
> 박제하고 지금 만들지 않는다.

**context 전파 (설계 지금, 구현 보류).** task 간 값 전달은 두 경우다. (A) **handle 참조**는 이미
모델에 내재한다 — dispatch가 낸 handle은 attempt.response에 보존되고, check가 그걸 폴링하며
task_check.external_handle(확인한 id)이 attempt.response의 id와 매칭되어 연결된다(신규 없음). (B) **handle 외 산출(생성 리소스 ID·응답값)** 은
attempt 산출을 `pipeline.parameters`(= 진화하는 context)에 **append-only로, 상태 전이와 같은
트랜잭션(결정 3.2)에** 머지하고 후속 task가 읽는다(덮어쓰기 금지 → 재시도 결정성·crash 안전).
효과: task 간 의존이 컬럼·코드 결합이 아니라 **공용 context의 read/write**로 표현되어 task는
서로를 모르고 context만 안다 — **Execution/Check가 특정 차원(실행 단위)이 아니라 추상 context에만
의존**하므로 실행 단위가 바뀌어도 형태·불변식이 불변이다. parameters 컬럼과 "append-only·tx 머지
가능한 context"로의 **설계는 지금** 하되, task→task 값 전달 **머지 구현은 실제 필요가 확인될 때**
한다(YAGNI).
...
4. 복구 규칙: response 없이(dispatch 응답 미영속) dispatch timeout(Part II)보다 늙은 DISPATCHING
   행은 단순 재dispatch. 최악의 경우 중복 제출이 가지만 실행은 1회다.

terraform_job_id는 요청별 서버 측 발급이므로 재dispatch는 항상 새 job_id를 낳는다. BFF는
최신 attempt.response에서 추출한 미완 id만 폴링하고, 이전 고아 제출은 방치된다 — worker dedup이 2차 실행을 막고,
고아 job이 결과를 영영 안 보고해도 아무도 폴링하지 않는다. execution timeout(결정 4)이 결과
유실 job 포함 모든 대기 경로를 bound한다.

**attempt는 action 단수를 유지하되, dispatch 산출은 `attempt.response(jsonb)`에 보존한다**(단수
external_handle 컬럼 제거 — handle 1/N/0개를 JSON 구조로 흡수). 한 dispatch가 원자적으로 N개
external_id를 내는 fan-out(①)에서도 attempt는 1개(action 1회)이고 확인만 N개다(attempt 1 : check
N — 결정 2/1.6). 재dispatch = 통째로 새 attempt = 새 response이므로 dedup·복구 추론("action 1회")이
그대로 성립한다. `task_check.external_handle`은 *어느 id를 확인했는지의 참조*일 뿐 handle 저장소가
아니다(1차안 "handle을 task_check로 단일화"는 산출물과 관측을 섞은 오류라 철회). check 행 단위
(id별 N행 vs 1행)와 attempt↔check 명시 링크용 `attempt_id` 사용은 Open O24/O26.
...
| dispatch 호출 후, attempt.response 영속화 전 | DISPATCHING 잔류 → dispatch 복구가 재dispatch; dedup이 중복 흡수 |
...
- **감지 (open).** primary: dispatch된 job이 pickup window(Part II) 내 미수령 — job 상태가
...
→ WAIT_EXTERNAL 하나로 충분. **task 상태는 9종**(BLOCKED 제거 — 의존성 대기는 seq 순서에서
파생되므로 불필요, 1.2 참조). 그 외 상태는 각각 고유 전이/복구를 가져 더 줄이면 다른 구분자가
늘어 순손실이다.
...
"무제약"이 아니다. **C는 단일 전역 budget**(O15)으로 IM·Backend Manager·provider가 같은 C에서
차감한다 — fast/slow lane이나 backend별로 쪼개지 않는다. ("M"은 *보호 대상*(IM API pod 수)이지
*제약값*이 아니다 — 실제 한도는 C다.)

- **메커니즘: tick당 in-flight 동시성 budget C.** 매 tick 리더가
  `in_flight = COUNT(task_check WHERE api_result=PENDING AND started_at > now()-deadline)`,
...
- **in-flight를 새 상태가 아니라 PENDING task_check로 센다.** D-T5가 모든 호출을 직전에 PENDING으로
  ...
  "활성 호출"이 아니다(2000개 대기 ≠ 5개 호출 중). ③ budget은 *호출* 단위인데 task_check가 호출당
  1행이라 granularity가 맞다(한 task가 dispatch+check+post-check로 여러 호출). deadline을 지난
  PENDING은 in-flight가 아니라 좀비이므로 count에서 제외하고 조사 플래그(D-T5).
- **신규 스키마 없음.** `task_check.api_result=PENDING`과 `started_at`은 D-T5가 이미 추가한다.
- **C는 넉넉히 잡는다.** 대부분의 호출은 sub-second로 끝나 budget을 즉시 비우므로, C는 드문 느린
  호출(200초+) 집중과 burst만 묶으면 된다. R5 런타임 설정(Part II; 값 산정 O17).
...
- 실행 입력이 `pipeline.parameters`(데이터)가 되어 실행 단위·입력 변경이 스키마 ALTER를 강제하지
  않고, Execution/Check가 특정 차원이 아닌 추상 context에만 의존한다(결정 2; 1:N 등 단위 확장은
  additive·O22).
- dispatch 산출을 `attempt.response(jsonb)`로 보존해 handle 1/N/0개를 정형 컬럼 없이 흡수하고,
  check 입력을 `(attempt, parameters)`로 통일해 id 없는 task가 특수 케이스가 되지 않으며, 한 action이
  N개를 깔 때도 attempt 1 : check N + 집계로 흡수한다(결정 1.1/1.4/1.6).
...
| N (slot cap) | 10 (초안 3) | 전역 | 동시 slot 보유 task soft target (4b) |
| C (외부 호출 동시성 budget) | 넉넉히 (값 O17) | 전역 | 동시 in-flight 외부 호출 상한; poll 부하 보호 (D-T7) |
| max_fail_count | task별 | task별 | 자동 재시도 한도 (1.2) |
| breaker pickup window | 5분 | 전역 | dispatch 후 미수령 감지 (4d) |
...
- **BLOCKED 상태 제거 (2026-06-13):** task 상태 10→9종. 순차 chain에서 의존성 대기는 seq에서
  파생되므로 별도 상태 불필요.
- **결정 모델 정련 (2026-06-13~14):** Task=훅 조합·slot=공유 자원·N/C 두 축·kind=CHECK 통합·
  선기록 위치·pipeline.parameters·context 전파·dispatch 산출=attempt.response·attempt 1:check N·
  입력 계약(attempt,parameters) (Resolved S14–S25).
...
| S13 | task 상태 BLOCKED 제거(10→9종) — 순차 chain이라 의존성 대기는 seq 순서에서 파생되어 별도 상태 불필요; task는 READY로 시작, reconciler가 최저 seq 비terminal task만 전진 → 결정 1.1/1.2, 6 D-T6 (2026-06-13) |
...
| S19 | pipeline.parameters(jsonb) 신설 — 실행 입력을 고정 컬럼 아닌 데이터로; target_source_id는 parameters 키(+조회용 비정규화 복제); 생성 시 박제(append-only); 현재 실행 단위 = target_source_id(1:1), 1:N은 의도 아님 → 결정 1.2/2, Open O21/O22 (2026-06-14) |
| S20 | context 전파: handle 참조는 모델 내재(A); handle 외 산출(B)은 parameters에 append-only·tx 머지로 후속 task에 전달 → Execution/Check가 실행 단위 아닌 추상 context에 의존. 설계 지금, 머지 구현 보류(YAGNI) → 결정 2 (2026-06-14) |
| S21 | external_handle 복수/fan-out: attempt는 action 단수 유지(배열 컬럼 금지); ② 독립 action은 task 분리(기본), ① 원자적 N handle은 handle별 CHECK 행 + "모든 handle SUCCEEDED" 집계(새 테이블 불요); ①+재dispatch에서만 task_check.attempt_id 발사 시점 기록 → 결정 1.2/2/3.1, Open O23 (2026-06-14) **(일부 S22가 supersede)** |
| S22 | (S21·1차 §1.8 일부 supersede) dispatch 산출 = task_attempt.response(jsonb)에 보존(단수 external_handle 컬럼 제거); handle 1/N/0개를 JSON으로 흡수, write-once; handle home은 attempt이지 task_check 아님 — task_check.external_handle은 "확인한 id 참조"(1차 "handle을 task_check로"는 산출/관측 혼동 오류라 철회) → 결정 1.2/2/3.1 (2026-06-14) |
| S23 | attempt 1 : check N — 한 dispatch가 N external_id를 내면 attempt 단수·확인 N(②task 분리와 무관); success = reconciler 집계(분모는 response의 id 집합); 다음 check는 미완 id만 재호출 → 결정 2/1.6/1.7 (2026-06-14) |
| S24 | check/postCheck 입력 계약 = (attempt, parameters) — external_id 직접 안 받음(id 없는 task가 특수 케이스 안 됨); 동기/비동기 비분기(response 형태로 흡수); 차이는 결과의 쓰임뿐(check=전이 입력, postCheck=관측) → 결정 2/1.4/1.5 (2026-06-14) |
| S25 | task_check = 관측 장부; external_handle은 "확인한 id 참조"(저장소 아님); id마다 자기 행 — 행 단위(N행 vs 1행)는 Open O24, attempt_id는 O26, 동기 check 행은 O25, 완료 id 보존은 O27 (2026-06-14) |
...
- **DB migration** — task_check.api_result에 PENDING 추가, task_check.started_at 추가,
  ...
  parameters 키 + 비정규화 복제 컬럼 유지(O21). **task_attempt.external_handle 제거 →
  response(jsonb)**(dispatch 원응답, write-once); **task.external_handle(단수) 제거**
  (handle home=attempt.response). task_check.attempt_id(nullable)는 행 단위 O24 확정에 종속(O26).
...
| O13 | next_check_at 발사 시 미는 간격의 구체 산정 | 방향만 확정(발사 시 밀기); 간격값 미정 |
| O14 | PENDING으로 오래 남은 task_check의 조사 플래그/정리 정책 | 보존은 확정; 운영 도구 미정 |
| O15 | 느린 호출/빠른 호출 async 자원 격리(별도 lane) 필요? | 대량 동시 설치/삭제 실재 시만 재검토; 현재 불필요. D-T7 budget C는 단일 전역 한도라 fast/slow lane 분리와는 별개(O17) |
| O16 | 200초 check API 자체의 최적화(provider 비동기화 / 확인 빈도 완화) | 별도 과제로 분리 — 결정 6은 아키텍처 방어이지 근본 최적화가 아님 |
| O17 | 외부 호출 동시성 budget C 기본값 + 단일 tick 내 동시 발사 상한(thundering-herd) | 방향 확정(PENDING 기반 in-flight budget, D-T7); C 값·tick당 발사 상한 미정. 빠른 호출 다수라 C는 넉넉히 |
...
| O21 | `pipeline.target_source_id` 비정규화 복제 컬럼 유지 여부 | 진실의 출처는 parameters; 기존 인덱스 pipeline(target_source_id, started_at) 유지엔 복제 필요 vs 단일 출처. 현재 복제 유지, 미정 |
...
| O24 | N개 id 확인 시 task_check 행 단위 | (가) 외부 호출마다 1행(id별 N행 — 개별 latency/error·PENDING·C 카운트·조사 보존, 권장) vs (나) check 1회당 1행(종합). 입력 계약과 독립. 다음 확정 1순위 |
| O25 | 외부 호출 없는 check(동기·조건)의 행 기록 | "1평가 1행"으로 호출 없어도 행 남길지; 남기면 C budget은 실제 외부 호출 발생 행만 카운트(호출 없는 평가는 IM 부하 0). 미정 |
| O26 | task_check.attempt_id 사용 여부 | N handle+재시도 시 check↔attempt 연결용. O24를 (가)로 확정하면 필요(발사 시점 기록); 단수/무재시도면 불요. O24에 종속 |
