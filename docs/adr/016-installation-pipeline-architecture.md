...
| S21 | external_handle 복수/fan-out: attempt는 action 단수 유지(배열 컬럼 금지); ② 독립 action은 task 분리(기본), ① 원자적 N handle은 handle별 CHECK 행 + "모든 handle SUCCEEDED" 집계(새 테이블 불요); ①+재dispatch에서만 task_check.attempt_id 발사 시점 기록 → 결정 1.2/2/3.1, Open O23 (2026-06-14) **(일부 S22가 supersede; attempt_id는 O26 해소로 드롭)** |
...
O-번호는 본문 cross-reference와 **공유**하며(O7·O18–O23·O25·O27), 해소되면 Part III **Resolved**로 옮긴다.
