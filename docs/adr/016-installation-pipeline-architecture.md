# ADR — 설치/삭제 파이프라인 오케스트레이션 아키텍처 (개정 4판)

## Status

Proposed (2026-06-11) · 살아있는 문서 — 토의로 발전 중.
개정 4판(2026-06-14) — **단순화 리팩토링**: workflow engine 일반화를 줄이고 현재 실제
요구사항(target source 설치/삭제 자동화)에 맞는 최소 구조로 수렴. circuit breaker/canary 제거,
global C-budget → tick당 발사 상한(`max_external_calls_per_tick`), force-check 제거,
postCheck 0..1로 축소, `pipeline.parameters` 제거 → `target_source_id` 고정, TaskDefinition 훅
일반화 제거 → TaskKind 3종, `TaskExecutionContext` 제거. 변경 이력은 Part III.
...
- BFF가 다중 replica로 뜨면 DB advisory lock(`pg_try_advisory_lock`)을 잡은 한 pod만 tick을
  ...
  안전하다(검증: 결정 3.3).
- UI 액션(재시도/중단)은 Infra Manager를 직접 호출하지 않는다. row에 intent를
...
task            id, pipeline_id, seq, name,
                kind(TERRAFORM_JOB|GENERAL_JOB|CONDITION_CHECK),
                ...
                -- kind = TaskKind 3종(결정 2); dispatch/poll 흐름과 slot 소비 여부가 kind에서 정해진다
                --   (TERRAFORM_JOB이 공유 IM slot을 소비, 결정 4b).
...
| Observation | task_check | Task에 대한 모든 외부 호출 — dispatch, check(핸들 폴링·조건 평가), post-check |
...
| WORKER_OUTAGE_SUSPECTED — systemic timeout 롤업 1건 | critical | 인앱 |
...
2. 행/task 패널: 다음 확인 카운트다운; WAITING_SLOT 큐 순번. (강제 확인 버튼은 제거 — 모든
...
- **TERRAFORM_JOB** — `dispatch` → job_id 획득 → `poll` → terminal. Infra Manager run API를
  호출해 job_id를 받고(`attempt.response = {job_id}`), 그 job을 terminal까지 폴링한다. **BFF 발 terraform job 총량을 N·K로 bound**하기 위해 공유 IM
  동시성 slot(제약 #5)을 소비한다(결정 4b).
...
- **reconciler가 유일한 외부 호출자.** UI/API 액션은 intent만 기록하고 tick이 실행한다.
  TaskKind별 dispatch/poll/postCheck의 모든 외부 호출은 reconciler tick에서만
...
**N-cap의 목표: BFF 발 terraform job 총량 bound.** N-cap은 BFF(현재 유일한 자동화 caller)가 생성하는
동시 실행 TERRAFORM_JOB의 총량을 **N·K 이하로 bound**하는 장치다(N = slot cap, K = 재dispatch 상한 =
max_fail_count). 목표는 글로벌 하드캡이 아니라 **무분별한 job 생성 방지** — Pipeline이 통제 없이
terraform job을 양산하는 것을 막고, 최악의 경우(연쇄 crash 재dispatch 포함)에도 BFF가 만드는 동시
job이 N·K를 넘지 않음을 BFF 스스로 보증한다. 이 보증은 **IM의 backpressure(429) 신뢰성에 의존하지
않는다** — IM이 429를 던지지 않아도 BFF가 자기 발 동시성을 N·K로 묶는다. (반면 poll/check 부하
제어인 `max_external_calls_per_tick`은 정밀 강제를 IM 429에 위임한다 — 이 둘의 차이가 N-cap의 고유
가치다.)

- **TERRAFORM_JOB task가 slot을 소비한다(결정 2).** slot 보유 task는 WAITING_SLOT로 진입하고, 매
  tick 스케줄러가 slot 보유 task(DISPATCHING|RUNNING)를 COUNT해 N 미만일 동안 큐에서 admit한다.
  **별도 카운터 없이 상태 COUNT로 센다** — 카운터 증가와 상태 변경이 둘로 갈리는 dual-write를
  피한다. admission은 전역 task 순서를 따른다 — 파이프라인 내 FIFO, 파이프라인 간 공정. 우선순위
  클래스 없음.
- **slot이 보호하는 것은 공유 IM 동시성 자원(제약 #5)이다.** 현재 자원 풀은 하나(TerraformWorker
  큐, cap=N)이고 그것을 소비하는 kind는 TERRAFORM_JOB뿐이다. 다중 풀(named resource pool)이나
  kind별 slot 분배는 두 번째 한도 backend가 실재할 때 additive로 확장하며 지금은 만들지 않는다
  (YAGNI).
- **N-cap은 best-effort soft target이다 (불변식 아님) — 무엇에 대해 soft인가.** terraform run 요청은
  사실상 잘 설계된 TerraformWorker 큐로의 enqueue이고 실질 backpressure는 그 큐가 한다. N·K는
  **최악의 상한**이고, 단일 리더가 happy path에서 admission(COUNT→admit)을 직렬화해 이를 지킨다.
  soft target이 깨지는 **유일한 실질 경로는 leader failover split-brain(≤30초)** 동안 두 pod가 동시에
  COUNT→admit하는 일시 이중 admit이다(서로 다른 task라 행 CAS가 못 막는다 — read-then-act, 3.2).
  이 경우에도 초과분은 **N·K 언저리로 bound**되며 폭주가 아니다 — "무분별 방지"라는 목표는 일시
  초과에도 충족된다.
- **그래서 admission을 CAS화하지 않는다.** "N·K를 절대 안 넘김"을 강제하려고 admission에 단일 행
  CAS를 들이면 dual-write가 생겨 오히려 복잡도가 높아진다. 목표가 "대체로 N·K 이하(무분별 방지)"
  이므로 현재 COUNT→admit soft target으로 충분하다. (3.2 "상태기계 정확성은 leader 무관"과 무모순 —
  N-cap은 정확성 불변식이 아니라 용량 목표치다.)
- **세 누수 경로 중 일시 초과 요인은 split-brain뿐.** 고아 job은 N·K 정의에 **이미 포함**되고(아래
  재dispatch 헤드룸), 수동 IM job은 **통제 범위 밖**(아래 통제 범위)이라 N·K bound의 대상이 아니다.
  셋 다 폭주가 아니라 N·K 언저리로 bound된다.
- N은 런타임 설정(Part II; 초안 N=3에서 상향), 변경은 이벤트로 감사.
- execution timeout으로 slot이 해제된 task의 기저 job(고아 job)은 계속 돌 수 있어 실효 동시성이
  일시 N을 넘을 수 있다 — 이는 아래 **재dispatch 헤드룸**의 N·K bound에 이미 포함된다(별도 감수 아님).
- **통제 범위 = BFF 발 job.** enforcement는 BFF에만(사용자 결정). N·K bound의 대상은 BFF가 만드는
  job이고, Infra Manager에서 사람이 직접 띄운 **수동 job은 애초에 통제 대상이 아니다** — BFF는 자기가
  만드는 job을 정확히 센다. 수동 job이 IM 총 부하에 더해지는 것은 한계(blind spot)가 아니라 범위
  정의다(전체 caller를 합산하는 글로벌 하드캡이 필요하면 아래 분기 참조).
- **글로벌 하드캡(모든 caller 합산)은 N-cap의 목표가 아니다 — 필요해지면 IM 429.** N-cap은 BFF 발
  job만 bound하면 목표상 충분하다. 모든 caller를 합산한 하드 글로벌 상한이 *별도로* 필요해지는
  시나리오 — 다른 자동화 caller 등장 또는 수동 실행 빈발 — 가 오면, 그 자리는 모든 caller를 보는
  유일한 계층인 **Infra Manager(429)**다. 그때 IM 429를 2선 방어로 추가하고 BFF는 429를 일시
  requeue로 처리한다. 이는 N-cap의 부족이 아니라 별개 시나리오의 분기다.
- **재dispatch 헤드룸 = N·K bound의 정량화.** worker dedup이 없어 crash 재dispatch가 낳은 고아 job과
  최신 job이 각각 slot을 소비하므로, **재dispatch를 포함한 최악의 동시 실행 job 수는 N·K**다(N = slot
  cap, K = 재dispatch 상한 = max_fail_count, 결정 3.1). 이 최악 상한을 IM 실제 수용량 이하로 두기 위해
  **`N ≤ IM 수용량 / K`(즉 `N·K ≤ 수용량`)로 설정**한다 — 보정 계산이 아니라 "BFF 발 동시 job ≤ N·K"
  라는 목표 자체의 정량화다. K는 IM 스펙에 따라 정하는 런타임 설정(Part II)이며, 고아 job은 멱등
  apply라 "변경 없음"으로 빠르게 terminal에 도달하고 execution timeout(결정 4a)이 점유를 bound하므로
  실제 동시 중복은 보통 1~2개에 그친다 — N·K는 드문 연쇄 crash burst까지 덮는 최악 상한이다.
...
- **per-call deadline은 TaskKind별로 오버라이드한다**(기본값 Part II; 일반화 `timeoutPolicy` 슬롯은
  ...
  200초면 240초). post-check가 이미 전역값(60초)을 벗어나 있으며 같은 패턴의 명시화일 뿐이다.
...
라벨이며, **상태 전이 입력 여부는 행의 kind가 아니라 reconciler가 실행한 단계(poll vs postCheck)가
결정한다.** 그래서 JOB_POLL과 CONDITION_CHECK를 `CHECK`로 통합한다 — reconciler는 완료 판정
(DONE|PENDING|FAILED)만 보고 "핸들 폴링이냐 조건 평가냐"로 분기하지 않으며, 그 구분이 필요하면
`external_handle` 유무로 파생한다. `CHECK`와 `POST_CHECK`는 **UX 목적**으로만 구분을 유지한다
("완료를 판정한 확인" vs "관측 전용"). `if kind == POST_CHECK`로 상태 판정을 회피하는 코드를 만들지
말 것 — 상태 무영향은 *단계가* 보장한다. (FORCE_CHECK kind는 제거 — 강제 확인 개념 자체를 두지
않는다, 개정 4판.)
...
- 일시 장애, BFF crash/재배포, worker outage, 외부 호출 실행 주체 죽음이 자가 회복한다
  (fail-count 재시도, 결정 3.3 불변식, 4d timeout+retry·알림 롤업, 결정 6 관측/상태 분리).
...
- poll/check 호출량이 Infra Manager API 서버를 과부하시키지 않도록 tick당 발사 상한
  `max_external_calls_per_tick`으로 burst를 완화한다 — 정확한 global concurrency 보장은 하지 않되
  catch-up·동시 due 폭주를 tick 경계로 자르고, 정밀 강제는 IM 429→requeue에 위임한다(결정 6 D-T7).
- 재시도 의미론이 "새 run"으로 확정되어 terminal 단순함이 보존되고, 완료분 스킵 확장이 additive로
  열려 있다.
- 실행 단위를 `pipeline.target_source_id`로 고정해(실행 입력 일반화 미도입) 모델을 단순화하고,
  Task를 TERRAFORM_JOB/GENERAL_JOB/CONDITION_CHECK 3종으로 고정해 훅 일반화·`TaskExecutionContext`
  추상을 제거한다 — 새 task = 새 TaskKind 코드 클래스(결정 2; target 묶기는 비채택, O22).
- dispatch 산출을 `attempt.response(jsonb)`로 보존해 TaskKind별 응답 형태(TERRAFORM_JOB {job_id}·
  GENERAL_JOB {handle})를 정형 컬럼 없이 흡수하고 poller가 거기서 handle을 추출한다; 모든 dispatch가
  단수 handle이라 attempt:handle은 1:1이다(그 handle은 terminal까지 N번 폴링되어 attempt:task_check는 1:N).
...
- 관측 로그(task_check)는 poll/check마다 행을 쓰고, 이제 호출 전 선기록까지 더해진다. cadence로
  ...
  "시도 이력" 구분이 요구사항).
- queued vs running 구분이 불가능하므로(O7 해소) worker 장애 감지는 EXECUTION_TIMEOUT에만 의존하며
  감지가 execution timeout 분만큼 둔화된다(~30분+) — 구조적 상수로 감수. 단일 job은 terminal까지
  폴링하며 execution timeout(드문 안전망)에 걸리면 정상 retry되고, systemic 연속 timeout은 알림으로
  롤업된다(circuit breaker 없음 — 결정 4d).
...
O-번호는 본문 cross-reference와 **공유**하며(O10·O19·O20), 해소되면 Part III **Resolved**로 옮긴다.
