...
task_check      id, task_id, checked_at, started_at,
                ...
                --   행 = check 호출 1회(1 call = 1 row); fan-out handle은 호출마다 1행(그 귀결). O24 해소.
...
(**O23 해소**: fan-out = ① "한 호출이 분리 불가한 원자적 N id를 깐다". ②(독립 action)는 fan-out이 아니라
일반 task 분리[모델 무변경]라 별개 — 판별은 task 설계 시점 구조(분리 불가 원자적 → ①, 분리 가능 → ②).
현재 알려진 dispatch는 단수 handle(terraform=요청당 1 job_id)이라 ①은 실재 시 모델 변경 없이 수용.)
...
아니다(1차안 "handle을 task_check로 단일화"는 산출물과 관측을 섞은 오류라 철회). check 행은 **호출 1회당 1행**(fan-out 미완 handle은 handle마다 개별 호출 → handle마다 1행;
O24 해소). attempt↔check 명시 링크용 `attempt_id` 컬럼은 미도입(O26 해소: job_id 고유 발급이라
soft-link 무모호).
...
- **Open questions 분리 (2026-06-14):** 미해결 질문을 별도 트래커 파일
  ...
  유지); ADR 본문엔 포인터만 남겼다.
- **O24·O26 해소 (2026-06-14):** O24(task_check 행 = check 호출 1회, 1 call=1 row; fan-out handle별
  행은 그 귀결)·O26(attempt_id 드롭 — job_id 고유 발급이라 soft-link 무모호) 확정. S25의 "행 단위
...
- **O23 해소 (2026-06-14):** fan-out = ①(한 dispatch가 분리 불가한 원자적 N id) 확정 — attempt
  1:check N + 집계(S21/S23, O24 행 단위와 정합); ②(독립 action)는 fan-out이 아니라 일반 task 분리라
  별개. 미해결 11→10건(O7–O10·O18–O20·O22·O25·O27).
...
| S19 | pipeline.parameters(jsonb) 신설 — 실행 입력을 고정 컬럼 아닌 데이터로; target_source_id는 parameters 키(+조회용 비정규화 복제); 생성 시 박제(append-only); 현재 실행 단위 = target_source_id(1:1), 1:N은 의도 아님 → 결정 1.2/2, Open O22 (2026-06-14) **(target_source_id 비정규화 복제는 O21 해소 행이 supersede — 컬럼 제거, 식 인덱스 대체)** |
...
| S21 | external_handle 복수/fan-out: attempt는 action 단수 유지(배열 컬럼 금지); ② 독립 action은 task 분리(기본), ① 원자적 N handle은 handle별 CHECK 행 + "모든 handle SUCCEEDED" 집계(새 테이블 불요); ①+재dispatch에서만 task_check.attempt_id 발사 시점 기록 → 결정 1.2/2/3.1, O23 (2026-06-14) **(일부 S22가 supersede; attempt_id는 O26 해소로 드롭; O23 = 아래 해소 행)** |
...
| S25 | task_check = 관측 장부; external_handle은 "확인한 id 참조"(저장소 아님); id마다 자기 행 — 행 단위(N행 vs 1행)는 O24, attempt_id는 O26, 동기 check 행은 O25, 완료 id 보존은 O27 (2026-06-14) **(행 단위는 O24, attempt_id는 O26 해소 행이 supersede; O25·O27은 미해결 유지)** |
| O24 | task_check 행 생성 단위 = **check 호출 1회**(1 call = 1 row); fan-out 미완 handle은 handle마다 개별 check 호출이라 handle마다 1행(별도 N행 규칙 아님, "1호출=1행"의 귀결); external_handle 단수(확인한 단일 id 참조); C(D-T7) = PENDING task_check = in-flight 호출 = handle 수준 IM 부하; 배치 check(1요청 N handle)는 범위 밖(additive 박제) — **S25의 "행 단위 미정" 문구 supersede** → 결정 1.2/2/3.1 (2026-06-14) |
...
- **DB migration** — task_check.api_result에 PENDING 추가, task_check.started_at 추가,
  ...
  **task_attempt.external_handle 제거 →
  ...
  (O26 해소 — job_id 고유 발급이라 soft-link로 충분).
...
O-번호는 본문 cross-reference와 **공유**하며(O7–O10·O18–O20·O22·O25·O27), 해소되면 Part III **Resolved**로 옮긴다.
