...
워커 loop는 claim 결과의 **work-kind로 먼저 분기**한다(종단 알림 vs RUNNING 실행) — 종단
행에는 READY task도, 의미 있는 `cancel_requested`도, slot-gate도 없으므로 ADR-021의
...
- **at-least-once 전달.** `notified_at`은 **한 번만 찍히는 상태 마커**(파이프라인당 종단
  ...
  한다**(`pipeline_id` + 종단 종류로 dedupe). 순서: 파이프라인당 알림 상태가 1개라
  파이프라인 내부 순서 문제는 없고, 같은 target의 이전/이후 파이프라인 알림은 `pipeline_id`
  키로 소비자가 구분한다.
...
- **소비자 계약**: (a) 멱등 dedupe 키(`pipeline_id` + 종단 종류)를 충분히 오래 보관,
