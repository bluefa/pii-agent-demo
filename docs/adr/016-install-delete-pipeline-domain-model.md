...
- `terraform_result(id, task_id, attempt_number, job_id, succeeded, result_path, result,
  ...
  (design detail: the Terraform-client / postCheck design, §4.4, in the orchestrator implementation).
...
- [ADR-022](022-terminal-state-notification.md) — terminal-state notification. Rather than the "event outbox" from Costs we accept, it derives notifications from state (a `notified_at` marker), honoring this model's original cut. Adds ADR-022-owned notification metadata columns to `pipeline` (same category as ADR-021's execution columns); domain state is unchanged.
