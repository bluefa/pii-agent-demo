...
stored. Pipeline status is derived from its tasks.
...
`ErrorCode`). The pipeline's recipe (its ordered task list) is a code default per
`(type, provider)`.
...
### 4. Execution model: a single server with one worker per pipeline
...
there is no row lease, no ownership check, and no leader election. Slow calls (a
minutes-long job poll) occupy only their own worker thread; the pool size is the
limit on concurrent calls. (Mechanism detail: PR #509.)

### 5. Safe under at-least-once dispatch

Correctness comes from **idempotency by construction**, not an exactly-once machine.
Every dispatch is idempotent (a duplicate submit is harmless; "already in the desired
state" counts as success), so a crash between "InfraManager started the job" and "we
...
- Absorbs **long (minutes-scale) external calls** without blocking, via the worker pool.
...
- The maximal design's **full per-call audit ledger and event outbox are dropped.**
  The audit trail moves to the dedicated server's **logs and metrics**, plus a
  `last_requested_at` marker on the task to tell "attempted" from "not attempted."
  (This is the V1 answer to the old audit requirement.)

## What this gives the product

- A pipeline runs to completion with no browser open.
- Every target's run history is visible.
- What happened on each task is auditable — through the server's logs/metrics plus the
  attempt marker.
