...
> 필드 정책, 인가 거부 추적 반영. **선행 문서**: 전략 = `observability-strategy-overview.md` ·
...
| 1-2 | 이벤트 스키마·DB 필드 확정 | 공통: `ts`·`source`(browser/server)·`severity`·`surface`·`pageTemplate`·`pathTemplate`·`clientAction`·`method`·`status`·`code`·`durationMs`·`requestId`·`userId`·`role`·`targetSourceId?`. **`errorMessage` 컬럼 없음.** `errorName`·`fingerprint`(새니타이즈 스택 해시) 포함 여부는 열린 결정 — 미포함 시 code 없는 에러는 DB에서 구분 불가함을 명시하고 결정 | 스키마 문서 |
...
| 4-2 | 관제 화면 | 신규 `app/admin/observability/page.tsx` + `_components/` | §3 기능 A1~A3 (최근 에러·추이·API top-N). 기존 admin 패턴(theme.ts 토큰·한국어 UI) 준수 | 목업 모드 렌더 + 실데이터 |
| 4-3 | 드릴다운 | 4-2 하위 | §3 기능 B4~B5 (에러 상세·requestId 추적·화면 이동 링크) | 에러 1건에서 requestId 체인 추적 성공 |
| 4-4 | 엔티티 축 | 4-2 하위 + `app/admin/.../targets/` 탭 | §3 기능 C6~C7 (고객별 타임라인·타깃소스별 이력). C7의 "현재 도메인 상태 병합"은 열린 항목 — MVP 포함 여부 결정 후 | 타깃 1건의 행동 흐름 시간순 재구성 |
| 4-5 | 보안·쓰기 감사 | 4-2 하위 | §3 기능 D8~D9 (403 시도 목록·CTA 실패). D9는 전용 뷰 vs 필터 축 — 열린 항목 | 403 목록·쓰기 실패 필터 동작 |

**완료 기준**: 운영자가 Admin에서 "최근 에러 → 상세 → 해당 고객/타깃 이력 → 403 시도"까지 도달.
**규모**: 중~대. **전제**: Phase 1·2 (403·userId 축은 3).

### Phase 5 — page_view 이벤트 (방문 추적)

| # | 작업 | 대상 파일 | 상세 | 검증 기준 |
|---|---|---|---|---|
| 5-1 | page_view 이벤트 | `app/components/ObservabilityInit.tsx` + 수신 라우트 확장 | 라우트 전환 감지(`usePathname`) → `{event:"page_view", pageTemplate, surface}` 전송. 사람 묶기는 userId(서버가 심음) — **sessionId 없음** | 전환 시 1건, 새로고침 중복 없음 |
...
### In-app Admin (확정 — Phase 4)

| # | 그룹 | 기능 | 데이터 | 출처 |
|---|---|---|---|---|
| A1 | 관제 | **최근 에러 목록** (50건) | 시각·source(browser/server)·surface·pageTemplate·code·status·requestId | 원안 |
| A2 | 관제 | **에러 추이** (24h/7d) | browser/server 분리, surface별 스택 | 원안 |
| A3 | 관제 | **API 사용량 top-N** | clientAction별 호출 수·p95·실패율, 폴링(`getInstallationStatus` 등) 분리 표시 | 원안 |
| B4 | 드릴다운 | **에러 상세** | 행 확장: breadcrumbs(직전 API 10건)·requestId 체인(브라우저→FE→BFF 대조)·**"에러 난 화면으로 이동" 링크**(로그의 실경로) | 원안+추가 |
| B5 | 드릴다운 | **requestId 검색** | 단일 요청의 전체 흐름 — "FE 에러인데 BFF는 200" 판별 | 원안 |
| C6 | 엔티티 | **고객(userId)별 활동 타임라인** | "이 고객이 최근 뭘 했나" — 방문·API·에러 시간순 | 추가 (인증 전제로 당겨짐) |
| C7 | 엔티티 | **타깃소스별 활동 이력 + 현재 상태** | targetSourceId 필터 이력 + 도메인 API로 현재 process status 병합 — 로그는 이력, 도메인은 현재 | 원안+추가 |
| D8 | 보안 | **인가 거부(403) 시도 목록** | userId·role·시도 경로·시각 — "권한 없는 곳에 접근 시도한 케이스" | 추가 |
| D9 | 감사 | **CTA(쓰기) 실패 추적** | method(POST/PUT/DELETE)+clientAction+status 필터 — "쓰기가 진짜 됐나" requestId 대조 | 추가 (전용 뷰 vs 필터: 열린 항목) |
| E10 | 운영 | **접근 통제·조회 창** | 운영자 role만, 기본 24h 창(확장 가능), 보존 정책 표시. 자동 갱신 여부는 열린 항목(관제탑 vs 조사실) | 원안 개정 |
