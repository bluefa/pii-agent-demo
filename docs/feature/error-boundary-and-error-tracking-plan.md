...
>
> ⚠️ **2026-07-12 아키텍처 수정**: FE 서버 아웃바운드가 BFF·OAuth·인가 서버로 제한됨이 확인되어 트래커 배치·터널·알림 구조가 바뀌었다.
> **`observability-via-bff.md`가 상위 아키텍처를 대체**한다 (그 문서 §6에 이 문서와의 차이 매핑). Part A(바운더리)와 B-2/B-3 코드는 그대로 유효.
