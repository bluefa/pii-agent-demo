...
- **업로드는 내용 기준으로 idempotent하다.** 바이트가 완전히 같은 파일을 다시 올리면 새 파일을 만들지 않고 **처음의 `url`을 그대로 돌려준다**(sha256 등 내용 해시로 판정). 이 규칙이 없으면 다국어 게시글의 가장 자연스러운 작성 흐름 — 한국어 본문에 스크린샷을 넣고, English 탭에서 같은 스크린샷을 다시 넣는 흐름 — 이 그림 한 장으로 10칸 중 2칸과 두 배의 용량을 쓴다. 5MB짜리 스크린샷이면 그림 하나로 10MB 상한에 닿는다.
- 그 결과 저장소의 한 파일을 여러 게시글이 참조할 수 있다. 고아 파일 정리 정책(§5 범위 밖, D16)은 "본문에서 빠지면 삭제"가 아니라 **참조 수가 0이 될 때 삭제**여야 한다.
...
- 프론트엔드에는 위 표에 없는 라우트가 하나 더 있다 — `GET /pass/api/v1/admin/posts/images/{imageId}`. **mock 전용**이며 BFF에 대응 엔드포인트가 없다. 실제 배포에서는 업로드된 파일이 저장소에 있고 `img.src`가 그쪽을 직접 가리키므로, 이 라우트는 `USE_MOCK_DATA=false`에서 404를 반환한다. mock이 업로드를 base64로 본문에 인라인하지 않고도 prefix 검사를 통과하는 URL을 돌려주기 위해 존재한다.
