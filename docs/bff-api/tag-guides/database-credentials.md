...
Database credential 조회와 Target Source resource credential 갱신을 담당하는 BFF API Tag다.
...
openapi: 3.0.1
info:
  title: BFF API - Database Credentials
  version: v0
servers:
...
- name: Database Credentials
  description: Database credential APIs
paths:
  /install/v1/target-sources/{targetSourceId}/resources/credential:
    put:
      tags:
      - Database Credentials
      summary: Update resource credential for target source
      operationId: updateResourceCredential
      parameters:
      - name: targetSourceId
        in: path
        required: true
        ...
        '503':
          ...
          content:
            '*/*':
              schema:
                $ref: '#/components/schemas/ErrorMessage'
  /install/v1/target-sources/{targetSourceId}/secrets:
    get:
      tags:
      - Database Credentials
      operationId: getTargetSourceSecrets
      parameters:
      - name: targetSourceId
        in: path
        required: true
...
| Method | Path | 설명 | 상태 |
| --- | --- | --- | --- |
| PUT | `/install/v1/target-sources/{targetSourceId}/resources/credential` | resource credential 갱신 | Draft |
| GET | `/install/v1/target-sources/{targetSourceId}/secrets` | secret 조회 | Draft |

## 4. Response 설명

| Response 항목 | 설명 | 관련 기준 |
| --- | --- | --- |
| TBD | credential/secret response의 노출 기준을 작성 | 보안 정책 |

## 5. 주요 동작 규칙

- credential 갱신 가능 조건과 실패 기준을 설명한다.
- secret 조회 시 masking, 권한, 보안 주의사항을 설명한다.
