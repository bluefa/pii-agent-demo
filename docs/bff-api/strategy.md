...
- BFF API의 timestamp 계열 필드는 JSON 응답에서 `string`으로 정의한다.
- 에러 응답의 `timestamp`는 UTC 기준 ISO-8601 문자열을 사용한다.
  - 예: `2026-04-29T02:27:09.123Z`
- timezone 정보가 없는 `LocalDateTime` 형태의 문자열은 신규/변경 계약에서 사용하지 않는다.
- 프론트엔드는 API DTO에서 timestamp를 `Date`로 선언하지 않는다. 화면 표시가 필요한 경우에만 클라이언트 로컬 timezone 기준으로 변환한다.
- Tag 가이드의 `ErrorMessage.timestamp` schema에는 `type: string`, `format: date-time`, UTC ISO-8601 설명, `...Z` 예시를 함께 기록한다.

#### Error response timestamp 규칙

- BFF upstream이 error response에 `timestamp`를 제공하면 FE/BFF proxy 계층은 문자열 값을 그대로 보존한다.
- FE에서 새로 생성하는 `ProblemDetails`도 `new Date().toISOString()` 형식의 UTC ISO-8601 문자열을 사용한다.
- 에러 처리 계층은 timestamp를 정렬/표시 목적으로 즉시 파싱하지 않는다. 파싱은 필요한 호출부 또는 UI 표시 계층에서만 수행한다.
