...
| `상태` | 모든 문서 | enum은 §1.2의 종류별 값을 따른다 |
| `API Tag` | tag-guides/* | Tag 표시 이름 (Swagger의 `tags[].name`) |
| `담당` | tag-guides/*, discussions/* | 책임 팀/사람 (TBD 허용) |
| `작성일`, `마지막 수정일` | 모든 문서 | YYYY-MM-DD |
| `Tags` | discussions/* | 영향받는 Tag 슬러그 (다중일 경우 콤마 구분, 예약어 `error-codes`/`multiple` 허용) |
| `Change type` | discussions/* | Added / Changed / Deprecated / Removed / Fixed |
| `Direction` | discussions/* | BE-first / FE-first / Joint (§4.5) |
| `Related PR` | tag-guides/*, discussions/* | URL |
...
- 모든 문서에 `> 상태:` 가 있고 §1.2의 종류별 enum 범위 안인지
- tag-guides/* 가 `Confluence`, `API Tag`, `담당`, `마지막 수정일`을 가지는지
- discussions/* 가 `Confluence Title`, `Tags`, `Change type`, `Direction`을 가지는지
- `Released` 상태의 tag-guide는 `관련 PR` URL이 채워져 있는지
- 거버넌스 문서 (`README.md`, `strategy.md`, `management-plan.md`)는 `Confluence`와 `상태`(거버넌스 enum)를 가지는지
