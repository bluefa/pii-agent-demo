...
**확정: 게시글당 이미지 10개, 총 10MB.** 두 상한 모두 `contents.ko`와 `contents.en`을 합쳐 계산하고, 같은 URL이 여러 번 쓰이면 한 번만 센다 — 저장소에 파일이 하나이기 때문이다.

검사 시점은 업로드가 아니라 **저장**이다. 업로드는 파일 단위 요청이라 그 시점에는 어느 게시글에 들어갈지 알 수 없다. 초과 시 `POST_IMAGE_LIMIT_EXCEEDED` / `POST_SIZE_LIMIT_EXCEEDED`로 거부한다.

파일 1개 5MB(D17)와 게시글 10MB는 서로 다른 층이다. 5MB 파일 2개면 개수는 여유가 있어도 용량에서 걸린다.
...
- **error code 영향**: 아래 4건이 **신규 후보**다. 이 문서가 `Accepted`가 되기 전에는 `catalogs/error-codes.md`에 행을 추가하지 않는다(관리 계획 §4.4.1 — 트리거는 "backend가 추가하거나 추가 예정"이며, 현재는 FE 제안 단계).
  ...
  | `POST_IMAGE_LIMIT_EXCEEDED` | 400 | 게시글 저장 시 이미지 10개 초과 |
  | `POST_SIZE_LIMIT_EXCEEDED` | 400 | 게시글 저장 시 총 10MB 초과 |
...
| D16 | 고아 이미지 정리 | 미정. 단 D20 때문에 **참조 수 0**이 삭제 조건이다 — "본문에서 빠지면 삭제"는 다른 게시글의 이미지를 지운다 | 예 — 업로드 후 미저장분, 게시글에서 제거된 이미지의 수명 정책 |
| D17 | 이미지 크기 상한 | 파일 1개당 5MB · 요청 1회 1개. 조각(resumable) 업로드 배제 | 아니오 — 확정. 상한을 올리려면 이 결정을 다시 연다 |
| D18 | 게시글 전체 무게 | **이미지 10개 · 총 10MB.** ko/en 합산, 중복 URL은 1회만 계산 | 아니오 — 확정 |
| D19 | 목록 응답의 본문 포함 여부 | **미포함.** 아코디언 펼침 시 단건 조회 | 아니오 — 확정. §2.3의 초안 결정을 뒤집은 것 |
| D20 | 동일 파일 재업로드 | **내용 해시로 판정해 같은 `url`을 돌려준다(idempotent).** 없으면 ko/en에 같은 그림을 넣는 표준 흐름이 D18 예산을 두 배로 쓴다 | 예 — BE가 해시 인덱스를 갖는지 확인 필요. FE mock은 sha256으로 구현 |
