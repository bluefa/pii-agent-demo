...
파일 1개 상한은 게시글 전체를 제한하지 않는다. 5MB 이미지 20장을 넣은 게시글이 만들어질 수 있고, 목록 API가 본문을 통째로 내려주므로 그 게시글이 있는 카드는 이미지 URL 20개를 안고 열린다(바이트는 lazy 로드라 즉시 내려오지 않지만, 펼치면 100MB를 받는다).
