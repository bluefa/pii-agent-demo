...
Options:
  ...
  --body       PR body (required when creating a PR)
EOF
}
...
if [[ -z "${pr_number}" || "${pr_number}" == "null" ]]; then
  ...
  if [[ -z "${BODY}" ]]; then
    echo "[pr-flow] --body is required for PR creation." >&2
    echo "[pr-flow] Write body manually using /pr-description skill and retry." >&2
    exit 1
  fi
