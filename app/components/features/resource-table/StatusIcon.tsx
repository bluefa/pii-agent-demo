...
interface StatusIconProps {
  type: 'selected' | 'new' | 'disconnected' | 'configured' | 'needsConfig';
}
...
const configs = {
  ...
  new: {
    icon: (
      <svg className={cn('w-5 h-5', statusColors.info.text)} viewBox="0 0 20 20" fill="currentColor">
    ...
    tooltip: '신규 발견된 리소스',
    bgColor: statusColors.info.bgLight,
  },
  disconnected: {
    icon: (
...
export const StatusIcon = ({ type }: StatusIconProps) => {
  ...
  return (
    <div className="group relative">
      <div className={cn('p-1 rounded', config.bgColor, 'cursor-help')}>
        {config.icon}
      </div>
      {/* Tooltip */}
