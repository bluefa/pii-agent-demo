...
describe('resolveStepSlot — AWS', () => {
  it('maps AWS to auto variant slot keys for all 7 steps', () => {
...
describe('resolveStepSlot — out of range', () => {
  it('returns null for step 0', () => {
    expect(resolveStepSlot('AWS', 0 as ProcessStatus)).toBeNull();
  });

  it('returns null for step 8', () => {
    expect(resolveStepSlot('AWS', 8 as ProcessStatus)).toBeNull();
