...
export const TcSummaryCard = ({
  ...
  // `state === 'success'` 만으로는 부족하다. success 는 세 변형을 함께 든다(전부 성공·일부
  // 미확인·보고 0건) 인데, 승인 게이트는 `buckets.ok === total` 을 요구하므로 뒤의 둘에서는
  // 버튼이 회색으로 잠긴다. 그때도 "승인을 요청하면 넘어가요" 라고 적으면 카드가 닫힌 문을
  // 가리키게 된다 — 특히 ok===0 변형은 제목이 "확인된 리소스가 없어요" 라고 말한 직후다.
