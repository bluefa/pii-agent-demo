...
const COLUMNS = ['요청유형', '서비스코드', '서비스명', 'Provider', 'Cloud 정보', '요청시간', '처리시간', '결과', 'Action'] as const;

export const CompletedTasksTable = ({ items, loading, onDetail }: CompletedTasksTableProps) => {
  if (loading) {
    return (
  ...
  if (items.length === 0) {
    return (
      <div className="p-12 text-center">
        ...
        <p className={textColors.tertiary}>완료된 요청이 없습니다</p>
      </div>
    );
  }
            ...
            <tr key={item.approvalRequestId} className={tableStyles.row}>
              <td className={tableStyles.cell}>
                <RequestTypeBadge
                  ...
                  statusSuffix={item.status === 'APPROVED' ? '승인' : '반려'}
                  statusVariant={item.status === 'APPROVED' ? 'approved' : 'rejected'}
                />
              </td>
              ...
              <td className={tableStyles.cell}>
                {item.status === 'APPROVED' ? (
                  <Badge variant="success" size="sm">승인</Badge>
                ) : (
                  <div className="flex flex-col gap-1">
                    <Badge variant="error" size="sm">반려</Badge>
                    {item.rejectionReason && (
                      <span
                        className={cn('text-xs truncate max-w-[120px]', textColors.tertiary)}
