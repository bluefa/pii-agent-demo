...
export const ApprovalDetailModal = ({
  ...
  const resourceItems = approvalRequest.resources ?? [];
  const includedCountFromSnapshot = resourceItems.filter((resource) => resource.selected).length;
  const totalCount = approvalRequest.resource_total_count ?? resourceItems.length;
  ...
  const hasSnapshotSummary = resourceItems.length > 0;
