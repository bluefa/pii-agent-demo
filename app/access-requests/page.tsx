 ...
 *
 * 표는 흰 종이 위에 올라간다. 행에 면이 없는 표는 구분선이 페이지 캔버스
 * (`serviceSidebarStyles.canvas`) 위에 바로 그어지는데 `--pl-border` 대 그 바닥은
 * 1.132:1 — 있어도 안 보이는 선이다. 흰 바닥에서 같은 선이 1.239:1 이고, 그래서 손대는
 * 것은 선이 아니라 바닥이다(브라우저 실측).
 *
  ...
  return (
    <div>
      ...
      {/* 세 탭 모두 흰 종이 위의 표다(오너 지시 2026-08-17). 행에 면이 없는 표는 구분선이
          캔버스 위에 바로 그어지고, `--pl-border` 대 페이지 캔버스는 1.132:1 — 선이
          있어도 안 보인다. 흰 바닥에서 같은 선이 1.239:1 이다. 머리 줄은 없다
          (`head={null}`) — 제목은 위의 탭이 이미 쓰고 있어서 대신 그릴 것조차 없다. */}
      {tab !== 'mine' ? (
        ...
        /* 기록. 설명 줄이 없는 건 반려 안내를 헤더 판정이 이미 말하기 때문이다.
           `bare` 를 처음 뗀 목록이다(오너 지시 2026-08-17) — 표는 흰 종이 위에 올라간다.
           그때 서비스 두 탭은 아직 카드 더미라 캔버스가 카드 사이를 갈라 줬지만, 표의
           행에는 면이 없어서 구분선이 캔버스 위에 바로 그어졌고 `--pl-border` 대 그
           바닥은 1.132:1 — 선이 있어도 안 보인다. 흰 바닥에서는 같은 선이 1.239:1 이고,
           이게 Azure 콘솔을 비롯한 모든 표가 읽히는 이유다: 선을 진하게 하는 게 아니라
           종이를 깐다. 서비스 두 탭도 표가 되면서 같은 길을 갔다 — 이제 셋 다 종이 위다. */
        <PagedCard
          className="mt-4"
          head={null}
          title="내 요청 내역"
