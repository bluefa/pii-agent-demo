...
export const getIdcSourceIpRecommendation = async (ipType: string): Promise<SourceIpRecommendation> => {
  const res = await fetchInfra(`${BASE}/source-ip-recommendation?ipType=${ipType}`);
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.error?.message || 'Source IP 추천 조회에 실패했습니다.');
  }
  return parseInfraCamelJson<SourceIpRecommendation>(res);
};

export const getIdcInstallationStatus = async (targetSourceId: number): Promise<IdcInstallationStatus> => {
  const res = await fetchInfra(`${BASE}/target-sources/${targetSourceId}/installation-status`);
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.error?.message || 'IDC 설치 상태 조회에 실패했습니다.');
  }
  return parseInfraCamelJson<IdcInstallationStatus>(res);
};

export const checkIdcInstallation = async (targetSourceId: number): Promise<IdcInstallationStatus> => {
  const res = await fetchInfra(`${BASE}/target-sources/${targetSourceId}/check-installation`, {
    method: 'POST',
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
...
export const confirmIdcFirewall = async (targetSourceId: number): Promise<ConfirmFirewallResponse> => {
  const res = await fetchInfra(`${BASE}/target-sources/${targetSourceId}/confirm-firewall`, {
    method: 'POST',
  });
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
  ...
  return parseInfraCamelJson<ConfirmFirewallResponse>(res);
};

export const getIdcResources = async (targetSourceId: number): Promise<{ resources: IdcResourceInput[] }> => {
  const res = await fetchInfra(`${BASE}/target-sources/${targetSourceId}/resources`);
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
...
export const confirmIdcTargets = async (
  ...
  const res = await fetchInfra(`${BASE}/target-sources/${targetSourceId}/confirm-targets`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ resources }),
  });
  ...
  return parseInfraCamelJson<{ confirmed: boolean; confirmedAt: string; project: unknown }>(res);
};
