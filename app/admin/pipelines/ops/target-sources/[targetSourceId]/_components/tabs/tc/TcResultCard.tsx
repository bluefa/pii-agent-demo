...
/**
 * Test Connection 결과 card (design/pipeline/ops-target-source-tabs.html `tabTc()`) —
 * the Task Queue P5 결과 view condensed: status head + 요약, the per-resource 결과
 * table with a read-only 논리 DB drill-down, and the 재실행 요청 / 연동 승인 actions.
 *
...
export interface TcResultCardProps {
  targetSourceId: number;
  detail: RawTargetSourceDetail;
  /** Bumped by the tab to refetch (a sibling card's save invalidates the counts). */
  reloadKey: number;
  onReload: () => void;
}
...
export function TcResultCard({
  ...
  reloadKey,
  onReload,
}: TcResultCardProps): ReactElement {
  ...
  // Cache key carries the load generation, so a reload invalidates the per-resource
  // 논리 DB lists without a separate clearing step.
  const ldbKey = useCallback(
    (resourceId: string) => `${loadKey}:${resourceId}`,
    [loadKey],
  );
