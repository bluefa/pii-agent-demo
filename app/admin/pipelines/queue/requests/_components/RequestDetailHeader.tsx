...
export interface RequestDetailHeaderProps {
  /** null(또는 빈 문자열) = the P2 header row carried no name — the h1 falls back
   *  to #id alone, so the id never prints twice. */
    ...
    <div className={HEAD_FRAME}>
      <div>
        ...
        <div className="flex flex-wrap items-baseline gap-2">
          {/* 판정은 truthiness — loose codegen 이라 '' 도 이름 없음이다(브레드크럼과 같은 규칙). */}
 ...
 * The disabled CTA assumes the common case (대기 요청); on a decided request it
 * disappears when the facts land — the skeleton cannot know the verdict ahead.
 */
export function RequestDetailHeaderSkeleton(): ReactElement {
  return (
    <div className={HEAD_FRAME} role="status" aria-busy="true" aria-label="요청 정보를 불러오는 중">
      <div>
