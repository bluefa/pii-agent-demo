 ...
 * TASK_DESCRIPTION_TOO_LONG) surface via the summary-step error line — the
 * builder prevents all three in normal use (provider-scoped catalog, counter
 * gating), so they remain defensive. All user feedback goes through the
 * caller's PlToast (`showToast`).
 */
...
export function PreviewModal({
  ...
  const [chosen, setChosen] = useState<BuilderTask[]>([]);
            ...
            {chosen.map((t, i) => (
              <Fragment key={t.entry.name}>
                ...
                <span className={pv.node}>
                  ...
                  <span className={pv.nodeName}>{t.entry.display_name}</span>
                  {t.description.trim() ? (
                    <span className={bd.nodeDesc} title={t.description}>
                      {t.description}
                    </span>
                  ) : null}
                </span>
