...
import { GET } from '@/app/api/integration/v1/target-sources/[targetSourceId]/approved-integration/route';
import { client } from '@/lib/api-client';

const mockedGetApprovedIntegration = vi.mocked(client.confirm.getApprovedIntegration);

describe('GET /api/integration/v1/target-sources/[targetSourceId]/approved-integration', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('approved integration이 없으면 snapshot-specific 404 problem을 반환한다', async () => {
    ...
    const response = await GET(
      new Request('http://localhost/api/integration/v1/target-sources/1005/approved-integration'),
