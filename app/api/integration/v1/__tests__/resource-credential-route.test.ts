...
import { PUT } from '@/app/api/integration/v1/target-sources/[targetSourceId]/resources/credential/route';
import { client } from '@/lib/api-client';

const mockedUpdateResourceCredential = vi.mocked(client.confirm.updateResourceCredential);

describe('PUT /api/integration/v1/target-sources/[targetSourceId]/resources/credential', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('Issue #222 PUT 계약으로 credential update를 프록시한다', async () => {
    ...
    const response = await PUT(
      new Request('http://localhost/api/integration/v1/target-sources/1003/resources/credential', {
        method: 'PUT',
        body: JSON.stringify({
          resourceId: 'res-1',
          credentialId: 'cred-1',
  ...
  it('유효하지 않은 targetSourceId면 problem response를 반환한다', async () => {
    const response = await PUT(
      new Request('http://localhost/api/integration/v1/target-sources/abc/resources/credential', {
        method: 'PUT',
        body: JSON.stringify({
          resourceId: 'res-1',
          credentialId: 'cred-1',
