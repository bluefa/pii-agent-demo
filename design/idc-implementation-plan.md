...
### 4.1 Demo vs Cutover 데이터 출처 (마이그레이션 핵심)

본 구현은 **mock-first 데모**라 행 데이터를 대부분 `GET /idc/.../resources`(mock이 스냅샷·health·connection을
동봉)에서 읽는다. 실 백엔드 cutover 시 각 Step은 아래 **문서상 정본 출처**로 전환한다. 전환은 도메인
모델(`IdcResourceView`)을 그대로 두고 *데이터 출처 함수만* 바꾸므로 UI/표/배지는 무변경(§5 격리 규칙).

| Step | 데모 출처 (현재 구현) | Cutover 정본 출처 | 전환 작업 |
|------|----------------------|------------------|----------|
| 1 로드 | `getIdcResources` | 동일 (`GET /resources` → `IdcResource[]`) | 무변경 |
| 1 제출 | `updateIdcResources` 후 `refreshProject` (전이 TODO) | **save-then-submit**: `updateIdcResources` 응답(`IdcResource[]`, **id 포함**)으로 `resource_inputs` 구성 → `createApprovalRequest`(plural `/approval-requests`) → WAITING_APPROVAL. ⚠ id 없는 신규 행은 승인 normalizer가 누락하므로 **반드시 저장 응답의 id 사용** | submit 핸들러 wiring (현재 TODO) |
| 2·3 스냅샷 | `getIdcResources` | **[공용]** `getApprovalRequestLatest` / `getApprovedIntegration` | 공용 승인 DTO → `IdcResourceView` 매퍼를 `app/lib/api/idc.ts`에 추가 후 Step 2/3 교체 |
| 4 설치 | `getIdcInstallationStatus` + `getIdcResources` 병합 | 동일 (이미 `installation-status.resources` 병합) | 무변경 — **cutover-safe** |
| 5 연결 테스트 | 로컬 `setTimeout` 시뮬 | **[공용]** `testConnection` + `getTestConnectionLatest` 폴링 | `runTest` 교체 (§Step5·6) |
| 6 연결 확인 | `getIdcResources` | **[공용]** 승인 스냅샷 + 테스트 결과 | 2·3과 동일 매퍼 재사용 |
| 7 health | `getIdcResources` (매퍼 기본값 `HEALTHY`) | **[공용]** `getProject` / confirmed-integration health | health를 프로젝트/confirmed에서 읽도록 교체. ⚠ 매퍼 기본 `HEALTHY`는 **데모 전용** — 실 데이터 누락을 Healthy로 오인 금지 |

> 모든 전환은 **`app/lib/api/idc.ts`(wire↔domain 매퍼 경계) + 각 Step의 fetch 호출**만 바꾼다.
> `IdcResourceView`·표·배지 UI는 불변. 이것이 §5 격리 규칙이 보장하는 마이그레이션 표면이다.
