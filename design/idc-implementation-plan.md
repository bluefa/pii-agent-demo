...
> **남은 백엔드 의존 항목(cutover 시 §5.2 절차):** G1(승인 전이 분리) · G3(임시저장 `exclusion_reason` 확정) ·
> Step 5 연결 테스트 실 API 연결(§4 참조, 현재 클라이언트 시뮬레이션). Step 4 Source IP·방화벽은
> `installation-status.resources` 병합으로 이미 cutover-safe.
