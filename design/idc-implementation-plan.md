...
|------------|-----|--------------|------|------|------|
| 진입 시 목록 로드 | **[IDC]** getIdcResources | `GET /idc/target-sources/{id}/resources` | — | `{ resources: IdcResourceInput[] }` | 임시 저장된 수동입력 목록 |
| 연동 대상 추가/수정 → 저장 | **[IDC]** updateIdcResources | `PUT /idc/target-sources/{id}/resources` | `{ resources: IdcResourceInput[] }` | `{ resources: IdcResourceInput[] }` | 전체 목록 덮어쓰기(임시 저장). 프로젝트 리소스 미반영 |
...
| 기존 연동 요청 불러오기 | **[공용]** getApprovalHistory **또는** **[IDC]** prev-request | `GET /target-sources/{id}/approval-history` (잠정) | page | 직전 요청 스냅샷 | §6 G4 — 전용 endpoint 없음. mock은 고정 IDC_PREV_REQUEST |
| "연동 대상 승인 요청" 제출 | **[공용]** createApprovalRequest | `POST /target-sources/{id}/approval-request` | `{ resource_inputs:[...selected/excluded] }` | 승인요청 생성 | cloud와 동일. → WAITING_APPROVAL |
...
HTML `runIdcConnTest`: 셀을 Testing… → 1.8s 후 success. mock testConnection이 동일 흐름 제공.
...
- `docs/swagger/idc.yaml` 을 본 구현에 맞춰 갱신(§6 갭 반영). **swagger = 정본**(Contract-First, route.ts
  파싱 금지). 타입은 yaml에서 도출.
...
| # | 갭 | idc.yaml | HTML/요구사항 (진실) | 결정 |
|---|-----|----------|---------------------|------|
| G1 | 승인 플로우 | `confirm-targets` → INSTALLING 직행 | Step1→2(승인)→3→4 | **HTML 채택.** 승인은 공용 confirm 플로우. `confirm-targets`는 전이 부수효과 없이 리소스 확정 저장으로만 mock. yaml 갱신 필요(transition 문구 제거 또는 분리) |
| G2 | IP 최대 개수 | `ips.maxItems: 3` | `IDC_MAX_IPS = 6` (결정 #52) | **6 채택.** yaml `maxItems: 6` 로 갱신 |
| G3 | 제외 사유 필드 | `IdcResourceInput`에 없음 | 제외 사유 필수(결정 #27·28) | 도메인 모델에 `excluded`/`exclusionReason`. wire는 공용 `resource_inputs`의 `exclusion_reason` 경유(승인 요청). yaml에 IDC 임시저장용 `exclusion_reason?` 추가 권장 |
| G4 | 기존 요청 불러오기 | 전용 endpoint 없음 | "기존 연동 요청 정보 불러오기"(결정 #10·44) | 공용 `approval-history` 재사용 가정. mock은 고정 `IDC_PREV_REQUEST`(7건). 전용 endpoint 필요 시 추가 |
| G5 | DB Type enum | `ORACLE/MYSQL/POSTGRESQL/MSSQL` (4) | MySQL/PostgreSQL/Oracle/MSSQL/**MariaDB/MongoDB/Redis** (7, 결정 #54) | **7 채택.** yaml enum 7종 확장. 기본 포트 매핑 동반(§7) |
| G6 | Source IP·방화벽 입자 | `firewall_opened: boolean`(글로벌), Source IP 행별 필드 없음 | 행별 Source IP(1~2개)·행별 방화벽 상태 필요(Step2+, Step4 모달) | **도메인 모델은 per-resource**(`sourceIps[]`, `firewallOpen`). mock이 per-resource 합성. 실 계약은 installation-status에 `resources:[{resource_id, source_ips[], firewall_open}]` 추가 권장. 글로벌 `firewall_opened`는 roll-up(전 행 open) |
| G7 | Source IP 출처 | recommendation(글로벌) 만 존재 | 행별 할당값(승인 후) | admin `nlb_index` 할당의 SIT 표현(admin-page-requirements §4.3.5). mock 고정값(172.16.0.11/.12). 실 계약은 confirmed/installation에 노출 |

> 위 갭은 모두 "API 미확정"의 자연스러운 결과다. **본 구현은 HTML을 따르고**, 각 갭의 yaml 갱신
> 권고를 명시했다. 백엔드 확정 시 §5.2 절차로 정렬한다.
