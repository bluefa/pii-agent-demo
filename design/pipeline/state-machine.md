...
| CANCELLING | CANCELLED | tick 파생 ①(최우선): **모든 task가 terminal 도달** — 미dispatch(BLOCKED/READY)·CONDITION_CHECK·job_id 미영속 DISPATCHING은 즉시 CANCELLED, in-flight RUNNING TF는 drain으로 자연 terminal(아래 취소 표) | 1.1, 4c |
...
| WAITING_EXTERNAL | FAILED | tick: check api_result=ERROR(**비-backpressure**)로 fail_count++ == **K** (NOT_MET·429/503 backpressure는 미가산 — "아직"·"나중에"는 실패 아님) | CONDITION_CHECK |
...
| From | → To | 트리거 / Guard | kind |
|---|---|---|---|
| DISPATCHING | DISPATCHING (in-place) | dispatch 복구 timeout(response 없음) 또는 dispatch api_result=ERROR(`IM_REJECTED` 비-backpressure) → 현재 attempt FAIL 마감(error_code=DISPATCH_NO_RESPONSE\|IM_REJECTED) fail++ → **새 attempt(attempt_no+1) 생성 + next_check_at 갱신**; **slot 보유한 채** 재dispatch | TERRAFORM_JOB |
| DISPATCHING | DISPATCHING (in-place, **backpressure**) | dispatch가 IM **429/503**(backpressure) → **fail_count 미소모**(실패 아님), task_check(kind=DISPATCH, api_result=ERROR, error_code 미해당) 기록, **attempt 미마감(`finished_at=null` 유지) → 동일 attempt 재사용**(새 attempt 생성 안 함·`attempt_no` 불변; fail_count 미소모와 정합), **slot 보유**, next_check_at 미뤄 재dispatch (IM_REJECTED 하드 거부와 구분 — 그건 위 행 attempt 마감 fail++·새 attempt) | TERRAFORM_JOB |
...
| RUNNING | RUNNING (in-place) | poll observed=RUNNING(잡 진행 중) **또는 poll 호출 오류**(api_result=ERROR 비-backpressure — 잡 상태를 *못 읽음*이지 잡 실패 아님 → **fail 미소모**, task_check 기록, 재-poll) **또는 429/503 backpressure**(fail 미소모, Retry-After 후 재-poll) → slot 보유한 채 계속 폴링 | TERRAFORM_JOB |
| WAITING_EXTERNAL | WAITING_EXTERNAL (in-place) | NOT_MET(미가산) 또는 check ERROR(**비-backpressure**, fail++) 또는 429/503 backpressure(fail 미소모; 다음 check = `next_check_at=max(Retry-After, ≥10분 polling guard)`) → 계속 폴링 | CONDITION_CHECK |
...
4. **WAITING_EXTERNAL 자기루프(폴링)** — ≥10분 cadence(WAIT_EXTERNAL polling guard, 관리자 조정)마다 tick이
   ...
   "아직"은 실패 아님); api_result=ERROR(비-backpressure 호출 실패)면 fail_count++, < K면 계속(429/503 backpressure는 미가산).
5. **→ DONE** — observed=MET.
