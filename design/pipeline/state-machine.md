...
(다른 모든 파생을 누른다) > ② status=FAILED task 존재 → FAILED(**재시도 소진 fail_count==K 또는 HANDLER_NOT_FOUND 등 fail_count 미소모 영구 실패 포함**) > ③ TTL EXPIRED FAILED > ④ 전 task DONE.
...
2. **handler_key resolve**(READY 이상 task만; BLOCKED 제외) — 미해결이면 **즉시 FAILED**(forward·admit·poll보다 먼저; 존재하지 않는 handler로는 dispatch/poll할 수 없다, 종결표 HANDLER_NOT_FOUND 행).
...
4. **timeout 판정** — (3)에서 DONE이 아니면: TERRAFORM_JOB execution timeout → attempt 실패(fail++ < K면 RUNNING→READY, ==K면 FAILED) · CONDITION_CHECK TTL 초과 → EXPIRED.
...
| READY | DISPATCHING | tick: handler resolve 성공(평가 순서 2) 후 **admit** — COUNT(DISPATCHING\|RUNNING) < N일 때(READY TF = slot 큐); CAS + task_attempt 생성 + next_check_at | TERRAFORM_JOB |
...
| From | → To | 트리거 / Guard | kind |
|---|---|---|---|
| RUNNING | FAILED | tick: poll FAILED / execution timeout으로 fail_count++ == **K** → slot 반납 (DB: attempt result=FAIL + error_code=`JOB_FAILED`(poll observed=FAILED) \| `EXECUTION_TIMEOUT`(timeout); JOB_FAILED 관측 자체는 task_check.observed=FAILED가 보유 — api §0) | TERRAFORM_JOB |
...
| WAITING_EXTERNAL | FAILED | tick: check api_result=ERROR(**비-backpressure**)로 fail_count++ == **K** (NOT_MET·429/503 backpressure는 미가산 — "아직"·"나중에"는 실패 아님). 사유 `CHECK_ERROR`(또는 `CALL_TIMEOUT`)는 **그 호출의 `task_check.error_code`**가 보유(②task_check 귀속, api §0) | CONDITION_CHECK |
...
> 실패*(fail_count++, slot 해제 — 결정 4a; **DB 기록 = result=FAIL + error_code=EXECUTION_TIMEOUT**, 별도 result enum 아님)이며, **비-CANCELLING 일반 경로에서는 재시도 소진(==K) 시에만 FAILED**가 된다(미소진이면 RUNNING→READY 재큐). **CANCELLING drain 중에는 fail_count 잔여와 무관하게 직접 종결** — 취소 표 RUNNING 행 참조. (결정 4a timeout 표의
