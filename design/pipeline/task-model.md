...
- **TERRAFORM_JOB** — `dispatch` → job_id 획득 → `poll` → terminal. Infra Manager run API를
  ...
  active Terraform task 수를 slotCap (N) 이하로 admission 제어**하기 위해 공유 IM 동시성 slot(제약 #5)을
  소비한다(결정 4b).
- **CONDITION_CHECK** — dispatch 없음 → 조건 확인 반복 → MET. 외부 상태가 충족(MET)될 때까지
  `WAITING_EXTERNAL` 상태에 체류하며 폴링만 하는 task. dispatch가 없어 attempt도 없다.
...
- **handler 계약 (자동 레지스트리·중복 없음).** ① 각 handler는 **안정 `key()`를 선언**(클래스명과 무관 —
  ...
  **문자열이 아니라 class로 참조**(컴파일 타임 안전 — 오타·없는 handler는 컴파일 에러), 저장되는 `handler_key`는
  `handler.key()`에서 파생. ④ 두 단계 검증: (a) **부팅 시** default recipe가 참조하는 모든 handler가 등록됐는지
  assert(bad 배포 아티팩트 fail-fast), (b) **non-terminal pipeline의 `handler_key`가 전부 resolve되는지는
  pre-deploy CI 게이트가 authoritative** — 배포 전(promotion 전) prod DB의 non-terminal `handler_key` 대상으로
...
아래 task_check은 **CHECK 관측 run**으로 적힌다(연속 동일 관측 collapse·poll_count++; 관측 변화는 새 run —
RLE 후속17; DISPATCH만 호출당 1행). 정리하면 **attempt : handle = 1:1, attempt : task_check-run = 1:N**
(폴 횟수는 run의 poll_count) — handle이 1:1이지 check-run이 1:1이 아니다.
