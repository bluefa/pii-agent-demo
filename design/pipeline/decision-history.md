...
- **개정 6판 후속21 — N/M/K 변수명 의도 기반 영어 rename (사용자 지시; 표기만 변경·설계/수치/동작 무변경).** 사용자 지적: 단일 글자가 의도(특히 M=동시성 hard cap vs N=soft 제출 throttle 구분)를 못 담음. **Scheme A 채택(역할기반·기존명 활용):** `M → workerPoolSize`(고정 worker 풀 크기 = 동시 terraform 실행 hard cap), `N`/`N-cap → slotCap`(BFF admission 제출 throttle, slotCap ≈ workerPoolSize, soft), `K → maxFailCount`(task FAILED 전 최대 비성공 시도; `max_fail_count` snake 컬럼의 prose 약칭). **합성:** `N·K → slotCap × maxFailCount`·`N≈M → slotCap ≈ workerPoolSize`·`N≫M → slotCap ≫ workerPoolSize`. **적용 범위:** ADR + orchestrator-design·operations·state-machine·task-model·api·requirements·migrations + adr-016-html-spec; 각 문서 **첫 등장 1회만 `(M)/(N)/(K)` 약칭 병기** 후 descriptive 이름만. **보존(false positive 미변경):** `N pod(s)`·`N-pod`(다중 replica) · `N개·1:N·N번 폴링·N회 확인·N건 알림`(cardinality) · `max_fail_count`(snake DB 컬럼/설정 키). **이 history의 과거 엔트리는 append-only라 N/M/K 표기 그대로 두며 본 엔트리가 매핑 정본이다.** 적용 후 codex+opus 재리뷰로 점수 회귀 없음 확인(사용자 요구). → 전 문서 표기 규약 (2026-06-21)
