...
- **`task_check.attempt_id` (v1 추가)** — 비블로킹 async(D-T2)에서 tick은 발사한 호출의 관측을 **다음 tick에 committed row로 읽어** 전이한다(D-T4). 어느 attempt의 관측인지 식별이 필요한데, *wall-clock `started_at` 비교*는 시스템 시계가 뒤로 점프(NTP 보정)하면 직전 attempt의 stale 관측을 현재 것으로 오인해 **이중 fail**할 수 있다. 그래서 DISPATCH 행과 TERRAFORM_JOB poll run에 **`attempt_id`(nullable BIGINT)** 를 기록하고 tick은 `attempt_id`로 스코프한다(시계 무관·결정론적). CONDITION_CHECK check·synthetic 행은 attempt가 없어 null(MET/EXPIRED 읽기는 단일 시퀀스라 스코프 불요). 인덱스 `ix_check_attempt(task_id, kind, attempt_id, started_at DESC, id DESC)`. 동기 모델이던 원설계 §1.2엔 없던 컬럼 — async 단일-writer 상관을 명시적으로 만드는 구현 보강.
