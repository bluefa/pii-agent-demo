...
변경, pipeline_event 감사, 재배포 불요; settings API 편집 대상). **예외: `workerPoolSize`(worker 풀 크기)은 배포 설정이라
...
| workerPoolSize | 고정 (배포 설정 — **settings API 비편집**; 출발값은 IM worker 풀 스펙에 맞춤, 예 4~8) | 전역 | **동시 terraform 실행 hard cap** — 초과 제출은 pubsub 큐가 흡수; 재배포로만 변경; slotCap ≈ workerPoolSize의 기준 (4b) |
| slotCap | ≈ workerPoolSize (초안 3) | 전역 | pubsub 큐를 얕게 유지하는 제출 throttle(slotCap ≈ workerPoolSize); 동시성 hard cap은 workerPoolSize이지 slotCap 아님 (4b) |
