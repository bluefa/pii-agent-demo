...
| queue wait 길어짐 | slot(N) 포화 또는 worker 정체 | N·worker 용량 점검 |
| task FAILED | K회 시도 모두 실패 | error_code 확인 후 retry=새 run |

## 동시성 제어의 의미

- **N-cap (slot)** — BFF 발 terraform job을 N·K로 bound(무분별 방지). IM 429에 의존하지 않고 BFF가 자기 발 동시성을 스스로 묶는다(결정 4b).
- **max_external_calls_per_tick** — tick당 발사 호출 수 상한(burst 완화). 정확한 global 동시성 보장이 아니라 완화 장치이며 정밀 강제는 IM 429에 위임(결정 6 D-T7).
