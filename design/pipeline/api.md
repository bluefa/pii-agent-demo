...
- **Check** — `{ id, taskId, kind(DISPATCH|CHECK|POST_CHECK), name, apiResult(PENDING|OK|ERROR), observed(RUNNING|SUCCEEDED|FAILED|MET|NOT_MET), errorCode, externalHandle, checkedAt, latencyMs, detail{} }`  ← `name`=호출 라벨; `detail`=kind가 정의하는 **타입 결과**(예: TERRAFORM_JOB postCheck = `{ type:"TERRAFORM_LOG", logPointer, excerpt }`) — full terraform 로그는 `logPointer`로 IM 조회(BFF는 발췌만 보존)
