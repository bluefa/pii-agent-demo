...
- **Attempt** — `{ id, taskId, response{}, errorCode, startedAt, finishedAt, outcome(SUCCEEDED|FAILED|EXECUTION_TIMEOUT) }`  ← `response`=dispatch 원응답(TERRAFORM_JOB {jobId} · GENERAL_JOB {handle}, 단수); `errorCode`=attempt 실패 사유. **`outcome`은 DB 저장값이 아니라 `task_attempt.result` + `error_code`에서 파생한 API 표현**(DB는 `result(OK|FAIL)`+`error_code`만 저장; `outcome` 컬럼 없음):
...
> **"latest" 의미(결정 8 — 다중 non-terminal 가능):** 그 target의 **active** pipeline(현재/다음 실행 =
> `start_at ≤ now` 최소 non-terminal); non-terminal이 없으면 가장 최근 terminal run.
...
> **정확한 endpoint·트리거 주체(설치 시작 버튼 vs CONFIRMED 전이)는 ADR 결정이 아니라 실제 구현 시 기존 흐름과 배선하며 확정한다(out of ADR scope).** 기존 install/delete 흐름(ADR-006/009)이 그 path를 소유하므로 본 ADR은 고정하지 않는다. ADR-016이 못 박는 **생성 계약**은 결정 7·8뿐이다 — 생성 시 `(target,type)` recipe **resolution**(override vs 코드 default) → **cap 체크**(non-terminal ≤ `maxNonTerminalPipelinesPerTarget`) → **원자적** task row 생성 + snapshot 박제. 아래 endpoint 예시는 참고일 뿐 정본 아님.
...
```
{
  ...
  maxFailCount,
  maxNonTerminalPipelinesPerTarget,  // target당 non-terminal pipeline 상한 (default 3, 결정 8)
...
## 6. Custom Pipeline 설정 (결정 7)

TargetSource별 custom recipe override의 조회·편집. 표준 target은 행이 없고 코드 default를 resolve해 쓴다(결정 7).

### `GET /admin/target-sources/{targetSourceId}/pipeline-recipes/{type}`
- `type` = `INSTALL|DELETE`. custom override 또는 (없으면) 코드 default를 resolve해 반환.
- `200` → `{ source(CUSTOM|DEFAULT), version, type, tasks: [{ taskKey, order, kind, name }] }`

### `PUT /admin/target-sources/{targetSourceId}/pipeline-recipes/{type}`
- custom override 생성/교체(**full 교체**). 저장 시 catalog 검증(없는 `taskKey`·잘못된 순서·type 불일치 거부),
  성공 시 **version +1**, 변경은 `pipeline_event`로 감사. (정확성용 version 아님 — 재현=snapshot, skip=content-hash, 결정 7.3.)
- `200` → 갱신된 recipe. `409` → 동시편집 충돌(version mismatch). `422` → catalog 검증 실패.

> custom override는 **진행 중·과거 run에 영향 없다** — run은 생성 시 snapshot으로 구성을 고정한다(결정 7 Layer 3).
...
- **미확정 (O29 — write-once 캡처라 중요):** `detail`의 kind별 **정확한 스키마**와 **full terraform 로그 조회 경로**(logPointer 형식 · IM 로그 API 존재 여부 · 포인터 위임 vs BFF 프록시). 타입 그릇 패턴 자체는 결정됨(S27).
