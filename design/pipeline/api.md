...
- **Task** — `{ id, seq, name, kind(TERRAFORM_JOB|CONDITION_CHECK), status(BLOCKED|READY|DISPATCHING|RUNNING|WAITING_EXTERNAL|DONE|FAILED|EXPIRED|CANCELLED), dependsOn:[taskId], failCount, maxFailCount, latestCheck:Check? }`
...
- **Check** — `{ id, taskId, kind(DISPATCH|CHECK), name, apiResult(PENDING|OK|ERROR), observed(RUNNING|SUCCEEDED|FAILED|MET|NOT_MET), errorCode, externalHandle, checkedAt, latencyMs, detail{} }`  ← `name`=호출 operation 식별자(어떤 API/동작 — 예 `im.terraformApply`·`im.jobStatus`); `detail`(jsonb)=kind가 정의하는 타입 결과 그릇 — v1 미사용(컨테이너만 예약; terminal 스냅샷 캡처는 v2 defer)
...
> **정확한 endpoint·트리거 주체(설치 시작 버튼 vs CONFIRMED 전이)는 ADR 결정이 아니라 실제 구현 시 기존 흐름과 배선하며 확정한다(out of ADR scope).** 기존 install/delete 흐름(ADR-006/009)이 그 path를 소유하므로 본 ADR은 고정하지 않는다. ADR-016이 못 박는 **생성 계약**은 결정 7뿐이다 — 생성 시 `(type,provider)` 코드 default recipe **resolution** → **원자적** task row 생성 + snapshot 박제. 동일 target 중복 생성은 부분 unique 제약 `unique(target_source_id) WHERE non-terminal`(결정 5)이 잡아 **기존 non-terminal pipeline 1건을 반환**한다. 아래 endpoint 예시는 참고일 뿐 정본 아님.
