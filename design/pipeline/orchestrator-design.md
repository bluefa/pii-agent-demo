...
│                  │ pipeline_event   │      │ · slot 스케줄 (slotCap)   │   │
...
backpressure는 미가산 — 실패 아님). **max_fail_count = maxFailCount = 비성공 시도 상한이며, fail_count가 maxFailCount에 도달하면 task FAILED.**
...
**실제 동시 terraform 실행 수는 고정 크기 TerraformWorker 풀이 hard-cap한다(≤ workerPoolSize).** worker가 pubsub를
