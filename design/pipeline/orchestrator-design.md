...
- **이 파생 규칙은 병렬이 아니라 순서대로 평가된다 — CANCELLING이 최우선이다.** 위 세 파생
  ...
  *파생을 수행하는 시점에 pipeline.status가 CANCELLING인지*다 — 상태 기준이지 시간 기준이 아니다
  ...
  task가 있으면(재시도 소진 fail_count==K 또는 HANDLER_NOT_FOUND 등 fail_count 미소모 영구 실패) FAILED, **③ CANCELLING이 아니고** TTL EXPIRED task가 있으면
...
backpressure는 미가산 — 실패 아님). **max_fail_count = K = 비성공 시도 상한이며, fail_count가 K에 도달하면 task FAILED.**
어떤 사건이 fail_count를 올리는지는 kind에 따라 다르다:
- **TERRAFORM_JOB:** K = **초기 dispatch를 포함한 최대 attempt 횟수**(재dispatch 상한이 아니라 *최대 attempt 수*).
  매 비성공 attempt(IM 거부·crash 재dispatch·execution timeout)가 fail_count를 올려, K번째 attempt가 성과 없으면
  FAILED. 따라서 **총 attempt ≤ K, 그중 재dispatch ≤ K−1회**다(job poll 호출 오류는 fail 미소모라 attempt를 안 늘림 — §1.2).
- **CONDITION_CHECK:** dispatch·attempt가 없다(attempt 0개). K = **비-backpressure CHECK ERROR 허용 횟수**로,
  누적 CHECK ERROR가 K에 도달하면 FAILED(NOT_MET·backpressure는 미가산 — state-machine 88행). "총 attempt ≤ K"는 CONDITION_CHECK엔 비적용.

retry/orphan headroom 산정값 N·K의 K는 TERRAFORM_JOB 해석(최대 attempt 수)이다. BFF가 작업을 완료시키지
못한 것 자체가 실패이며 멱등으로 안전해도 사람이 봐야 한다. **별도의 dispatch 발사 카운터는 두지 않는다** —
시도-실패를 fail_count 하나로 세는 것이 의미상 단순하고 K 강제와 알림을 동시에 만족한다.

> **K는 crash-recovery headroom을 포함해 잡는다.** crash가 dispatch accepted~response 영속 *직전* 좁은 창에
...
> *K번* 연속으로 crash해야 FAILED이므로 확률은 무시 수준이나(좁은 창의 K제곱), K를 IM 스펙 최소가 아니라
