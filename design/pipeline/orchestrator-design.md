...
pipeline_event  id, pipeline_id?, task_id?, type, severity, payload(jsonb),
                ...
                --   비-pipeline 감사 이벤트(settings 변경·custom recipe 편집, api.md §4/§6)는 pipeline_id=null.
...
| PIPELINE_CANCELLING ([중단] 요청; actor=human) | info | 인앱 |
...
- **reconciler가 유일한 외부 호출자이자 task 전진의 단일 writer.** API/UI 액션은 외부 호출을
  하지 않고 task를 전진시키지 않는다. TaskKind별 dispatch/poll/postCheck의 모든 외부 호출과
...
**N-cap의 목표는 BFF가 새로 admit하는 TERRAFORM_JOB task 수를 제한하는 것이다.** BFF는 현재 slot을
보유한 task(DISPATCHING|RUNNING)를 COUNT하고, 그 수가 N 미만일 때만 READY 상태의 TERRAFORM_JOB
task를 admit한다. 따라서 happy path에서 BFF가 동시에 관리하는 active Terraform task 수는 N을 넘지
않는다.

단, 이 값은 TerraformWorker에서 실제로 동시에 실행 중인 모든 job 수의 hard cap이 아니다. BFF는 자신이
관측 중인 task 상태를 기준으로 admission을 제어한다. execution timeout, crash recovery, 결과 유실로
인해 이전 attempt가 worker에서 계속 실행되는 동안 새 attempt가 제출될 수 있으며, 이 경우 실제 worker
측 job 수는 BFF가 보는 active slot 수보다 많을 수 있다. 이 차이는 at-least-once dispatch와 worker
dedup 부재를 받아들이는 데 따른 비용이다.

K = max_fail_count는 admission cap이 아니라 retry/orphan headroom 산정 계수다. 하나의 admitted task는
최대 K번의 dispatch attempt를 만들 수 있으므로, Infra Manager/Worker 용량을 잡을 때는 N·K를 worst-case
제출 여유로 본다. 즉,

```text
N · K <= Infra Manager / Worker가 감당 가능한 worst-case 제출 여유
```

가 되도록 N과 K를 설정한다.

따라서 이 설계가 직접 보장하는 것은 다음이다.

```text
BFF-visible active Terraform task count <= N
```

반면 다음은 BFF 단독으로 보장하지 않는다.

```text
actual global TerraformWorker job concurrency <= N
actual global TerraformWorker job concurrency <= N · K
```
...
- **slot이 보호하는 것은 BFF-visible active Terraform task 수다.** 현재 slot을 소비하는 kind는
  TERRAFORM_JOB뿐이다. 다중 풀(named resource pool)이나 kind별 slot 분배는 두 번째 한도 backend가
  실재할 때 additive로 확장하며 지금은 만들지 않는다(YAGNI).
- **N-cap은 best-effort soft target이다 (불변식 아님) — 무엇에 대해 soft인가.** 단일 리더가 happy
  path에서 admission(COUNT→admit)을 직렬화해 BFF-visible active task 수를 N 이하로 유지한다. soft
  target이 깨지는 주요 경로는 leader failover split-brain(≤30초) 동안 두 pod가 동시에 COUNT→admit하는
  일시 이중 admit이다(서로 다른 task라 행 CAS가 못 막는다 — read-then-act, 3.2). 이 경우에도 폭주는
  아니며, 다음 tick들이 다시 BFF-visible active task count를 N으로 수렴시킨다.
- **그래서 admission을 CAS화하지 않는다.** "N을 절대 안 넘김"을 강제하려고 admission에 단일 행 CAS를
  들이면 dual-write가 생겨 오히려 복잡도가 높아진다. 목표가 BFF-visible active task 수의 happy-path
  admission 제어이므로 현재 COUNT→admit soft target으로 충분하다. (3.2 "상태기계 정확성은 leader 무관"과
  무모순 — N-cap은 정확성 불변식이 아니라 admission 제어 장치다.)
- N은 런타임 설정(Part II; 초안 N=3에서 상향), 변경은 이벤트로 감사.
- **글로벌 하드캡(모든 caller·수동 job·orphan job 합산)은 N-cap의 목표가 아니다 — 필요해지면 IM
  429/503.** 모든 caller, 수동 job, orphan job까지 포함한 global hard cap이 필요해지는 경우에는 BFF가
  아니라 모든 caller를 관측하는 Infra Manager 계층에서 429/503 backpressure를 제공해야 한다. 그 경우
  BFF는 429/503을 실패로 세지 않고 `next_check_at`을 뒤로 미루는 requeue로 처리한다.
...
- **실행은 active 1개만.** reconciler는 target별로 **`start_at`(= `scheduled_at`이 있으면 그것, 없으면
  ...
  N-cap)에 **per-target active 게이트**가 한 겹 더 얹힌다(결정 4b 연장). **게이트 적용 위치 = reconciler의
  due-task 선별**: active pipeline의 task만 후보로 두어 모든 전진 전이(BLOCKED→READY · READY→DISPATCHING ·
  READY→WAITING_EXTERNAL)를 차단한다(non-active pipeline은 아직 RUNNING task가 없어 poll 대상도 없다).
  state-machine.md 전진 전이표의 공통 전제다.
...
**cap도 같은 부류의 soft target이다** — 생성 시 COUNT→insert는 동시 생성 race로 일시적으로 cap을 넘을 수
있으나(N-cap COUNT→admit과 동형) 무해하며, 엄격 강제가 필요하면 생성 트랜잭션에 per-target advisory lock을
둔다(구현 선택 — 트리거 path와 함께, api.md §3).
