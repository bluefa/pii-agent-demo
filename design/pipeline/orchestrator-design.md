...
### 1.5 상태 전이 (Pipeline · Task 전이도)

§1.1(파생 우선순위)·§1.2(상태 집합)·결정 2(TaskKind)·결정 3.1(dispatch 5단계)·결정 4(timeout·취소)·
결정 6(단일 writer)에 흩어진 전이 규칙을 하나의 전이도로 합친다. **새 규칙이 아니라 기존 결정의 합성**
이며, 충돌 시 각 결정 본문이 정본이다. 모든 전이의 writer는 reconciler tick이다(상태 전이는 tick에서만 —
결정 6 D-T4; 관측·산출은 호출 스레드).
...
(DONE·FAILED·EXPIRED·CANCELLED). 보드 라벨 매핑은 §1.2 표.
...
*최대 attempt 수* — 최악 동시 job 회계 N·K의 K가 이것이다): 매 비성공 시도(crash 재dispatch 포함)가
...
동시 실행 TERRAFORM_JOB의 총량을 **N·K 이하로 bound**하는 장치다(N = slot cap, K = max_fail_count =
초기 dispatch 포함 최대 attempt 횟수; 재dispatch는 ≤ K−1). 목표는 글로벌 하드캡이 아니라 **무분별한 job 생성 방지** — Pipeline이 통제 없이
...
- **재dispatch 헤드룸 = N·K bound의 정량화.** worker dedup이 없어 crash 재dispatch가 낳은 고아 job과
  ...
  cap, K = max_fail_count = 초기 dispatch 포함 최대 attempt 횟수, 결정 3.1). 이 최악 상한을 IM 실제 수용량 이하로 두기 위해
