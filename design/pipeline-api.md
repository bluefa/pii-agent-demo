...
- **Attempt** — `{ id, taskId, response{}, startedAt, finishedAt, outcome(SUCCEEDED|FAILED|EXECUTION_TIMEOUT|WORKER_OUTAGE) }`  ← `response`는 dispatch 원응답(dispatch 종류별 응답 형태 흡수 — 단수 handle)
