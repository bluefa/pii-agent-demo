...
- **그룹 축은 `dag_registry`의 컬럼(`target_source_id`)이다** —
  DAG:논리DB가 1:1이고 논리DB는 target source 하나에 속하므로,
  멤버십은 DAG의 속성이다. 별도 join 테이블도, 이벤트·상태 행에
  그룹을 스탬핑하는 것도 없다. 편입은 write-once(불변)로 취급한다.
...
target source = 그룹, 그룹 하나가 DAG 최대 1만 개를 가진다.
우려는 "매 요청마다 멤버 id N개를 불러와서 N개를 조회"하는 부하인데,
그 패턴 자체를 없애는 게 답이다:

1. **멤버십을 우리 DB로 로컬화** — 어차피 도는 카탈로그 sync가
   dag→target_source_id를 함께 받아 `dag_registry` 컬럼에 upsert한다.
   조회 시점에 외부 시스템에 멤버 목록을 묻는 일이 없다.
2. **id 목록을 앱으로 가져오지 않는다** — 그룹 화면은
   `WHERE target_source_id = ?` + `(target_source_id, dag_name)`
   인덱스로 **그룹 안에서 keyset 페이지네이션**한다. 1만 개 id를
   메모리에 올려 IN절로 되던지는 단계가 아예 없고, 요청당 비용은
   페이지 크기(예: 100)에만 비례한다 — N이 1만이어도 동일하다.
3. **집계는 기존 주간 조회와 동일 경로** — 그룹 페이지의 DAG들에
   대해서만 `dag_run_status`를 (dag_id, logical_date) 인덱스로 읽는다.
   그룹 전용 집계 테이블이 필요 없다.

**불변성의 이용**: "한번 편입되면 영원히"이므로 sync upsert는
`COALESCE(target_source_id, new.target_source_id)` 가드로 첫 값을
고정한다 — 원천이 흔들려도 우리 쪽 멤버십은 이동하지 않는다. 같은
이유로, 나중에 캐시가 필요해져도 무효화 없이 TTL만으로 안전하다
(캐시가 틀릴 수 있는 방향이 "새 멤버가 TTL만큼 늦게 보임"뿐이고,
잘못된 멤버십은 원천적으로 불가능). 다만 인덱스 keyset 조회가 이미
싸므로 캐시는 넣지 않는다 — p95가 문제로 측정되면 그때 추가한다.

이벤트 경로는 그룹을 모른다 — 발송 시점(transport)에도 소비
시점에도 target_source_id를 알 필요가 없고, 그룹은 읽기 시점에
registry join으로만 해석된다. 신규 DAG의 멤버십은 다음 sync 주기에
나타난다(신규 DAG 자체의 노출 지연과 동일, 최대 1시간).

기각한 대안:

| 대안 | 기각 사유 |
|------|----------|
| `dag_run_status` 행에 target_source_id 스탬핑 (소비 시점) | 소비 콜백마다 registry 조회가 필요해 수집이 카탈로그 신선도에 결합되고, sync 이전에 도착한 이벤트는 NULL로 오염된다. 읽기 시점 join이면 전부 불필요 |
| 별도 멤버십 join 테이블 | DAG당 그룹이 최대 1개(1:1×N:1)라 컬럼으로 충분. 테이블 분리는 sync·조회 양쪽에 join만 추가 |
...
10. **카탈로그 원천이 dag→target_source_id를 주는지 확인**: 그룹
    컬럼은 카탈로그 sync에 실려 온다는 전제다. dag-id API 응답에
    target source가 없으면 매핑을 주는 별도 엔드포인트(논리 DB를
    생성하는 시스템이 보유)를 sync에 하나 더 붙인다 — 어느 쪽이든
    이벤트 경로는 불변.
