...
### DAG 카탈로그와 논리 DB (dag_registry)

`dag_registry`는 두 역할을 겸한다: **이름→논리 DB 매핑**이자, 주간
...
1. **초기 백필 1회**: 카탈로그 API의 목록(벌크) 조회로 10만 건 적재.
...
이벤트 처리 경로는 카탈로그 API를 전혀 호출하지 않는다 — API 장애가
...
- "주간 1회 이상 성공"의 성공 시각은 **가장 최근 성공**의
  eventTime을 쓴다.
- 논리 DB ID는 카탈로그 페이지에서 함께 읽으므로 항상 채워져 있다.
...
5. **카탈로그 API의 목록(벌크) 조회 유무**: 카탈로그 sync의 전제.
