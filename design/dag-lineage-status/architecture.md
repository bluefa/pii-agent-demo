...
2. **resolve (`DatabaseUriResolver`, 비동기)**: `database_uri IS
   NULL`인 이름만 골라 Pipeline Manager에 조회해 채운다. 벌크가 없으니
   단건 호출을 **50 병렬**로 돌리고(오너 확인 한도), 한 주기에 최대
   2000건까지 시도한다 — 이 상한이 곧 상류에 대한 레이트 리밋이고,
   콜드 스타트(10만 건) 소요를 결정한다(기본값 ~50분). 매핑이 1:1
   불변이라 **DAG 하나당 평생 1회**다. 실패는 `attempts`를 올리고 다음
   주기에 재시도하되, 상한(기본 5회)을 넘으면 멈춘다 — 상류가 영영
   모르는 이름을 매 주기 두드리지 않기 위해서다.
...
역방향이 없다는 뜻이다). 정상 상태에선 신규 DAG가 첫 이벤트를 낸 뒤
한 주기(기본 1분) 안이고, 콜드 스타트에서만 창이 길다 — 그래서
resolver를 50 병렬로 돌린다. 미해결 건수는 알람 대상이다(운영 절).
...
5. **Pipeline Manager 단건 호출의 지연·타임아웃**: 병렬 50은 확정
   됐지만(오너), 호출 1건의 지연을 모르면 한 주기(1분) 안에 2000건이
   실제로 끝나는지 알 수 없다. 100ms면 4초, 1s면 40초다. 실측 후
   `batch-size`와 타임아웃을 확정한다.
