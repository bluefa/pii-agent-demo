...
**Admin은 별도 navigation을 가진 독립 페이지.** "관리자" 진입점을 클릭하면 서비스
담당자 화면(7-step 프로세스)에서 **완전히 다른 페이지로 전환**된다.
메뉴는 5개: Queue Board / Service 관리 / 설치·삭제 파이프라인 / API 현황 / **가이드 관리**.

> **2026-08-12 개정 (PR #695)** — 진입점이 상단 nav 항목에서 **우측 계정 카드 안**으로
> 옮겨졌고 **ADMIN 에게만** 노출된다. 상단 바 항목이던 시절엔 비관리자 전원에게 눌러야
> 차단만 되는 목적지를 보여줬다. 노출 판정은 `lib/roles.ts` 의 `isAdminRole` 하나를 서버
> 게이트(`app/admin/layout.tsx`)와 공유한다 — 갈라지면 "메뉴는 보이는데 열면 차단"이 된다.
> 아래 IA 스케치의 상단 바 `[관리자●]` 는 개정 이전 모습이고, 그에 따라 관리자 화면에서
> 상단 바 활성 표시가 사라진 것도 오너가 수용했다.

```
┌─ 상단 nav: [SIT 연동]  [관리자●] ← 2026-08-12 계정 카드로 이동 ──┐
├──────────┬──────────────────────────────────────────────────────┤
