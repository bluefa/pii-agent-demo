...
public interface TaskAttemptRepository extends JpaRepository<TaskAttempt, Long> {
    ...
    @Query("update TaskAttempt a set a.response = :response "
            ...
            + "and t.status = com.bff.pipeline.type.TaskStatus.DISPATCHING)")
     ...
     */
    @Modifying
    @Query("update TaskAttempt a set a.result = com.bff.pipeline.type.AttemptResult.FAIL, "
            + "a.finishedAt = :now, a.errorCode = :errorCode "
            + "where a.id = :id and a.finishedAt is null")
    int closeFailed(@Param("id") Long id,
                    @Param("errorCode") @Nullable ErrorCode errorCode,
     ...
     */
    @Modifying
    @Query("update TaskAttempt a set a.result = com.bff.pipeline.type.AttemptResult.OK, "
