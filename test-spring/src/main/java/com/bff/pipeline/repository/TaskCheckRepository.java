...
public interface TaskCheckRepository extends JpaRepository<TaskCheck, Long> {
    ...
    Optional<TaskCheck> findFirstByTaskIdAndKindAndNameAndExternalHandleOrderByStartedAtDescIdDesc(
            Long taskId, CheckKind kind, String name, @Nullable String externalHandle);
    ...
    @Query("select coalesce(sum(c.pollCount), 0) from TaskCheck c where c.taskId = :taskId "
            + "and c.kind = com.bff.pipeline.type.CheckKind.CHECK "
            + "and c.apiResult = com.bff.pipeline.type.ApiResult.ERROR "
            + "and c.errorCode in (com.bff.pipeline.type.ErrorCode.CHECK_ERROR, com.bff.pipeline.type.ErrorCode.CALL_TIMEOUT)")
