    ...
    public ImFault classify(Throwable t) {
        ...
        if (t instanceof FeignException fe) {
            ...
            if (status == TOO_MANY_REQUESTS || status == SERVICE_UNAVAILABLE) {
                return ImFault.Backpressure.builder().retryAfter(retryAfter(fe.responseHeaders())).build();
            }
            return ImFault.HardError.builder().detail("HTTP " + status).build();
        }
        return ImFault.HardError.builder().detail(String.valueOf(t.getMessage())).build();
    }
