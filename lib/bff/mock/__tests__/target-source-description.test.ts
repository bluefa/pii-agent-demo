 ...
 * `supportRawData` 시드가 두 wire 에 실리는지, 그리고 §9 쓰기의 왕복.
 *
...
describe('mockTargetSources.setDoesSupportRaw', () => {
  ...
  afterEach(() => {
    const project = mockData.getProjectByTargetSourceId(RAW_TARGET);
    if (project) mockData.updateProject(project.id, { supportRawData: true });
  });

  it('끈 값이 다음 GET 에 그대로 보인다', async () => {
    const off = await mockTargetSources.setDoesSupportRaw(RAW_TARGET, false);
    expect(off.status).toBe(204);
    expect(readSupportRawData(await body(await mockTargetSources.get(String(RAW_TARGET))))).toBe(false);

    await mockTargetSources.setDoesSupportRaw(RAW_TARGET, true);
    expect(readSupportRawData(await body(await mockTargetSources.get(String(RAW_TARGET))))).toBe(true);
  });
...
describe('supportRawData 시드', () => {
  it('TargetSourceDetail 에 실린다 — 두 상태 모두', async () => {
    ...
    expect(readSupportRawData(raw)).toBe(true);
    // 근거가 없는 대상도 `false` 로 실린다: 끌 수 있는 값이 된 순간, 키가 없는 것은
    // "꺼짐" 이 아니라 "못 읽음" 이고 화면이 그 둘을 다르게 그린다.
    expect(readSupportRawData(plain)).toBe(false);
  });

  /**
   * 두 태그가 전부 이 한 홉에 걸려 있다: 이 두 응답이 아직 선언하지 않은 키라,
   * 생성 스키마가 `.passthrough()` 를 잃고 `.strip()` 로 바뀌면 파스가 조용히 키를
   * 버리고 태그는 모든 대상에서 꺼진다 — 목도 라우트도 에러를 내지 않으므로 나머지
   * 테스트는 전부 초록이다. gen:api 재생성이 그 성질을 바꾸면 여기서 걸린다.
   */
  it('생성 스키마가 선언되지 않은 키를 통과시킨다 (.passthrough)', () => {
    expect(schemas.TargetSourceInfo.parse({ targetSourceId: 1, supportRawData: true })).toHaveProperty(
      'supportRawData',
      true,
    );
    expect(schemas.TargetSourceDetail.parse({ targetSourceId: 1, supportRawData: true })).toHaveProperty(
      'supportRawData',
      true,
    );
  });
  ...
  it('TargetSourceInfo (camel wire) 에도 실린다 — /target-sources/page?serviceCode', async () => {
    ...
    expect(readSupportRawData(byId.get(RAW_TARGET))).toBe(true);
    expect(readSupportRawData(byId.get(PLAIN_TARGET))).toBe(false);
  });
});
