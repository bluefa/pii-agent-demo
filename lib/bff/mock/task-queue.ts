...
const PROC: ProcRow[] = [
  ...
  // 서비스 이름 99자 — 절단 확인용(오너 2026-08-20). 계약상 상한은 30자라 이것은
  // 상한 밖 입력이고, 그래서 표가 이름을 어디서 자르는지 보려면 이 한 건이 필요하다.
  { ts: 1388, svc: '과금 청구·수납·정산 원장 통합 관리 및 미납 채권 회수 자동화 플랫폼 (구독형 요금제·후불 정산·세금계산서 발행·카드 자동이체 재시도·연체 이자 산정 포함) 통합 운영계 서비스', code: 'BIL', pv: 'AWS', st: 'CONFIRMED', delay: 118000, at: '2026-07-19T10:53:00Z' },
...
const TS_DESCRIPTION: Record<number, string> = {
  ...
  // Target 번호로만 판단해야 한다.
  //
  // 1388 은 **절단 확인용 길이**다 (오너 2026-08-20: "길어지는 상황에서 어떻게
  // 표현되는지 한번 보자"). 설명 300자 · 서비스 이름 99자(PROC 1388). 평범한 길이만
  // 있는 픽스처에서는 이름·설명 열이 잘리는 것을 한 번도 볼 수 없어서, 두 열이 서로를
  // 밀어내는지 툴팁이 전문을 들고 있는지 확인할 방법이 없다.
  1388:
