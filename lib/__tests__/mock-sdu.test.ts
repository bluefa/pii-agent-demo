...
const SDU_PROJECT_ID_2 = 'sdu-proj-1';
...
describe('mock-sdu behavior lock-in', () => {
  ...
  describe('getSduInstallationStatus', () => {
    it('SDU project → 초기 installation status 반환', () => {
      const result = getSduInstallationStatus(SDU_PROJECT_ID);
    ...
    it('존재하지 않는 project → NOT_FOUND 에러', () => {
      const result = getSduInstallationStatus(NONEXISTENT_PROJECT_ID);
    ...
    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = getSduInstallationStatus(AWS_PROJECT_ID);
    ...
    it('캐시: 두번째 호출도 동일 객체 반환 (reference equality)', () => {
      const first = getSduInstallationStatus(SDU_PROJECT_ID);
      const second = getSduInstallationStatus(SDU_PROJECT_ID);
  ...
  describe('checkSduInstallation', () => {
    it('Math.random=0.5 → 진행 없음 (모든 조건 threshold 미달)', () => {
      getSduInstallationStatus(SDU_PROJECT_ID);
      const result = checkSduInstallation(SDU_PROJECT_ID);
    ...
    it('Math.random=0 → 모든 단계 진행 (crawler → athena → setup)', () => {
      ...
      const r1 = checkSduInstallation(SDU_PROJECT_ID);
      ...
      const r2 = checkSduInstallation(SDU_PROJECT_ID);
      expect(r2.data?.athenaSetup.status).toBe('COMPLETED');
    });

    it('존재하지 않는 project → NOT_FOUND 에러', () => {
      const result = checkSduInstallation(NONEXISTENT_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_FOUND');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = checkSduInstallation(AWS_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });
    ...
    it('lastCheckedAt 갱신 (fake timer tick)', () => {
      ...
      const second = checkSduInstallation(SDU_PROJECT_ID);
  ...
  describe('getS3UploadStatus', () => {
    it('SDU project → 초기 PENDING 상태 반환', () => {
      const result = getS3UploadStatus(SDU_PROJECT_ID);
    ...
    it('존재하지 않는 project → NOT_FOUND 에러', () => {
      const result = getS3UploadStatus(NONEXISTENT_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_FOUND');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = getS3UploadStatus(AWS_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });

    it('캐시: 두번째 호출도 동일 객체 반환', () => {
      const first = getS3UploadStatus(SDU_PROJECT_ID);
      const second = getS3UploadStatus(SDU_PROJECT_ID);
      expect(second.data).toBe(first.data);
    });
  });
  ...
  describe('checkS3Upload', () => {
    it('Math.random=0 → PENDING → CONFIRMED 전환', () => {
      vi.spyOn(Math, 'random').mockReturnValue(0);
      getS3UploadStatus(SDU_PROJECT_ID);

      const result = checkS3Upload(SDU_PROJECT_ID);
    ...
    it('이미 CONFIRMED → 그대로 반환 (추가 전환 없음)', () => {
      vi.spyOn(Math, 'random').mockReturnValue(0);
      getS3UploadStatus(SDU_PROJECT_ID);
      const firstConfirm = checkS3Upload(SDU_PROJECT_ID);
      expect(firstConfirm.data?.status).toBe('CONFIRMED');

      // Random 을 다시 0.5 로 바꿔도 CONFIRMED 유지
      vi.spyOn(Math, 'random').mockReturnValue(0.5);
      const second = checkS3Upload(SDU_PROJECT_ID);
      expect(second.data?.status).toBe('CONFIRMED');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = checkS3Upload(AWS_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });
  });
  ...
  describe('getIamUser', () => {
    it('SDU project → deterministic IAM user 생성', () => {
      const result = getIamUser(SDU_PROJECT_ID);
    ...
    it('존재하지 않는 project → NOT_FOUND 에러', () => {
      const result = getIamUser(NONEXISTENT_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_FOUND');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = getIamUser(AWS_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });

    it('캐시: 두번째 호출도 동일 객체 반환', () => {
      const first = getIamUser(SDU_PROJECT_ID);
      const second = getIamUser(SDU_PROJECT_ID);
      expect(second.data).toBe(first.data);
    });
  });
  ...
  describe('issueAkSk', () => {
    it('SDU project → accessKey AKIA prefix + 구조 확인', () => {
      const result = issueAkSk(SDU_PROJECT_ID, 'issuer@example.com');
    ...
    it('IAM user의 akSkIssuedBy 를 파라미터로 갱신', () => {
      getIamUser(SDU_PROJECT_ID); // 캐시 생성 (admin@example.com)
      issueAkSk(SDU_PROJECT_ID, 'new-issuer@example.com');
      const after = getIamUser(SDU_PROJECT_ID);
    ...
    it('존재하지 않는 project → NOT_FOUND 에러', () => {
      const result = issueAkSk(NONEXISTENT_PROJECT_ID, 'x@example.com');
      expect(result.error?.code).toBe('NOT_FOUND');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = issueAkSk(AWS_PROJECT_ID, 'x@example.com');
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });
  });
  ...
  describe('getSourceIpList', () => {
    it('SDU project → 초기 entries 2개 반환 (CONFIRMED + PENDING)', () => {
      const result = getSourceIpList(SDU_PROJECT_ID);
    ...
    it('존재하지 않는 project → NOT_FOUND 에러', () => {
      const result = getSourceIpList(NONEXISTENT_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_FOUND');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = getSourceIpList(AWS_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });
  });
  ...
  describe('registerSourceIp', () => {
    it('신규 CIDR 추가 → PENDING entry 로 append', () => {
      const result = registerSourceIp(SDU_PROJECT_ID, '172.16.0.0/16', 'alice@example.com');
      ...
      const listed = getSourceIpList(SDU_PROJECT_ID);
      expect(listed.data?.entries).toHaveLength(3);
      expect(listed.data?.entries[2].cidr).toBe('172.16.0.0/16');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = registerSourceIp(AWS_PROJECT_ID, '10.0.0.0/24', 'x@example.com');
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });

    it('존재하지 않는 project → NOT_FOUND 에러', () => {
      const result = registerSourceIp(NONEXISTENT_PROJECT_ID, '10.0.0.0/24', 'x@example.com');
      expect(result.error?.code).toBe('NOT_FOUND');
    });
  });
  ...
  describe('confirmSourceIp', () => {
    it('PENDING entry 를 CONFIRMED 로 전환', () => {
      const result = confirmSourceIp(SDU_PROJECT_ID, '192.168.1.0/24', 'bdc@example.com');
    ...
    it('등록되지 않은 CIDR → SOURCE_IP_NOT_REGISTERED', () => {
      const result = confirmSourceIp(SDU_PROJECT_ID, '99.99.99.0/24', 'bdc@example.com');
      expect(result.error?.code).toBe('SOURCE_IP_NOT_REGISTERED');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = confirmSourceIp(AWS_PROJECT_ID, '10.0.0.0/24', 'x@example.com');
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });
  });
  ...
  describe('getAthenaTables', () => {
    it('SDU project → 초기 tables 2개 반환 (deterministic 이름)', () => {
      const result = getAthenaTables(SDU_PROJECT_ID);
    ...
    it('존재하지 않는 project → NOT_FOUND 에러', () => {
      const result = getAthenaTables(NONEXISTENT_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_FOUND');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = getAthenaTables(AWS_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });
  });
  ...
  describe('getSduConnectionTest', () => {
    it('SDU project → 초기 NOT_TESTED 상태 반환', () => {
      const result = getSduConnectionTest(SDU_PROJECT_ID);
    ...
    it('존재하지 않는 project → NOT_FOUND 에러', () => {
      const result = getSduConnectionTest(NONEXISTENT_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_FOUND');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = getSduConnectionTest(AWS_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });

    it('캐시: 두번째 호출도 동일 객체 반환', () => {
      const first = getSduConnectionTest(SDU_PROJECT_ID);
      const second = getSduConnectionTest(SDU_PROJECT_ID);
      expect(second.data).toBe(first.data);
    });
  });
  ...
  describe('executeSduConnectionTest', () => {
    it('Math.random=0 (<0.8) → PASSED', () => {
      vi.spyOn(Math, 'random').mockReturnValue(0);
      const result = executeSduConnectionTest(SDU_PROJECT_ID);
    ...
    it('결과는 getSduConnectionTest 캐시에 반영', () => {
      vi.spyOn(Math, 'random').mockReturnValue(0);
      executeSduConnectionTest(SDU_PROJECT_ID);
      const cached = getSduConnectionTest(SDU_PROJECT_ID);
      expect(cached.data?.status).toBe('PASSED');
    });

    it('비-SDU project → NOT_SDU_PROJECT 에러', () => {
      const result = executeSduConnectionTest(AWS_PROJECT_ID);
      expect(result.error?.code).toBe('NOT_SDU_PROJECT');
    });
  });
  ...
  describe('store 격리 — 두 SDU project 간 cross-contamination 없음', () => {
    it('proj-sdu-001 의 IAM user 변경이 sdu-proj-1 에 영향 없음', () => {
      issueAkSk(SDU_PROJECT_ID, 'issuer-a@example.com');
      const other = getIamUser(SDU_PROJECT_ID_2);
