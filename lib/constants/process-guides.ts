 ...
 * AWS 자동 설치 프로세스 가이드
 *
 * stepNumber 는 `ProcessStatus` enum 값(1-7)과 동일하다.
 * 1: WAITING_TARGET_CONFIRMATION / 2: WAITING_APPROVAL / 3: APPLYING_APPROVED /
 * 4: INSTALLING / 5: WAITING_CONNECTION_TEST / 6: CONNECTION_VERIFIED /
 * 7: INSTALLATION_COMPLETE
 */
 ...
 * AWS 수동 설치 프로세스 가이드
 *
 * stepNumber 는 AWS_AUTO_GUIDE와 동일하게 `ProcessStatus` enum 값(1-7)과 정렬된다.
 */
