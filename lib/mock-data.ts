...
export const mockUsers: User[] = [
  ...
  },
  {
    ...
    //
    // 여덟 개인 이유는 **목록이 여러 장이 되게** 하려는 것이다(2026-08-17). 둘뿐이던
    // 때는 "내가 접근할 수 있는 서비스"가 한 장으로 끝나서 페이저도, 두 장째도 볼 수가
    // 없었다. 카탈로그 네 장에 골고루 걸치게 고른 건 그때 화면이 서버가 준 장 안에서만
    // 걸렀기 때문인데(갭 B6), 지금은 카탈로그를 끝까지 받아 거른 뒤 나누므로 어디에
    // 있든 상관없다 — 배치는 그대로 두되 이유는 이제 개수뿐이다.
    //
