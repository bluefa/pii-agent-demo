...
export const pipelineStyles = {
  ...
  modal: {
     ...
     *  우측 도킹(overlayPanel 이 overlay 를 대체). 폭 720 = dialogWide 와 같은 등급
     *  (4열 표의 최소 폭 — TaskDrawer 400 은 부족). 도킹면이라 radius 없이 border-l
     *  (TaskDrawer 문법), 패딩 0 — 머리(flex-none)와 본문(자기 스크롤)이 각자 여백을
     *  갖는다. `dialog` 를 통째로 대체한다(dialogApp 과 같은 규칙). */
    overlayPanel: 'fixed inset-0 z-50 flex justify-end bg-[rgba(16,24,40,0.5)]',
    dialogPanel:
      'w-[720px] max-w-[92vw] h-full flex flex-col overflow-hidden border-l border-[var(--pl-border)] bg-[var(--pl-bg-card)] shadow-[var(--pl-shadow-lg)]',
